...
### SetTenantLevel (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **set-AzApiManagementPolicy** imposta i criteri di ambito specificati per la gestione delle API.
...
### -ApiRevision
Identificatore della revisione API. Questo parametro è facoltativo. Se non specificato, il criterio verrà aggiornato nella revisione API attualmente attiva.
...
### -Format
Specifica il formato del criterio. Quando `application/vnd.ms-az-apim.policy+xml` si usa, le espressioni contenute nel criterio devono essere di escape XML. Quando si usa `application/vnd.ms-az-apim.policy.raw+xml` **non** è necessario che il criterio sia di escape XML.
Il valore predefinito è `application/vnd.ms-az-apim.policy+xml` .
...
### -PolicyUrl
URL in cui è ospitato il documento dei criteri. Questo parametro è obbligatorio se-Policy o-PolicyFilePath non è specificato.
