...
## Descrizione
Il cmdlet **set-AzExpressRouteConnection** aggiorna una connessione di route espressa creata tra un gateway Express Route e il peering del circuito di Route Express locale.
...
Il precedente creerà un gruppo di risorse, una WAN virtuale, una rete virtuale, un hub virtuale e un ExpressRouteSite in West US nel gruppo di risorse "testRG" in Azure. Un gateway ExpressRoute verrà creato in seguito nell'hub virtuale con due unità di scala.

Dopo aver creato il gateway, viene collegato al circuito ExpressRoute locale con il comando New-AzExpressRouteConnection.

La connessione viene quindi aggiornata per avere un RoutingWeight diverso usando il comando Set-AzExpressRouteConnection.
...
### -AuthorizationKey
Chiave di autorizzazione da usare per creare la connessione al gateway ExpressRoute.
...
### -InputObject
L'oggetto ExpressRouteConenction da aggiornare.
...
### -ResourceId
ID risorsa dell'oggetto ExpressRouteConenction da eliminare.
...
### -RoutingWeight
Il peso che deve essere assegnato alla connessione per il routing dei pacchetti.
