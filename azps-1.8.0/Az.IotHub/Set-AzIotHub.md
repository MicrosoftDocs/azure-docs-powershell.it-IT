...
### UpdateSku (impostazione predefinita)
```
...
### Esempio 1 aggiornare l'USK
```
PS C:\> Set-AzIotHub -ResourceGroupName "myresourcegroup" -Name "myiothub" -SkuName S1 -Units 5
```

Aggiornare l'USK a S1 e le unità a 5 per il IotHub denominato "myiothub"

### Esempio 2 aggiornare le proprietà di eventhub
```
PS C:\> Set-AzIotHub -ResourceGroupName "myresourcegroup" -Name "myiothub" -EventHubRetentionTimeInDays 4
```

Aggiornare il periodo di conservazione in giorni a 4 per gli eventi di telemetria e operationsmonitoringevents per la IotHub denominata "myiothub"

## PARAMETRI

### -CloudToDevice
Proprietà della coda di comandi cloud to Device. 
...
Contrassegno che specifica se le notifiche devono essere abilitate per il caricamento di file. 
...
### -FallbackRoute
Route di fallback per il routing
...
Nome del contenitore in cui caricare i file.
...
Numero massimo di recapito per le notifiche di caricamento file.  
...
Valore time to Live per i messaggi nella coda di notifica di caricamento file. 
...
### -FileUploadSasUriTtl
È ora di vivere per l'URI SAS generato per il caricamento di file. 
...
Stringa di connessione di archiviazione in cui caricare i file. 
...
Proprietà correlate al monitoraggio delle operazioni. 
...
### -Route
Route da aggiungere per il routing
...
### -RoutingProperties
Proprietà di routing per il routing dei messaggi agli endpoint esterni 
