...
## Descrizione
Il cmdlet **Get-AzIntegrationAccountAssembly** crea un nuovo assembly in un account di integrazione.

## ESEMPI

### Esempio 1: creare un nuovo assembly usando un file locale
```powershell
...
Crea un nuovo assembly usando il file locale che si trova nel percorso del file contenuto in "$localAssemblyFilePath".

### Esempio 2: creare un nuovo assembly usando i dati di byte
```powershell
...
Crea un nuovo assembly usando la matrice di byte contenuta in "$assemblyContent".

### Esempio 3: creare un nuovo assembly usando un collegamento contenuto
```powershell
...
Crea un nuovo assembly usando i dati a byte presenti nell'URL "$assemblyUrl". Questo è il metodo suggerito per la creazione di assembly di grandi dimensioni.
