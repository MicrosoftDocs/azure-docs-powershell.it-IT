...
## Sinossi
Crea un nuovo processo o aggiorna un processo esistente nell'abbonamento specificato.
...
### Esempio 1: creare un nuovo processo di ImportExport
```powershell
...
Questi cmdlet creano un nuovo processo di ImportExport.
...
### -BlobListBlobPath
Una raccolta di stringhe di percorsi BLOB.
...
Una raccolta di stringhe di prefisso BLOB.
...
### -CancelRequested
Indica se una richiesta è stata inviata per annullare il processo.
...
### -DiagnosticsPath
Directory BLOB virtuale in cui vengono archiviati i registri di copia e i backup dei file di manifesto dell'unità (se abilitati).
...
L'elenco di unità è un elemento obbligatorio per un processo di importazione; non viene specificato per i processi di esportazione.
...
URI relativo al BLOB del blocco che contiene l'elenco di percorsi BLOB o di prefissi di percorso BLOB come definiti sopra, a partire dal nome del contenitore.
Se il BLOB è nel contenitore radice, l'URI deve iniziare con $root.
...
Un percorso BLOB che punta a un BLOB di blocco che contiene un elenco di nomi BLOB non esportati a causa dello spazio di unità insufficiente.
...
### -Posizione
Specifica il percorso di Azure supportato in cui deve essere creato il processo
...
### -StorageAccountId
Identificatore delle risorse dell'account di archiviazione in cui verranno importati o esportati i dati.
...
<IDriveStatus [] >: elenco di unità fino a dieci che costituiscono il processo. L'elenco di unità è un elemento obbligatorio per un processo di importazione; non viene specificato per i processi di esportazione.
