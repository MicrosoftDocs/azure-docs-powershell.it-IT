...
## Descrizione
Il cmdlet **Get-AzApiManagementAuthorizationServerClientSecret** ottiene il segreto client del server di autorizzazione Gestione API di Azure.

## ESEMPI

### Esempio 1: ottenere un segreto del client del server di autorizzazione specificato per ID
```
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementAuthorizationServerClientSecret -Context $ApiMgmtContext -ServerId "0123456789"
```

Questo comando ottiene il segreto del server di autorizzazione specificato.
...
### -ResourceId
Identificatore delle risorse ARM del server di autorizzazione.
Se specificato cercherà di trovare il server di autorizzazione dall'identificatore.
Questo parametro è obbligatorio.
...
Se specificato troverà il server di autorizzazione dall'identificatore.
