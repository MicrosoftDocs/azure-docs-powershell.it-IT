...
## Sinossi
API per registrare un nuovo computer e quindi creare una risorsa rilevata in ARM
...
### Esempio 1: onboards il computer in cui ci si trova come computer connesso
```powershell
...
A bordo della macchina in cui ci si trova come computer connesso.

### Esempio 2: bordo di un computer remoto come dispositivo connesso
```powershell
...
Onboards un computer remoto come dispositivo connesso tramite la comunicazione remota di PowerShell.
Nota: in questo momento sono supportate solo le finestre come destinazione.
...
### -Posizione
La posizione per il ConnectedMachine creato.
...
### -Nome
Nome che verrà usato per questo computer.
Il nome host viene usato per impostazione predefinita.
...
### -Proxy
URI per il server proxy da usare
...
### -PSSession
Se specificato, il comando che viene eseguito da computer a Azure verrà eseguita all'interno di ogni sessione PSSession.
Nota: questo funziona solo su Windows per ora.
...
### -ResourceGroupName
Nome del gruppo di risorse a cui si vuole aggiungere il computer.
...
### -SubscriptionId
ID dell'abbonamento a cui si vuole aggiungere il computer.
