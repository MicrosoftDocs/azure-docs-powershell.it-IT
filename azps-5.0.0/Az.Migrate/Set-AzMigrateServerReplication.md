...
### Esempio 1: aggiornamento per ID
```powershell
...
### -InputObject
Specifica il server di replica per cui devono essere aggiornate le proprietà.
...
### -NicToUpdate
Aggiorna la scheda NIC per la VM di Azure da creare.
Per costruire, vedere la sezione Note per le proprietà di NICTOUPDATE e creare una tabella hash.
...
Specifica il set di disponibilità da usare per la creazione di VM.
...
### -TargetNetworkId
Aggiorna l'ID della rete virtuale nell'abbonamento a destinazione Azure a cui è necessario eseguire la migrazione del server.
...
### -TargetObjectID
Specifica il server replcating per cui devono essere aggiornate le proprietà.
...
Aggiorna l'ID del gruppo di risorse nell'abbonamento a destinazione Azure a cui deve essere eseguita la migrazione del server.
...
### -TargetVMSize
Aggiorna l'USK di Azure VM da creare.
...
INPUTOBJECT <IMigrationItem> : specifica il server di replica per cui devono essere aggiornate le proprietà. L'oggetto server può essere recuperato usando il cmdlet Get-AzMigrateServerReplication.
...
NICTOUPDATE <IVMwareCbtNicInput [] >: aggiorna la scheda NIC per la VM di Azure da creare.
  - `IsPrimaryNic <String>`: Valore che indica se si tratta della scheda NIC primaria.
  - `NicId <String>`: ID NIC.
  - `[IsSelectedForMigration <String>]`: Valore che indica se questa NIC è selezionata per la migrazione.
  - `[TargetStaticIPAddress <String>]`: Indirizzo IP statico.
