...
Il comportamento di confronto delle chiavi del set di dati di riferimento può essere impostato tramite questa proprietà.
Per impostazione predefinita, il valore è' Ordinal '-il che significa che il confronto tra maiuscole e minuscole verrà eseguito durante l'Unione di dati di riferimento con eventi o durante l'aggiunta di nuovi dati di riferimento.
Quando ' OrdinalIgnoreCase ' è impostato, verrà usato il confronto senza distinzione tra maiuscole e minuscole.
...
Per creare una tabella hash, vedere la sezione Note per le proprietà della proprietà.
...
Proprietà Key <IReferenceDataSetKeyProperty [] >: elenco delle proprietà chiave per il set di dati di riferimento.
  - `[Name <String>]`: Nome della proprietà Key.
  - `[Type <ReferenceDataKeyPropertyType?>]`: Il tipo della proprietà Key.
