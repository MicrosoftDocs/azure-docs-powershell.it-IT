...
## Descrizione
Crea una connessione ExpressRoute tra un circuito di ExpressRoute locale BGP che scruta il gateway ExpressRoute all'interno di un hub virtuale.
...
Il precedente creerà un gruppo di risorse, una WAN virtuale, una rete virtuale, un hub virtuale, un gateway di Route Express e un circuito di ExpressRoute con peering privato in West Central US nel gruppo di risorse "testRG" in Azure. Dopo aver creato il gateway, viene collegato al ExpressRoute Circuit peering usando il comando New-AzExpressRouteConnection.
...
### -AuthorizationKey
Chiave ottenuta dal proprietario del circuito ExpressRoute per poter creare una connessione con un gateway in un altro abbonamento.
...
ID risorsa del circuito Express Route peering a cui deve essere creata questa connessione Express Route gateway.
...
ExpressRouteGateway padre per la connessione.
...
### -ParentResourceId
ID risorsa dell'elemento padre ExpressRouteGateway per la connessione.
...
### -RoutingWeight
Peso per il routing dei pacchetti che deve essere assegnato alla connessione.
