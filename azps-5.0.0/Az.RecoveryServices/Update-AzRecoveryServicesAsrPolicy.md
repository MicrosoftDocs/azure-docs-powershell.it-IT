...
## Descrizione
Il cmdlet **Update-AzRecoveryServicesAsrPolicy** aggiorna i criteri di replica del ripristino del sito di Azure specificati.
...
Avvia l'operazione di aggiornamento dei criteri di replica di Azure ad Azure usando i parametri specificati e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
Avvia il criterio di replica di Update Azure to Azure usando i parametri specificati e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
### -AzureToVMware
Specifica l'Azure per il ripristino di emergenza vMWare.
...
### -HyperVToAzure
Parametro switch che indica che il criterio specificato viene usato per replicare le macchine virtuali Hyper-V in Azure.
...
### -MultiVmSyncStatus
Specifica lo stato di sincronizzazione di multiVm per il criterio.
...
Specifica l'ID dell'account di archiviazione di Azure della destinazione di replica. Usato come account di archiviazione di destinazione per la replica se non viene fornita un'alternativa durante l'abilitazione della replica tramite il cmdlet New-AzRecoveryServicesASRReplicationProtectedItem.
...
Ora in ore per mantenere i punti di ripristino dopo la creazione.
...
Il valore di soglia RPO in minuti per mettere in guardia.
...
### -VmmToVmm
Parametro switch che indica che il criterio specificato viene usato per replicare le macchine virtuali Hyper-V gestite da VMM tra due siti Hyper-V.
...
### -VMwareToAzure
Parametro switch che indica che il criterio specificato viene usato per replicare le macchine virtuali VMware in Azure.
