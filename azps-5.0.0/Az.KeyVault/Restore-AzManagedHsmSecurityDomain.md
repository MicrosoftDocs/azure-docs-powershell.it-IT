...
## Descrizione
Questo cmdlet Ripristina i dati del dominio di sicurezza di backup precedenti in un HSM gestito.
...
Prima di tutto, è necessario fornire le chiavi per decrittografare i dati del dominio di sicurezza. Quindi, il comando **Restore-AzManagedHsmSecurityDomain** Ripristina i dati del dominio di sicurezza di backup precedenti in un HSM gestito usando questi tasti.
...
Vedere esempi per il modo in cui viene costruito.
...
### -PassThru
Se specificato, viene restituito un valore booleano quando il cmdlet riesce.
...
### -SecurityDomainPath
Specificare il percorso per i dati del dominio di sicurezza crittografati.
