...
### ByHsmName (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **Restore-AzManagedHsmKey** crea una chiave nell'HSM gestito specificato.
Questa chiave è una replica della chiave di backup nel file di input e ha lo stesso nome della chiave originale.
Se l'HSM gestito ha già una chiave con lo stesso nome, questo cmdlet non riesce invece di sovrascrivere la chiave originale.
Se il backup contiene più versioni di una chiave, tutte le versioni verranno ripristinate.
L'HSM gestito in cui ripristinare la chiave può essere diverso dall'HSM gestito a cui è stato eseguito il backup della chiave.
Tuttavia, l'HSM gestito deve usare lo stesso abbonamento ed essere in un'area di Azure nella stessa geografia (ad esempio, Nord America).
...
Questo comando Ripristina una chiave, incluse tutte le relative versioni, dal file di backup denominato backup. blob nell'HSM gestito denominato testmhsm.
