...
## Descrizione
Il cmdlet Get-AzSqlInstanceKeyVaultKey ottiene informazioni sulle chiavi del Vault chiave in un'istanza di SQL Managed. Puoi visualizzare tutte le chiavi in un'istanza gestita o visualizzare un tasto specifico fornendo il KeyId.
...
Questo comando consente di ottenere tutte le chiavi del Vault chiave in un'istanza di SQL Managed.
...
Questo comando consente di ottenere la chiave di volta chiave con ID ' https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 '.

### Esempio 3: uso dell'oggetto Instance
```powershell
...
### Esempio 4: uso dell'ID risorsa istanza
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
