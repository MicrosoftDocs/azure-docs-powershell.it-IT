...
## Descrizione
Il cmdlet **Remove-AzSqlInstancePool** rimuove un pool di istanze SQL di Azure.

## ESEMPI

### Esempio 1: rimuovere un pool di istanze
```powershell
PS C:\> Remove-AzSqlInstancePool -ResourceGroup resourcegroup01 -Name instancePool0
```

### Esempio 2: rimuovere un pool di istanze in base all'identificatore delle risorse
```powershell
...
### Esempio 3: rimuovere un pool di istanze da un oggetto pool di istanze
```powershell
...
Questo comando rimuove un pool di istanze denominato instancePool0.
...
### -InputObject
Oggetto pool di istanze da rimuovere.
...
### -ResourceId
ID risorsa del pool di istanze da rimuovere.
