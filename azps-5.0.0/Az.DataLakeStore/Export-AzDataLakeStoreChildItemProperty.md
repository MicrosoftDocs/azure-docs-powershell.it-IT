...
## Sinossi
Esporta le proprietà (utilizzo del disco e ACL) per l'intero albero dal percorso specificato a un percorso di output
...
## Descrizione
L' **Export-AzDataLakeStoreChildItemProperty** viene usato per segnalare l'utilizzo dello spazio ADL o/e l'uso di ACL per la directory specificata, oltre a directory secondarie e file.
...
Ottenere l'utilizzo di ACL per tutte le sottodirectory e i file in/a. IncludeFile assicura che l'uso sia segnalato anche per i file. HideconsistentAcl in questo caso verrà visualizzato l'ACL di/a, non i relativi elementi figlio poiché tutti gli elementi figlio hanno lo stesso ACL di/a. Questo flag ignora l'output ACL del sottoalbero se tutti gli ACL sono uguali alla radice.
...
### -OutputPath
Percorso del file di output. Può essere un percorso locale o un percorso ADL. Per impostazione predefinita, è locale. Se si specifica SaveToAdl, si tratta di un percorso ADL nello stesso account
