...
## Sinossi
Reimposta i criteri di un tenant in Azure Attestationn.}
...
## Descrizione
Il cmdlet Reset-AzAttestationPolicy Reimposta i criteri di attestazione definiti dall'utente da un tenant in Azure Attestation.
...
Reimpostare i criteri per l'impostazione predefinita per il provider di attestazione *pshtest* per il tipo Tee *SgxEnclave*.
...
Se il provider di attestazione *pshtest* è configurato per l'uso del modello di attendibilità isolato, reimpostare il criterio per l'impostazione predefinita per il tipo di tee *SgxEnclave* includendo un criterio firmato.
...
### -Policy
Specifica il token Web JSON che descrive il documento di criteri da reimpostare.
...
Supportiamo quattro tipi di ambiente: SgxEnclave, OpenEnclave, CyResComponent e VBSEnclave.
