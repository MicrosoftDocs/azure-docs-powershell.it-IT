...
### Volume (impostazione predefinita)
```
...
Creare un processo di migrazione disco per eseguire la migrazione dei dischi all'unità e al volume della scala di destinazione.
...
Creare un processo di migrazione disco per eseguire la migrazione dei dischi alla condivisione di destinazione.
...
### -Dischi
Per costruire, vedere la sezione Note per le proprietà dei dischi e creare una tabella hash.
...
DISCHI <IDisk [] >: 
  ...
  - `[SharePath <String>]`: Percorso condivisione disco.
  - `[Status <DiskState?>]`: Lo stato del disco.
