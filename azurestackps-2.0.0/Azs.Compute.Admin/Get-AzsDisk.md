...
### Esempio 1: ottenere tutti i dischi 
```powershell
PS C:\> Get-AzsDisk
```

Senza parametri, `Get-AzsDisk` verranno elencati tutti i dischi.

### Esempio 2: ottenere un disco per nome
```powershell
...
Specifica un disco per il suo `Name` recupero.

### Esempio 3: ottenere un numero specificato di dischi
```powershell
...
Usa il `Count` parametro per recuperare un numero specifico di dischi.

### Esempio 4: ottenere tutti i dischi in una posizione specifica
```powershell
...
Usare il `ScaleUnit` `VolumeLabel` parametro or per elencare tutti i dischi in una posizione specifica
...
### -ScaleUnit
Unità di scala a cui appartiene la risorsa.
...
### -SharePath
La condivisione a cui appartiene la risorsa.
...
### -UserSubscriptionId
ID abbonamento utente a cui appartiene la risorsa.
...
### -VolumeLabel
Etichetta di volume del volume a cui appartiene la risorsa.
