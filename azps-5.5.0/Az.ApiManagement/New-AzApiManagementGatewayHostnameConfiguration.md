...
## DESCRIZIONE
Il cmdlet **New-AzApiManagementGatewayHostnameConfiguration crea** una configurazione del nome host per il gateway esistente.

## ESEMPI

### Esempio 1: Creare una configurazione del nome host per il gateway esistente
```powershell
...
Questo comando crea una configurazione del nome host "h01" per un gateway "g01".

## PARAMETERS

### -CertificateResourceId
Un identificatore di risorsa per l'ID certificato esistente. Questo parametro è obbligatorio.
...
Identificatore della connessione con il nome host del nuovo gateway.
...
Flag per applicare NegotiateClientCertificate.
Questo parametro è facoltativo.
