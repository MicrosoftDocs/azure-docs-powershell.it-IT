...
### GetTenantLevel (impostazione predefinita)
```
...
## DESCRIZIONE
Il cmdlet **Get-AzApiManagementPolicy** ottiene il criterio di ambito specificato.

## ESEMPI

### Esempio 1: Ottenere i criteri a livello di tenant
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -SaveAs "C:\contoso\policies\tenantpolicy.xml"
```

Questo comando recupera i criteri a livello di tenant e lo salva in un file denominato tenantpolicy.xml.

### Esempio 2: Ottenere i criteri di ambito del prodotto
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ProductId "0123456789"
```

Questo comando recupera i criteri di ambito del prodotto

### Esempio 3: Ottenere il criterio di ambito API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210"
```

Questo comando recupera i criteri di ambito API.

### Esempio 4: Ottenere il criterio di ambito delle operazioni
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210" -OperationId "777"
```

Questo comando recupera i criteri di ambito dell'operazione.

### Esempio 5: Ottenere il criterio di ambito tenant in formato RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Questo comando recupera i criteri dell'ambito tenant in formato di caratteri di escape non Xml.

## PARAMETERS

### -ApiId
Specifica l'identificatore dell'API esistente.
Se si specifica questo parametro, il cmdlet restituisce il criterio di ambito API.
...
### -ApiRevision
Identificatore della revisione API. Questo parametro è facoltativo. Se non viene specificato, il criterio verrà recuperato dalla revisione dell'API attualmente attiva.
...
Il valore predefinito per questo parametro è "xml".
...
Se si specifica questo parametro con *ApiId,* il cmdlet restituisce i criteri di ambito dell'operazione.
...
Se si specifica questo parametro, il cmdlet restituisce i criteri dell'ambito del prodotto.
...
Se non si specifica questo parametro, il risultato viene pipelined come sting.
