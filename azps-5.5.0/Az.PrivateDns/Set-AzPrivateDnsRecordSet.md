...
## SYNOPSIS
Aggiorna/imposta un set di record in un'area DNS privata.
...
## DESCRIZIONE
Il Set-AzPrivateDnsRecordSet cmdlet aggiorna un set di record nel servizio DNS privato di Azure da un oggetto RecordSet locale. È possibile passare un oggetto RecordSet come parametro o usando l'operatore pipeline. È possibile usare il parametro Confirm $ConfirmPreference Windows PowerShell per controllare se il cmdlet richiede conferma. Il set di record non viene aggiornato se è stato modificato nel DNS privato di Azure dopo il recupero dell'oggetto RecordSet locale. Questo fornisce protezione per le modifiche simultanee. È possibile disattivare questo comportamento usando il parametro Overwrite, che aggiorna il set di record indipendentemente dalle modifiche simultanee.
...
Il primo comando usa il cmdlet Get-AzPrivateDnsRecordSet per ottenere il set di record specificato e quindi lo archivia nella $RecordSet variabile. Il secondo e il terzo comando sono operazioni non in linea per aggiungere due record A al set di record. Il comando finale usa il cmdlet Set-AzPrivateDnsRecordSet per eseguire il commit dell'aggiornamento.
...
Il primo comando usa il cmdlet Get-AzPrivateDnsRecordSet per ottenere il set di record specificato e quindi lo archivia nella $RecordSet variabile. Il secondo comando aggiorna il record SOA specificato in $RecordSet. Il comando finale usa il cmdlet Set-AzPrivateDnsRecordSet per propagare l'aggiornamento in $RecordSet.
