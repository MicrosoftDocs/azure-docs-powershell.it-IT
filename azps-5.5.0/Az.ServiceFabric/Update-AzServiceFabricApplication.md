...
## SYNOPSIS
Aggiornare un'applicazione di tessuto di servizio. In questo modo è possibile aggiornare i parametri dell'applicazione e/o aggiornare la versione del tipo di applicazione che attiverà un aggiornamento dell'applicazione. Supporta solo ARM applicazioni distribuite.
...
## DESCRIZIONE
Questo cmdlet può essere usato per aggiornare i parametri dell'applicazione e la versione del tipo di applicazione. L'aggiornamento del parametro modifica il modello solo ARM lato client, solo quando viene usata una nuova versione, il comando attiva l'aggiornamento dell'applicazione. La versione del tipo specificata deve essere già creata nel cluster usando **New-AzServiceFabricApplicationTypeVersion.**
...
Questo esempio avvia l'aggiornamento di un'applicazione per aggiornare la versione tipo a "v2" e imposta anche alcuni parametri dei criteri di aggiornamento che avranno effetto rispetto all'aggiornamento corrente.
...
Specifica la percentuale massima di partizioni non erre per servizio consentite dai criteri di integrità per il tipo di servizio predefinito da usare per l'aggiornamento monitorato.
...
Specifica la durata, in secondi, dopo la quale Tessuto di servizio deve eseguire la verifica dell'integrità se la verifica dell'integrità precedente non riesce.
...
Specifica la percentuale massima di istanze dell'applicazione distribuite nei nodi nel cluster con lo stato di errore prima dell'errore dello stato di integrità dell'applicazione per il cluster.
