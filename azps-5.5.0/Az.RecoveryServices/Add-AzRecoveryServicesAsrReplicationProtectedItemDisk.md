...
## DESCRIZIONE
Il cmdlet **Add-AzRecoveryServicesAsrReplicationProtectedItemDisk** aggiunge il disco per la protezione per una macchina virtuale di Azure già protetta.
...
Avviare l'operazione per aggiungere la configurazione del disco specificata per la protezione.
...
Avviare l'operazione per aggiungere la configurazione del disco specificata per la protezione. Elemento protetto da replica di input di piping.

## PARAMETERS

### -AzureToAzureDiskReplicationConfiguration
Specifica la configurazione del disco da usare per la protezione del disco per lo scenario di ripristino di emergenza da Azure ad Azure.
...
### -InputObject
Oggetto di input per il cmdlet: oggetto elemento protetto da replica ASR corrispondente al nuovo disco da proteggere.
