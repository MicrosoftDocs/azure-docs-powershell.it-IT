...
Avvia l'operazione di creazione di elementi protetti da replica per una macchina virtuale all'interno del gruppo di posizionamento di prossimità e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario da Azure ad Azure).
...
Specifica l'ID vault segreto di crittografia del disco (crittografia disco di Azure) da usare come macchina virtuale di ripristino dopo il failover.
...
### -KeyEncryptionKeyUrl
Specifica l'URL della chiave di crittografia del disco con la versione (crittografia disco di Azure) da usare per la macchina virtuale di ripristino dopo il failover.
...
Specifica l'ID chiave-vault della chiave di crittografia del disco (crittografia del disco di Azure) da usare come macchina virtuale di ripristino dopo il failover.
...
### -ProcessServer
Server dei processi da usare per replicare il computer. Usare l'elenco dei server di processo nell'infrastruttura ASR corrispondente al server di configurazione per specificarne uno.
...
Specifica l'account di archiviazione per la diagnostica di avvio per il ripristino di Azure VM.
...
### -PiùVmm
Il parametro switch per specificare che l'elemento replicato è una macchina virtuale di Hyper-V replicata tra siti DI Hyper-V gestiti DA PIÙ.ER.
