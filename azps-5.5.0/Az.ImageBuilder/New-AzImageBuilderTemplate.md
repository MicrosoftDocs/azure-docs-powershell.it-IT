...
## SYNOPSIS
Creare un modello di immagine di macchina virtuale
...
### Esempio 1: Creare un modello di immagine di una macchina virtuale
```powershell
...
Questo comando crea un modello di immagine di macchina virtuale.
...
Durata massima di attesa durante la creazione del modello di immagine.
Omettere o specificare 0 per usare il valore predefinito (4 ore).
...
### -Personalizza
Specifica le proprietà usate per descrivere i passaggi di personalizzazione dell'immagine, ad esempio Origine immagine e così via. Per creare, vedere la sezione NOTE per le proprietà CUSTOMIZE e creare una tabella hash.
...
### -DefaultProfile
region HideParameter Le credenziali, l'account, il tenant e la sottoscrizione usati per le comunicazioni con Azure.
...
### -Distribuisci
I target di distribuzione a cui deve essere destinato l'output dell'immagine.
Per creare, vedere la sezione NOTE relativa alle proprietà DISTRIBUTE e creare una tabella hash.
...
### -Source
Descrive un'origine immagine di una macchina virtuale per la creazione, la personalizzazione e la distribuzione.
Per creare, vedere la sezione NOTE per le proprietà SOURCE e creare una tabella hash.
...
ID dell'identità assegnata all'utente.
...
Dimensioni del disco del sistema operativo in GB.
Omettere o specificare 0 per usare le dimensioni predefinite del disco del sistema operativo di Azure.
...
### -VMProfileVmSize
Dimensioni della macchina virtuale usata per creare, personalizzare e acquisire immagini.
Omettere o specificare una stringa vuota per usare la stringa predefinita (Standard_D1_v2).
...
### -VnetConfigSubnetId
ID risorsa di una subnet pre-esistente.
...
### Microsoft.Azure.PowerShell.Cmdlets.ImageMag.Models.Api20200214.IImageTemplate
...
CUSTOMIZE <IImageTemplateCustomizer[]>: specifica le proprietà usate per descrivere i passaggi di personalizzazione dell'immagine, come Origine immagine e così via.
  - `Type <String>`: il tipo di strumento di personalizzazione da usare nell'immagine. Ad esempio, "Shell" può essere un oggetto personalizzatore della shell
  - `[Name <String>]`: nome descrittivo per fornire contesto sull'attività di questo passaggio di personalizzazione

DISTRIBUTE <IImageTemplateDistributor[]>: gli obiettivi di distribuzione a cui deve essere destinato l'output dell'immagine.
  - `RunOutputName <String>`: nome da usare per il runOutput associato.
  - `Type <String>`: tipo di distribuzione.
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`: i tag che verranno applicati all'elemento dopo che questo è stato creato/aggiornato dal distributore.
    - `[(Any) <String>]`: indica che è possibile aggiungere qualsiasi proprietà all'oggetto.

SOURCE: <IImageTemplateSource> descrive un'origine immagine di una macchina virtuale per la creazione, la personalizzazione e la distribuzione.
  - `Type <String>`: specifica il tipo di immagine di origine da cui iniziare.
