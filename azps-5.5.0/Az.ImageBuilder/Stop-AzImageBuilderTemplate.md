...
## SYNOPSIS
Annullare la build di immagini a esecuzione lunga basata sul modello di immagine
...
### Esempio 1: Interrompere la creazione di modelli di immagini
```powershell
...
Questo comando interrompe la creazione di modelli di immagini.

### Esempio 2: Interrompere la creazione di modelli di immagini
```powershell
PS C:\> Start-AzImageBuilderTemplate -ImageTemplateName template-name-sn78hg -ResourceGroupName wyunchi-imagebuilder -AsJob 
...
### Microsoft.Azure.PowerShell.Cmdlets.ImageMag.Models.IImageMagIdentity
...
INPUTOBJECT <IImageBuilderIdentity> : parametro identity
  ...
  - `[RunOutputName <String>]`: il nome dell'output di esecuzione
