...
## DESCRIZIONE
Quando il livello del cloud è abilitato su un endpoint di server (una posizione specifica su un server registrato), questo comando può essere usato per richiamare tutti i file a più livelli sul disco locale. È consigliabile disabilitare l'applicazione dei livelli del cloud in questo endpoint server prima di iniziare il richiamo. Se il tiering è ancora in corso, il richiamo potrebbe comportare la visualizzazione di altri file in livelli che non vengono visualizzati per raggiungere l'obiettivo desiderato di tutto il contenuto che si trova sul disco locale.
