...
## DESCRIZIONE
Questo comando anregistrerà un server dal servizio di sincronizzazione di archiviazione. Avviso: l'annullamento della registrazione di un server comporta l'eliminazione a catena di tutti gli endpoint server su questo server. Deve essere chiamato solo quando si è certi che nessun percorso nel server deve più essere sincronizzato.
...
Questo comando annulla la registrazione del server, causando eliminazioni a catena di tutti gli endpoint del server su questo server.
