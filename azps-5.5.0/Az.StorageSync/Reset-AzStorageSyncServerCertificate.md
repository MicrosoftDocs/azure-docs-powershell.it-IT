...
## DESCRIZIONE
Questo comando rollirà il certificato del server di sincronizzazione dello spazio di archiviazione usato per descrivere l'identità del server nel servizio di sincronizzazione di archiviazione. Questa soluzione è pensata per essere usata negli scenari di risoluzione dei problemi. Quando si chiama questo comando, il certificato del server viene sostituito, aggiornando anche il servizio di sincronizzazione di archiviazione con cui è registrato il server inviando la parte pubblica della chiave. Poiché viene generato un nuovo certificato, viene aggiornata anche la data di scadenza del certificato. Questo comando può essere usato anche per aggiornare un certificato scaduto. Questo problema può verificarsi se un server è offline per un periodo di tempo prolungato.
...
Questo comando esegue il roll roll del certificato del server locale e informa in modo sicuro il servizio di sincronizzazione di archiviazione corrispondente della nuova identità del server.
