...
## DESCRIZIONE
Questo comando registra un server in un servizio di sincronizzazione di archiviazione, la risorsa principale per Azure File Sync. Viene creata una relazione di trust tra il server e il servizio di sincronizzazione di archiviazione che assicura canali sicuri di trasferimento e gestione dei dati. PowerShell o il portale di Azure possono quindi essere usati per configurare le sincronizzazioni in questo server. Un server può essere registrato solo con un singolo servizio di sincronizzazione di archiviazione. Se i server devono partecipare alla sincronizzazione dello stesso set di file, registrarli nello stesso servizio di sincronizzazione di archiviazione.
Il comando deve essere eseguito localmente nel server che deve essere registrato: viene eseguito direttamente o tramite una sessione remota di PowerShell. Un oggetto computer remoto non può essere accettato.
...
Questo comando registrerà il server locale su cui viene eseguito questo comando.
