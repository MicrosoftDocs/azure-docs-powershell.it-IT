...
## DESCRIZIONE
Il cmdlet **New-AzRelayAuthorizationRule** crea una nuova regola di autorizzazione per le entità di inoltro specificate (Spazio dei nomi/WcfRelay/HybridConnection).
...
Crea `AuthoRule1` con diritti **di** ascolto per lo spazio dei `TestNameSpace-Relay1` nomi.
...
Crea una regola di `AuthoRule1` autorizzazione **con diritti** di ascolto per WcfRelay. `TestWCFRelay1`
...
Crea `AuthoRule1` con diritti **di** ascolto per la connessione `TestHybridConnection` ibrida.
