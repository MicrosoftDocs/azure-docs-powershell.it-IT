...
### Esempio 1: Rigenerare le chiavi API per un membro member usando name
```powershell
...
Questo comando rigenera le chiavi API per un membro member usando name, resource group.

### Esempio 2: Rigenerare le chiavi API per un membro personale usando idenity
```powershell
PS C:\> $keyPair = Get-AzBlockchainMemberApiKey -BlockchainMemberName myblockchainhlqc92 -ResourceGroupName bc-rg
...
Questo comando rigenera le chiavi API per un membro member usando identity.
