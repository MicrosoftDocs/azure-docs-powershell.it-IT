...
## DESCRIZIONE
Il cmdlet New-AzEventHubNamespace crea un nuovo spazio dei nomi di tipo Hub eventi.
...
Crea uno spazio dei nomi Hub eventi MyResourceName nella posizione geografica specificata MyLocation, nel gruppo di \` \` risorse \` \` \` MyResourceGroupName. \`
...
Crea uno spazio dei nomi Hub eventi MyWithName nella posizione geografica specificata MyLocation, nel gruppo di risorse MyResourceGroupName e AutoInflate è abilitato con \` \` \` \` \` \` MaximumThroughputUnits 10.

### Esempio 3: Spazio dei nomi abilitato per Kafka
```powershell
...
Crea uno spazio dei nomi Hub eventi MyWithName nella posizione geografica specificata MyLocation, nel gruppo di risorse \` \` \` \` \` MyResourceGroupName \` con Kafka e AutoInflate abilitati.

### Esempio 4: Spazio dei nomi abilitato per ZoneRedundant
```powershell
...
### Esempio 5: Creazione di spazio dei nomi con Gestione identità in un cluster 
```powershell
...
### -ClusterARMId
ARM ID del cluster in cui deve essere creato lo spazio dei nomi
...
### -ZoneRedundant
Abilitazione o disabilitazione dell'area ridondante per lo spazio dei nomi
...
### Microsoft.Azure.Commands.EventHub.Models.PSAttributeAttributes
