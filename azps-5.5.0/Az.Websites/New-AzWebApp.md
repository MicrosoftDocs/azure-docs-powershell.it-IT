...
### -AppServicePlan
Nome del piano di servizio app o ID del piano di servizio dell'app. Se una WebApp e un piano di servizio per app sono in gruppi di risorse diversi, usare l'ID al posto del nome. L'ID del piano di servizio app può essere recuperato usando: $asp = Get-AzAppServicePlan -ResourceGroup myRG -Name MyWebapp $asp.id restituisce l'ID del piano di servizio dell'app.
