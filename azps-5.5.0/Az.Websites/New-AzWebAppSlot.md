...
## DESCRIZIONE
Il cmdlet **New-AzWebAppSlot** crea uno slot di Azure Web App in un dato gruppo di risorse che usa il piano di servizio app e il data center specificati.
...
Questo comando crea uno slot denominato Slot001 in un sito Contoso app Web esistente nel gruppo di risorse esistente denominato Default-Web-WestUS nel data center degli Stati Uniti occidentali.
Il comando usa un piano di servizio app esistente denominato ContosoServicePlan.

## PARAMETERS

### -AppServicePlan
Nome del piano di servizio app o ID del piano di servizio dell'app. Se il piano di servizio per slot e app si tratta di gruppi di risorse diversi, usare l'ID al posto del nome. L'ID del piano di servizio app può essere recuperato usando: $asp = Get-AzAppServicePlan -ResourceGroup myRG -Name MyWebapp $asp.id restituisce l'ID del piano di servizio dell'app.
...
L'opzione Hashtable viene sovrascritta nelle impostazioni delle app
