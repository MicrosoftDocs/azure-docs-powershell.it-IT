...
## DESCRIZIONE
Il cmdlet New-AzDataMigrationTask crea un'attività di migrazione dei dati. Questo cmdlet accetta i parametri per il tipo di attività, il gruppo di risorse di Azure, il nome del servizio di migrazione dei database di Azure associato e Project come input. 
...
Questo script di esempio mostra come creare una nuova attività di migrazione dei dati denominata MyMigrationTask nel progetto denominato myDMSProject e il servizio denominato TestService. 
...
### -MigrationValidation 
Oggetto risposta attività per chiamata di convalida, facoltativo ma consigliato.
...
### -Wait
Se aspettare il completamento dell'attività. Se il contrassegno è impostato, controlla ogni un secondo finché l'attività non viene completata e torna all'utente le proprietà dell'attività in cui è possibile controllare l'output o l'errore. 
```yaml
