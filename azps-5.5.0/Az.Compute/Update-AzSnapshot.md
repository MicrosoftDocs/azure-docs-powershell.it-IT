...
Il primo comando crea un oggetto di aggiornamento snapshot vuoto locale con dimensioni di 10 GB Premium_LRS tipo di account di archiviazione.  Configura anche il tipo di sistema operativo Windows e abilita le impostazioni di crittografia.
Il secondo e il terzo comando impostano le impostazioni della chiave di crittografia disco e della chiave di crittografia per l'oggetto di aggiornamento snapshot.
L'ultimo comando accetta l'oggetto di aggiornamento snapshot e aggiorna uno snapshot esistente con il nome 'Snapshot01' nel gruppo di risorse 'ResourceGroup01'.
...
Questi comandi aggiornano anche uno snapshot esistente con il nome "Snapshot01" nel gruppo di risorse "ResourceGroup01" alle dimensioni del disco di 10 GB.
