...
Il primo comando crea un oggetto di aggiornamento disco vuoto locale con dimensioni di 10 GB Premium_LRS tipo di account di archiviazione. Configura anche il tipo di sistema operativo Windows e abilita le impostazioni di crittografia. Il secondo e il terzo comando impostano le impostazioni della chiave di crittografia disco e della chiave di crittografia per l'oggetto di aggiornamento del disco.
...
### -DiskMBpsReadOnly
La velocità effettiva totale (MBps) consentita in tutte le macchine virtuali che montaggio il disco condiviso come ReadOnly. MBps significa milioni di byte al secondo - MB usa la notazione ISO, ovvero potenze di 10.
...
### -MaxSharesCount
Il numero massimo di vm che è possibile collegare al disco contemporaneamente. Il valore maggiore di uno indica un disco che può essere montato su più VM contemporaneamente.
