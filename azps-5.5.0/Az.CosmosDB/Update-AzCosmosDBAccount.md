...
## SYNOPSIS
Aggiornare gli attributi di un accountDbDb.
...
## DESCRIZIONE
Aggiornare le proprietà di un account Dibdb. Non è possibile aggiornare le aree account in modo simulato con altre proprietà.
...
Aggiornato DefaultConsistencyLevel a "Strong", abilitato AutomaticFailover, Enabled MultipleWriteLocations e Enabled VirtualNetwork per l'account CpsDB con nome accountName. 
...
Livello di coerenza predefinito dell'account di database DB di Coeli.
...
### -MaxStalenessPrefix
Quando viene usato con la coerenza della coerenza dei dati associati, questo valore rappresenta il numero di richieste non più corrette.
