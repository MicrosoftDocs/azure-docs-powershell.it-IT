...
## DESCRIZIONE
Il cmdlet **Get-AzStorageBlobQueryResult** applica una semplice istruzione Structured Query Language (SQL) sui contenuti di un BLOB e salva il sottoinsieme dei dati in query in un file locale.
...
Questo comando esegue una query in ccn di un BLOB con configurazione di input come CSV e configurazione di output come json e salva l'output nel file locale "c:\resultfile.js".
...
Questo comando ottiene prima di tutto un oggetto BLOB per lo snapshot BLOB, quindi esegue una query per lo snapshot BLOB e mostra il risultato include l'errore della query.
...
Configurazione usata per gestire il testo di input della query. Creare un oggetto configurazione con New-AzStorageBlobQueryConfig.
...
Configurazione usata per gestire il testo di output della query. Creare un oggetto configurazione con New-AzStorageBlobQueryConfig.
...
### -PassThru
Restituisce se la query del BLOB specificato viene eseguita correttamente.
