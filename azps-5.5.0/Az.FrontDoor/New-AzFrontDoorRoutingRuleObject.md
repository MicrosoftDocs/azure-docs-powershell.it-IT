...
### Esempio 1: Creare un oggetto PSRoutingRuleObject per la creazione in primo piano con una regola di inoltro
```powershell
...
### Esempio 2: Creare un oggetto PSRoutingRuleObject per la creazione in primo piano con una regola di reindirizzamento
```powershell
...
### -AcceptedProtocol
Combinazioni di protocolli corrispondenti a questa regola.
...
### -BackendPoolName
ID risorsa del BackendPool a cui la regola si instrada
...
### -Custom Deframment
Frammento da aggiungere all'URL di reindirizzamento. Frammento è la parte dell'URL che viene dopo #. Non includere il valore di errore #.
...
### -CustomHost
Host da reindirizzare. Lasciare vuoto per usare l'host della posta in arrivo come host di destinazione.
...
### -CustomQueryString
Set di stringhe di query da inserire nell'URL di reindirizzamento. L'impostazione di questo valore sostituirebbe qualsiasi stringa di query esistente; lasciare vuoto per mantenere la stringa di query in ingresso. La stringa di query deve essere in <key>=<value> . Il primo? e & verranno aggiunte automaticamente, quindi non includerle nella parte frontale, ma separare più stringhe di query con &.
...
### -EnabledState
Se abilitare o meno l'uso di questa regola.
Il valore predefinito è Abilitato
...
### -ForwardingProtocol
Il protocollo che verrà utilizzato da questa regola per l'inoltro del traffico al valore predefinito back-end è MatchRequest.
...
### -PatternToMatch
I modelli di percorso della regola, Non devono avere * tranne eventualmente dopo il finale / alla fine del percorso. Il valore predefinito è /*
...
### -RedirectProtocol
Protocollo della destinazione a cui viene reindirizzato il traffico. Il valore predefinito è MatchRequest
...
### -RedirectType
Tipo di reindirizzamento che la regola userà per il reindirizzamento del traffico. Il valore predefinito viene spostato
...
### -RulesToolName
Riferimento a una specifica configurazione del motore di regole da applicare a questa route.
