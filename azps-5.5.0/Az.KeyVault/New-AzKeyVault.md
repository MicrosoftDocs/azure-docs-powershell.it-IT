...
Questo comando crea una chiave vault denominata Contoso03Vault, nell'area di Azure negli Stati Uniti orientali. Il comando aggiunge la chiave vault al gruppo di risorse denominato Gruppo14. Poiché il comando non specifica un valore per il *parametro SKU,* crea un vault chiave standard.
...
Questo comando crea un vault delle chiavi, come nell'esempio precedente. Specifica tuttavia un valore Premium per il parametro *SKU* per creare un vault di chiavi Premium.
...
Se specificato, la protezione contro l'eliminazione immediata è abilitata per il vault; richiede che sia abilitata anche l'eliminazione rescisa.
...
Se specificato, consente di autorizzare le azioni dei dati dal controllo dell'accesso basato sui ruoli, quindi i criteri di accesso specificati nelle proprietà del vault verranno ignorati. Si noti che le azioni di gestione sono sempre autorizzate con il controllo degli accessi in base al ruolo.
