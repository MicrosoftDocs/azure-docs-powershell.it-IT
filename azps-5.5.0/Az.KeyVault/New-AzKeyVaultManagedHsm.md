...
## DESCRIZIONE
Il cmdlet **New-AzKeyVaultManagedHsm** crea un servizio HSM gestito nel gruppo di risorse specificato. Per aggiungere, rimuovere o elencare chiavi nell'HSM gestito, l'utente deve concedere le autorizzazioni aggiungendo l'ID utente all'amministratore.

## ESEMPI

### Esempio 1: Creare un HSM gestito da StandardB1
```powershell
...
Questo comando crea un servizio HSM gestito denominato myhsm nella posizione eastus2euap. Il comando aggiunge il servizio HSM gestito al gruppo di risorse denominato myrg1. Poiché il comando non specifica un valore per *il parametro SKU,* crea un Standard_B1 HSM gestito.

### Esempio 2: Creare un servizio HSM gestito da CustomB32
```powershell
...
Questo comando crea un HSM gestito, come nell'esempio precedente. Tuttavia, specifica un valore CustomB32 per il parametro *SKU* per creare un HSM gestito da CustomB32.

## PARAMETERS

### -Administrator
ID oggetto amministratore iniziale per questo pool HSM gestito.
...
### -Location
Specifica l'area geografica di Azure in cui creare il vault della chiave.
Usare il comando Get-AzResourceProvider con il parametro ProviderNtassi per visualizzare le opzioni disponibili.
...
Il nome può essere qualsiasi combinazione di lettere, cifre o trattini.
...
### -SKU
Specifica lo SKU dell'istanza di HSM gestita.
