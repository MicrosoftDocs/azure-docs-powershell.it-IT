...
## DESCRIZIONE
Il cmdlet **Remove-AzKeyVaultCertificate** rimuove un certificato da un vault delle chiavi.
...
Questo comando rimuove il certificato denominato SelfSigned01 dalla chiave vault denominata ContosoKV01.
Questo comando specifica il *parametro Force.*
Di conseguenza, il cmdlet non richiede conferma.

### Esempio 2: Ripulire definitivamente il certificato eliminato dal vault delle chiavi
```powershell
PS C:\> Remove-AzKeyVaultCertificate -VaultName 'Contoso' -Name 'MyCert' -InRemovedState
```

Questo comando rimuove definitivamente il certificato denominato "MyCert" dalla chiave vault denominata "Contoso".
L'esecuzione di questo cmdlet richiede l'autorizzazione di eliminazione, che deve essere stata concessa in precedenza ed esplicitamente all'utente in questo vault della chiave.
...
### -Name
Specifica il nome del certificato che questo cmdlet rimuove da un vault delle chiavi.
Questo cmdlet crea il nome di dominio completo (FQDN) di un certificato in base al nome specificato da questo parametro, al nome del vault delle chiavi e all'ambiente corrente.
...
### -VaultName
Specifica il nome del vault della chiave da cui questo cmdlet rimuove un certificato.
