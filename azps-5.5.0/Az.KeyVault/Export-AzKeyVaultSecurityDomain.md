...
## DESCRIZIONE
Esporta i dati del dominio di sicurezza di un servizio HSM gestito per l'importazione in un altro servizio HSM.
...
Questo comando recupera il file HSM denominato testmhsm gestito e salva una copia di backup del dominio di sicurezza HSM gestito nel file di output specificato.

## PARAMETERS

### -Certificates
Percorsi dei certificati usati per crittografare i dati del dominio di sicurezza.
...
### -Force
Specificare se sovrascrivere il file esistente.
...
### -OutputPath
Specificare il percorso in cui verranno scaricati i dati del dominio di sicurezza.
...
### -São
Il numero minimo di condivisioni necessario per decrittografare il dominio di sicurezza per il ripristino.
