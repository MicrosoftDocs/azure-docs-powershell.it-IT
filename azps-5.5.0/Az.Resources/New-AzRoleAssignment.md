...
Per impostazione predefinita, viene selezionata l'abbonamento. L'ambito dell'assegnazione può essere specificato usando una delle combinazioni di parametri a seguenti.
...
c.
ResourceName, ResourceType, ResourceGroupName e (facoltativamente) ParentResource: per specificare una particolare risorsa all'interno di un gruppo di risorse a cui concedere l'accesso.
...
Concedere l'accesso al ruolo di lettore a un utente in un ambito di gruppo di risorse con l'assegnazione di ruolo disponibile per la delega
...
Concedere l'accesso a un utente a una risorsa (sito Web)
...
Concedere l'accesso come lettore a un'entità servizio

## PARAMETERS

### -AllowDelegation
Il contrassegno di delega durante la creazione di un'assegnazione di ruolo.
...
### -ApplicationId
ID applicazione dell'attributo ServicePrincipal
...
### -Condizione
Condizione da applicare all'assegnazione di ruoli.
...
### -InputFile
Percorso del json dell'assegnazione di ruoli
...
Deve essere usato solo in combinazione con i parametri ResourceGroupName, ResourceType e ResourceName per creare un ambito gerarchico in forma di URI relativo che identifica una risorsa.
...
Crea un'assegnazione efficace per il gruppo di risorse specificato.
Se usato insieme ai parametri ResourceName, ResourceType e (facoltativamente) ParentResource, il comando crea un ambito gerarchico sotto forma di URI relativo che identifica una risorsa.
...
Deve essere usato solo in combinazione con i parametri ResourceGroupName, ResourceType e (facoltativamente)ParentResource per creare un ambito gerarchico sotto forma di URI relativo che identifica una risorsa.
...
Deve essere usato solo in combinazione con i parametri ResourceGroupName, ResourceName e (facoltativamente) ParentResource per creare un ambito gerarchico sotto forma di URI relativo che identifica una risorsa.
...
### -RoleDefinitionName
Nome del ruolo RBAC che deve essere assegnato all'entità, ad esempio Lettore, Collaboratore, Amministratore rete virtuale e così via.
...
Se non è specificato, creerà l'assegnazione di ruolo a livello di sottoscrizione.
