...
### ListOperation (impostazione predefinita)
```
...
## DESCRIZIONE
Il Get-AzManagementGroup cmdlet di gestione ottiene tutto o un gruppo di gestione specifico.
...
### Esempio 3: Ottenere un gruppo di gestione specifico e il primo livello di gerarchia
```
...
Con il `Expand` contrassegno, è possibile spostarsi nella `Children` matrice e ottenere i dettagli per ogni elemento figlio. Ad esempio, `Children[0]` verranno fornite informazioni dettagliate sul gruppo con il nome `TestGroup1DisplayName` visualizzato.

### Esempio 4: Ottenere un gruppo di gestione specifico e tutti i livelli di gerarchia
```
