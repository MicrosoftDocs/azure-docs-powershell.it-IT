...
## DESCRIZIONE
Il cmdlet **Set-AzResourceLock** modifica un blocco di risorse.

## ESEMPI

### Esempio 1: Aggiornare le note per un blocco
```
PS C:\>Set-AzResourceLock -LockLevel CanNotDelete -LockNotes "Updated note" -LockName "ContosoSiteLock" -ResourceName "ContosoSite" -ResourceType "microsoft.web/sites" -ResourceGroupName "ResourceGroup11"
```

Questo comando aggiorna la nota per un lucchetto denominato ContosoSiteLock.
...
### -LockId
Specifica l'ID del blocco modificato dal cmdlet.
...
Attualmente l'unico valore valido è CanNotDelete.
...
### -LockName
Specifica il nome del blocco modificato dal cmdlet.
...
Ad esempio, per specificare un database, usare il formato seguente: ID sottoscrizione ID gruppo di risorse Nome server nome database Per specificare un gruppo di risorse, usare il `/subscriptions/` `/resourceGroups/` formato `/providers/Microsoft.Sql/servers/` `/databases/` seguente: `/subscriptions/` NOME `/resourceGroups/` gruppo di risorse ID sottoscrizione
