...
## SYNOPSIS
Creare una nuova configurazione del controllo del controllo del codice sorgente di Kubernetes.
...
### Esempio 1: Creare una configurazione per il cluster kubernetes
```powershell
...
Questo comando crea una configurazione per il cluster kubernetes.

### Esempio 1: Creare una configurazione per il cluster kubernetes con specify paramter OperatorParamTer
```powershell
...
Questo comando crea una configurazione nel nuovo spazio dei nomi dell'operatore per il cluster kubernetes.
Tenere presente che non è possibile creare una configurazione in uno spazio dei nomi operatore esistente.
...
### -ClusterScoped
Se l'ambito della configurazione è stato superato, viene impostato su Cluster (l'impostazione predefinita è nameSpace).
...
### -ClusterType
Nome della risorsa cluster Kubernetes: managedClusters (per i cluster AKS) o connectedClusters (per i cluster K8S OnPrem).
...
### -EnableHelmOperator
Opzione per abilitare Operatore operatore operatore per questa configurazione git.
...
### -IntasatoOperatorChartValues
I valori vengono sovrascritti per il grafico a barre dell'operatore.
...
### -CorriereChartVersion
Versione del grafico a barre dell'operatore.
...
Nome dell'istanza dell'operatore, che identifica la configurazione specifica.
...
### -OperatorEnne
Spazio dei nomi in cui è installato l'operatore.
Massimo 253 caratteri alfanumerici minuscoli, solo trattini e punti.
...
### -OperatorParameters
Qualsiasi parametro per l'istanza operatore in formato stringa.
...
### -RepositoryUrl
URL del repository SourceControl.
