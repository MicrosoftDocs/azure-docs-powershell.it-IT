...
### Esempio 1: Aggiungere le autorizzazioni per le entità di database
```powershell
...
Il comando precedente aggiunge le autorizzazioni Entità database
...
### -Value
Elenco delle entità database Kusto.
Per creare, vedere la sezione NOTE relativa alle proprietà VALUE e creare una tabella hash.
...
VALUE <IDatabasePrincipal[]>: elenco delle entità database Kusto.
  - `Name <String>`: nome dell'entità database.
  - `Role <DatabasePrincipalRole>`: ruolo principale del database.
  - `Type <DatabasePrincipalType>`: tipo di entità database.
  - `[AppId <String>]`: ID applicazione- pertinente solo per il tipo di entità applicazione.
  - `[Email <String>]`: messaggio di posta elettronica dell'entità database, se esistente.
  - `[Fqn <String>]`: nome completo dell'entità database.
