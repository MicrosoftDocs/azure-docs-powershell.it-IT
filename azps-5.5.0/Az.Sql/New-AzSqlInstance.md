...
## DESCRIZIONE
Il cmdlet **New-AzSqlInstance** crea un'istanza gestita SQL database di Azure.
...
Questo comando crea una nuova istanza in un pool di istanze usando l'identificatore di risorsa del pool di istanze.
...
Questo comando crea una nuova istanza in un pool di istanze con il nome instancePool0

## PARAMETERS

### -AdministratorCredential
L SQL credenziali di autenticazione dell'istanza.
...
### -AssignIdentity
Generare e assegnare un'identità di Azure Active Directory per questa istanza gestita per l'uso con servizi di gestione delle chiavi come Azure KeyVault.
...
Ridondanza di archiviazione di backup usata per archiviare i backup per l'istanza gestita di Sql Azure. Le opzioni disponibili sono: Locale, Area e Geo
...
### -Collation
La fascicolazione dell'istanza SQL Azure da usare.
...
### -DnsZonePartner
ID risorsa del server gestito del partner da cui ereditare la proprietà DnsZone per la creazione dell'istanza gestita
...
### -InstancePoolName
Pool di istanze in cui inserire questa istanza.
...
- LicenseIncluded - Il prezzo di sconto AHB (Azure Hybrid Benefit) per i proprietari SQL Server licenze non viene applicato. Il prezzo del servizio Istanza gestita includerà un nuovo costo SQL Server licenze.
...
Se l'endpoint dati pubblico è abilitato o meno per l'istanza.
...
### -SKUName
Nome SKU per l'istanza, ad esempio "GP_Gen4", "BC_Gen4".
...
### -SubnetId
ID subnet da usare per la creazione dell'istanza
...
### -Tag
I tag da associare all'istanza
...
### -TimezoneId
ID del fuso orario per l'istanza da impostare. Un elenco di ID di fuso orario viene esposto tramite la visualizzazione sys.time_zone_info (Transact-SQL).
