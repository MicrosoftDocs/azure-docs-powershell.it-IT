...
## DESCRIZIONE
Il cmdlet **Get-AzSqlDatabaseLongTermRetentionBackup** ottiene tutti i backup di conservazione a lungo termine per una posizione, un server o un database oppure un backup di conservazione a lungo termine specifico.
...
Questo comando ottiene tutti i backup di conservazione a lungo termine per tutti i database (che potrebbero essere attivi o eliminati) nella regione nordeuropea, il gruppo di risorse verrà impostato solo se il server è attivo.
...
Questo comando recupera tutti i backup di conservazione a lungo termine per tutti i database (che potrebbero essere attivi o eliminati) in un gruppo di risorse nella regione nordeuropea.
...
Questo comando recupera il backup con nome 601061b7-d10b-46e0-bf77-a2bfb16a6add;1316556665500000000
...
Questo comando recupera tutti i backup di conservazione a lungo termine per database01

### Esempio 5: Ottenere backup di conservazione a lungo termine con i filtri
```powershell
...
Questo comando recupera tutte le copie di backup il cui nome inizia con "601061b7"
...
### -DatabaseState
Lo stato del database di cui si vogliono trovare le copie di backup, Alive, Deleted o All.
...
Se si desidera o meno ottenere solo il backup più recente per database.
...
### -ServerName
Il nome dell'account Azure SQL Server di backup.
