...
## SYNOPSIS
Esporta un database SQL Azure come file bacpac in un account di archiviazione.
...
## DESCRIZIONE
Il cmdlet **New-AzSqlDatabaseExport** esporta un database di azure SQL come file con estensione bacpac in un account di archiviazione.
La richiesta di stato di recupera database di esportazione può essere inviata per recuperare informazioni sullo stato per questa richiesta.
Questo cmdlet è supportato anche dal servizio SQL Server di estensione del database in Azure.

> [!IMPORTANT]
> Per usare questo cmdlet, il firewall nel SQL Server azure dovrà essere configurato su "Consenti ai servizi e alle risorse di Azure di accedere al server". Se questa configurazione non è configurata, verranno riscontrati errori di GatewayTimeout.
