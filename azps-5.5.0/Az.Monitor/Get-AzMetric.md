...
## DESCRIZIONE
Il cmdlet **Get-AzMetric** ottiene i valori della metrica per una risorsa specificata.
...
Questo comando ottiene i valori della metrica per sito Web3 con granularità di 1 minuto.
...
### Esempio 3: Ottenere un output dettagliato per una metrica specificata
```
...
Questo comando ottiene un output dettagliato per la metrica Richieste.

### Esempio 4: Ottenere l'output riepilogato per una metrica specificata con il filtro delle dimensioni specificato
```
...
Questo comando ottiene l'output riepilogato per la metrica PageViews con il filtro delle dimensioni e il tipo di aggregazione specificati.

## PARAMETERS

### -AggregationType
Il tipo di aggregazione della query
...
### -DetailedOutput
Indica che questo cmdlet visualizza un output dettagliato.
Per impostazione predefinita, l'output è riepilogato.
...
### -MetricFilter
Specifica il filtro delle dimensioni metriche per cui eseguire query sulle metriche.
...
### -MetricMetricMetric
Specifica lo spazio dei nomi metrico per cui eseguire query sulle metriche.
...
### -OrderBy
Specifica l'aggregazione da usare per l'ordinamento dei risultati e la direzione dell'ordinamento (esempio: somma asc).
...
### -ResultType
Specifica il tipo di risultato da restituire (metadati o dati).
...
### -TimeGrain
Specifica la granularità temporale della metrica come oggetto **TimeSpan** nel formato hh:mm:ss.
...
### -In alto
Specifica il numero massimo di record da recuperare (impostazione predefinita:10), da specificare con $filter.
...
Altre informazioni sulle metriche supportate sono disponibili all'indirizzo: https://docs.microsoft.com/en-us/azure/azure-monitor/platform/metrics-supported
