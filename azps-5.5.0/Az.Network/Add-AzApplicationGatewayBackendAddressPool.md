...
È possibile specificare un indirizzo back-end usando un indirizzo IP, un nome di dominio completo (FQDN) o ID di configurazione IP.
...
### Esempio 2: Aggiungere un pool di indirizzi back-end usando indirizzi IP del server back-end
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
### -BackendFqdns
Specifica un elenco di FQDN back-end aggiunti da questo cmdlet come pool di server back-end.
...
### -BackendIPAddresses
Specifica un elenco di indirizzi IP back-end che il cmdlet aggiunge come pool di server back-end.
