...
>[!NOTE] 
>Le configurazioni IP devono essere dinamiche prima di poter cambiare la subnet. In caso di configurazioni IP statiche, impostare la configurazione dinamica prima di procedere. 
>[!NOTE]
>Se l'interfaccia di rete ha più configurazioni IP, il quarto comando deve essere eseguito per tutte queste configurazioni IP prima di eseguire il comando Set-AzNetworkInterface di rete finale. Questa operazione può essere eseguita come specificato nel comando, ma sostituendo "0" con il numero appropriato. Se un'interfaccia di rete ha configurazioni N IP, è necessario che esistano N-1 di questi comandi.
