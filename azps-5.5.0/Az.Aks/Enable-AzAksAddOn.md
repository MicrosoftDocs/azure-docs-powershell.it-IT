...
## SYNOPSIS
Abilita i componenti aggiuntivi per gli aks.
...
Abilita i componenti `HttpApplicationRouting` `Monitoring` `AzurePolicy` aggiuntivi, , e `VirtualNode` per gli `KubeDashboard` aks.
...
### -WorkspaceResourceId
ID risorsa dell'area di lavoro di monitoraggio.
