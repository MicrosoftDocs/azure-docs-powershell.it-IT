...
## DESCRIZIONE
Il Get-AzBatchPoolNodeCount cmdlet consente ai clienti di ottenere il numero di nodi per ogni stato del nodo raggruppato per pool. Gli stati del nodo possibili sono: creazione, inattività, uscita dal pool, offline, preattivato, riavvio, riutilizzo, in esecuzione, avvio, avvioAttivitàFailed, sconosciuto, inutilizzabile e in attesaForStartTask. Il cmdlet accetta il parametro PoolId o Pool per filtrare solo il pool con l'ID pool specificato. 
