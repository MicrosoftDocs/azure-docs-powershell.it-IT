...
## DESCRIZIONE
Il **cmdlet Get-AzBatchTask** ottiene le attività batch di Azure per un processo batch.
Specificare un processo usando il *parametro JobId* o *Job.*
Per ottenere una singola attività, specificare il *parametro ID.*
È possibile specificare il *parametro Filter* per ottenere le attività che corrispondono a un filtro OData (Open Data Protocol).

## ESEMPI

### Esempio 1: Ottenere un'attività in base all'ID
```
...
Questo comando recupera l'attività con ID Attività03 nel processo Job01.
Usare il cmdlet Get-AzBatchAccountKey per assegnare un contesto alla $Context variabile.

### Esempio 2: Recuperare tutte le attività completate da un processo specificato
```
...
Questo comando recupera le attività completate dal processo con ID Processo02.
...
### -Expand
Specifica una clausola di espansione OData.
Specificare un valore per questo parametro per ottenere le entità associate dell'entità principale da ottenere.
...
### -Filter
Specifica una clausola del filtro OData per le attività.
Se non si specifica un filtro, questo cmdlet restituisce tutte le attività per l'account o il processo Batch.
...
### -Id
Specifica l'ID dell'attività che riceve questo cmdlet.
...
### -JobId
Specifica l'ID del processo che contiene le attività recuperate da questo cmdlet.
...
### -MaxCount
Specifica il numero massimo di attività da restituire.
