...
## DESCRIZIONE
Il cmdlet **Get-AzBatchAccount** ottiene un account batch di Azure nella sottoscrizione corrente. È possibile usare il parametro *AccountName* per ottenere un singolo account oppure il parametro *ResourceGroupName* per ottenere gli account in tale gruppo di risorse.
...
Questo comando recupera l'account batch denominato pfuller.
...
Questo comando recupera gli account batch associati al gruppo di risorse CmdletExampleRG.

## PARAMETERS

### -AccountName
Specifica il nome di un account.
Se si specifica un nome di account, questo cmdlet restituisce solo tale account.
