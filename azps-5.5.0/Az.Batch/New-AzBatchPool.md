...
## DESCRIZIONE
Il cmdlet **New-AzBatchPool** crea un pool nel servizio batch di Azure nell'account specificato dal *parametro BatchContext.*
...
Il pool è configurato per l'uso STANDARD_D1_V2 macchine virtuali con la versione del sistema operativo della famiglia quattro.
...
Questo comando crea un nuovo pool con ID MyPool usando il set di parametri TargetDedicated.
L'allocazione di destinazione è tre nodi di calcolo.
Il pool è configurato per l'uso STANDARD_D1_V2 macchine virtuali con l'immagine del sistema operativo Windows-2016-Datacenter.

### Esempio 3: Creare un nuovo pool usando il set di parametri di Gradazione automatica
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
Questo comando crea un nuovo pool con ID AutoScalePool usando il set di parametri AutoScale.
Il pool è configurato per l'uso di macchine virtuali STANDARD_D1_V2 con l'immagine del sistema operativo Windows-2016-Datacenter e il numero di nodi di elaborazione di destinazione è determinato dalla formula di gradazione automatica.
...
### -ApplicationLicenses
L'elenco di licenze di applicazioni che il servizio batch rende disponibile in ogni nodo di calcolo nel pool.
...
Specifica la quantità di tempo, in minuti, che deve trascorrere prima che le dimensioni del pool vengono regolate automaticamente in base alla formula Di scala automatica.
Il valore predefinito è 15 minuti e il valore minimo è 5 minuti.
...
### -AutoScaleFormula
Specifica la formula per ridimensionare automaticamente il pool.
...
Il servizio batch installa i certificati a cui si fa riferimento in ogni nodo di elaborazione del pool.
...
Specifica le impostazioni di configurazione per un pool in base alla piattaforma del servizio cloud Azure.
...
Specifica il numero massimo di attività che è possibile eseguire in un singolo nodo di calcolo.
...
Il valore è il valore dei metadati.
...
### -MountConfiguration
Elenco di file system da installare in ogni nodo nel pool. Supporta File di Azure, NFS, CIFS/SMB e BLOBfuse.
...
### -ResizeTimeout
Specifica il timeout per l'allocazione dei nodi di calcolo al pool.
...
### -StartTask
Specifica la specifica dell'attività di avvio per il pool.
L'attività di avvio viene eseguita quando un nodo di calcolo si unisce al pool o quando il nodo di calcolo viene riavviato o ricreato.
...
Specifica il numero di destinazione di nodi di elaborazione dedicati da allocare al pool.
...
Specifica il numero di destinazione di nodi di elaborazione con priorità bassa da allocare al pool.
...
Specifica i criteri di programmazione delle attività, ad esempio ComputeNodeFillType.
...
Per altre informazioni sulle dimensioni delle macchine virtuali, vedere Dimensioni per le macchine virtuali https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/ https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/) (nel sito di Microsoft Azure).
