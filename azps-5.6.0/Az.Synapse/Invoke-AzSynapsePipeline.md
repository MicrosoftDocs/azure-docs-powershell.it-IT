...
## DESCRIZIONE
Il **comando Invoke-AzSynapsePipeline** avvia un'esecuzione nella pipeline specificata e restituisce un ID per l'esecuzione. Il GUID può essere passato a **Get-AzSynapsePipelineRun** o **Get-AzSynapseActivityRun** per ottenere ulteriori dettagli sull'esecuzione.
...
### -InputObject
Informazioni sulla pipeline di esecuzione.
...
### -IsRecovery
Contrassegno modalità di ripristino.
Se la modalità di ripristino è impostata su true, la pipeline di riferimento specificata viene eseguita e la nuova esecuzione viene raggruppata sotto lo stesso groupId.
...
ID di esecuzione della pipeline per la riesegui.
Se si imposta l'ID di esecuzione, i parametri dell'esecuzione specificata verranno usati per creare una nuova esecuzione.
...
### -StartActivityName
In modalità di ripristino, la riesegui verrà avviata da questa attività.
Se non viene specificato, verranno eseguite tutte le attività.
