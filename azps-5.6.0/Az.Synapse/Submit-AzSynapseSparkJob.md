...
### RunSparkJobParameterSetName (impostazione predefinita)
```
...
## DESCRIZIONE
Il cmdlet **Submit-AzSynapseSparkJob** invia un processo Synapse Analytics Spark.
...
Questo comando invia un processo di Synapse Analytics Spark .NET.
...
Questo comando invia un processo di Synapse Analytics PySpark.
...
### -ExecutorCount
Numero di esecutori da allocare nel pool spark specificato per il processo.
...
### -ExecutorSize
Numero di core e memoria da usare per gli esecutori allocati nel pool Spark specificato per il processo.
...
### -MainClassName
L'identificatore completo o la classe principale nel file di definizione principale.
Obbligatorio per il processo Spark e .NET Spark.
ad esempio "org.apache.spark.examples.SparkPi"
...
### -MainDefinitionFile
Il file principale usato per il processo.
ad esempio " abfss://filesystem@account.dfs.core.windows.net/mySpark.jar "
...
### -ReferenceFile
File aggiuntivi usati per riferimento nel file di definizione principale. Elenco di URI di archiviazione con valori delimitati da virgole. ad esempio " abfss://filesystem@account.dfs.core.windows.net/file1.txt , abfss://filesystem@account.dfs.core.windows.net/result/ "
