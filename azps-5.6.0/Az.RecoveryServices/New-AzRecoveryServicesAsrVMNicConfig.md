...
## SYNOPSIS
Crea una configurazione NIC ASR contenente i dettagli della configurazione correlata al failover e al failover del test.
...
## DESCRIZIONE
Il cmdlet **New-AzRecoveryServicesAsrVMNicConfig** crea un oggetto configurazione NIC ASR che contiene i dettagli correlati al failover e al failover del test. Nel caso in cui qualsiasi informazione non venga passata, i valori corrispondenti vengono selezionati dall'elemento protetto da replica per evitare che questi valori vengano aggiornati a Null.
...
Crea un oggetto ASRVmNicConfig con le impostazioni di rete di failover e test configurate per nic. Qualsiasi proprietà non passata sopra viene recuperata dall'elemento protetto passato.
...
Crea un oggetto ASRVmNicConfig con le impostazioni di rete del faiover del test configurate per la ridenominazione NIC. Qualsiasi proprietà non passata sopra viene recuperata dall'elemento protetto passato.


### Esempio 3

Crea una configurazione NIC ASR che contiene i dettagli della configurazione correlata al failover e al failover del test. (autogenerati)
...
Specifica se la rete accelerata è abilitata al NIC di ripristino.
...
Specifica se la rete accelerata è abilitata al nic di failover di test.
...
### -NicId
Specificare il GUID di NIC asr.
...
Specifica gli ID dei pool di indirizzi back-end per il nic di ripristino.
...
Specifica l'ID del gruppo di rete associato a NIC di ripristino.
...
### -RecoveryNicName
Specifica il nome del nic di ripristino.
...
Specifica l'indirizzo IP del nic di ripristino.
...
Specifica l'ID dell'indirizzo IP pubblico associato a NIC di ripristino.
...
Specificare l'elemento protetto da replica ASR.
...
### -ReuseExistingNic
Specifica se una NIC esistente può essere usata durante il failover.
...
Specifica l'ID del gruppo di rete associato a NIC di failover del test.
...
Specifica il nome del gruppo di risorse NIC di failover del test.
...
Specifica l'ID dell'indirizzo IP pubblico associato a NIC di failover del test.
...
### -TfoReuseExistingNic
Specifica se una NIC esistente può essere usata durante il failover del test.
...
### -TfoVMNetworkId
Specifica l'ID della rete virtuale di failover dei test.
...
### -TfoVMSubnetName
Specifica il nome della subnet di failover del test.
