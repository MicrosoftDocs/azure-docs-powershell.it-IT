...
## DESCRIZIONE
Il cmdlet **New-AzRecoveryServicesAsrvCenter** aggiunge un server vCenter da cui individuare gli elementi proteggibili. Questo cmdlet registra il server vCenter per l'individuazione con il server di configurazione.
...
Aggiunge un server vCenter da cui individuare gli elementi proteggibili. (autogenerati)
...
### -Account
Account delle credenziali di accesso utente.
...
### -Name
Nome descrittivo del server vCenter.
...
### -Port
Porta TCP nel server vCenter da usare per l'individuazione.
