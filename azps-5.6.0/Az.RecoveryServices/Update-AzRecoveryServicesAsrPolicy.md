...
## DESCRIZIONE
Il cmdlet **Update-AzRecoveryServicesAsrPolicy** aggiorna i criteri di replica di Azure Site Recovery specificati.
...
Avvia l'operazione dei criteri di replica degli aggiornamenti usando i parametri specificati e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
Avvia l'operazione di aggiornamento da Azure a Criteri di replica di Azure usando i parametri specificati e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
Avvia l'aggiornamento dei criteri di replica di Azure ad Azure usando i parametri specificati e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
### -Autenticazione
Specifica il tipo di autenticazione usata.
...
### -AzureToVMware
Specifica il ripristino di emergenza da Azure a vMWare.
...
### -HyperVToAzure
Parametro di parametro che indica che il criterio specificato viene usato per replicare le macchine virtuali di Hyper-V in Azure.
...
### -InputObject
Oggetto di input per il cmdlet: specifica l'oggetto criterio di replica ASR corrispondente ai criteri di replica da aggiornare.
...
Specifica l'ID account di archiviazione di Azure della destinazione della replica. Usato come account di archiviazione di destinazione per la replica se non viene fornita un'alternativa durante l'abilitazione della replica con il cmdlet New-AzRecoveryServicesASRReplicationProtectedItem destinazione.
...
Tempo in ore per conservare i punti di ripristino dopo la creazione.
...
Specifica l'intervallo della frequenza di replica in secondi.
...
Non deve essere oltre le 24 ore dall'inizio del processo.
...
### -PiùVmm
Parametro di parametro che indica che il criterio specificato viene usato per replicare le macchine virtuali Di Hyper-V gestite IN ALTRO MODO tra due siti di Hyper-V.
...
### -VMwareToAzure
Parametro di parametro che indica che il criterio specificato viene usato per replicare le macchine virtuali VMware in Azure.
