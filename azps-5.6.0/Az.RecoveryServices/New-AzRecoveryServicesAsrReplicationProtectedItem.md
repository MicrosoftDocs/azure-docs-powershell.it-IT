...
Usare questo cmdlet per abilitare la replica per un elemento protetto da AsR.
...
Avvia l'operazione di creazione di elementi protetti da replica per l'elemento protetto da ASR specificato e restituisce il processo ASR usato per tenere traccia dell'operazione.
...
Avvia l'operazione di creazione di elementi protetti da replica per l'elemento protetto da ASR specificato e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario vmWare in Azure).
...
Avvia l'operazione di creazione di elementi protetti da replica per l'elemento protetto da ASR specificato e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario da Azure ad Azure).
...
Avvia l'operazione di creazione degli elementi protetti da replica per il valore VmId specificato e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario da Azure ad Azure).
...
Avvia l'operazione di creazione di elementi protetti da replica per l'elemento protetto da ASR specificato, inclusi i dischi selettivi, e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario da vmWare a Azure) con i dischi selezionati.
...
Avvia l'operazione di creazione degli elementi protetti da replica per il valore VmId specificato e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario da Azure ad Azure). Per il cmdlet di crittografia passato per la macchina virtuale di failover verranno usati i dettagli.
...
Avvia l'operazione di creazione di elementi protetti da replica per una macchina virtuale all'interno del gruppo di posizionamento di prossimità e restituisce il processo ASR usato per tenere traccia dell'operazione (scenario Azure su Azure).

## PARAMETERS

### -Account
L'account Esegui come da usare per eseguire l'installazione push del servizio per dispositivi mobili, se necessario. Deve essere nell'elenco di account run as nel tessuto ASR.
...
### -AzureToAzure
Parametro switch specifica la creazione dell'elemento replicato in uno scenario azure.
...
### -AzureVmId
Specifica l'ID della macchina virtuale di Azure per la protezione da ripristino di emergenza nell'area geografica di ripristino.
...
Specifica l'URL segreto di crittografia del disco con la versione (crittografia del disco di Azure) da usare come macchina virtuale di ripristino dopo il failover.
...
Specifica l'ID vault segreto di crittografia del disco (crittografia disco di Azure) da usare per la macchina virtuale di ripristino dopo il failover.
...
### -DiskType
Specifica il tipo di disco gestito dalla macchina virtuale di ripristino.
...
### -HyperVToAzure
Il parametro switch per specificare che l'elemento replicato è una macchina virtuale di Hyper-V che viene replicata in Azure.
...
### -IncludeDiskId
L'elenco dei dischi da includere per la replica. Per impostazione predefinita, sono inclusi tutti i dischi.
...
Specifica l'input di configurazione del disco per l'ID disco vMWare da proteggere dall'elemento protetto specificato.
...
### -KeyEncryptionKeyUrl
Specifica l'URL della chiave di crittografia del disco con la versione (crittografia disco di Azure) da usare come macchina virtuale di ripristino dopo il failover.
...
Specifica l'ID chiave-vault della chiave di crittografia del disco (crittografia disco di Azure) da usare per la macchina virtuale di ripristino dopo il failover.
...
### -LogStorageAccountId
Specifica l'ID dell'account di archiviazione cache o del log da usare per archiviare i log di replica.
...
### -ProcessServer
Il server dei processi da usare per replicare il computer. Usare l'elenco dei server di processo nell'infrastruttura ASR corrispondente al server di configurazione per specificarne uno.
...
### -ProtectableItem
Specifica l'oggetto elemento protetto da AsR per cui viene abilitata la replica.
...
Specifica l'oggetto mapping del contenitore di protezione ASR corrispondente al criterio di replica da usare per la replica.
...
ID del set di disponibilità in cui recuperare il computer in caso di failover.
...
Specifica l'area di disponibilità della macchina virtuale di ripristino dopo il failover.
...
ID della rete virtuale di Azure in cui recuperare il computer in caso di failover.
...
La subnet all'interno della rete virtuale di Azure di ripristino a cui deve essere collegata la macchina virtuale con errore in caso di failover.
...
Specifica l'account di archiviazione per la diagnostica di avvio per azure VM di ripristino.
...
Specifica l'ID risorsa del servizio cloud di ripristino su cui eseguire il failover della macchina virtuale.
...
Specificare l'ID gruppo di posizionamento per prossimità usato dalla macchina virtuale di failover nell'area di ripristino di destinazione.
...
Specifica l ARM identificatore predefinito del gruppo di risorse in cui la macchina virtuale verrà creata in caso di failover.
...
Specifica il nome del gruppo di replica da usare per creare punti di ripristino coerenti con più VM. Per impostazione predefinita, ogni elemento protetto da replica viene creato in un gruppo e non vengono generati punti di ripristino coerenti con più VM. Usare questa opzione solo se è necessario creare punti di ripristino coerenti con più vm in un gruppo di computer proteggendo tutti i computer nello stesso gruppo di replica. 
...
### -UseManagedDisk
Specifica se la macchina virtuale di Azure creata al failover deve usare dischi gestiti. Accetta vero o falso.
...
### -PiùVmm
Il parametro switch per specificare che l'elemento replicato è una macchina virtuale di Hyper-V che viene replicata tra siti DI Hyper-V gestiti DA DUET.
...
### -VMwareToAzure
Il parametro switch per specificare che l'elemento replicato è una macchina virtuale VMware o un server fisico che verrà replicato in Azure.
...
### -WaitForCompletion
Specifica che il cmdlet deve attendere il completamento dell'operazione prima di restituire.
...
### -Confirm
Chiede conferma prima di avviare l'operazione.
