...
## DESCRIZIONE
Il cmdlet Update-AzContainerRegistryCredential rigenera le credenziali di accesso per un Registro di sistema del contenitore.

## ESEMPI

### Esempio 1: Rigenerare le credenziali di accesso per un Registro di sistema del contenitore
```powershell
...
Questo comando rigenera le credenziali di accesso per il Registro di sistema del contenitore specificato.
L'utente amministratore deve essere abilitato per il registro del contenitore \` MyRegistry \` per rigenerare le credenziali di accesso.
...
### -PasswordName
Nome della password da rigenerare.
Valori consentiti: password, password2.
