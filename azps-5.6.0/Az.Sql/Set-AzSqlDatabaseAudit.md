...
## DESCRIZIONE
Il cmdlet **Set-AzSqlDatabaseAudit** modifica le impostazioni di controllo di un database SQL Azure.
...
### Esempio 1: Abilitare i criteri di controllo di archiviazione BLOB di un database SQL Azure
```powershell
...
### Esempio 2: Disabilitare i criteri di controllo di archiviazione BLOB di un database SQL Azure
```powershell
...
### Esempio 3: Abilitare i criteri di controllo di archiviazione BLOB di un database di SQL azure con i filtri usando un predicato di SQL BLOB
```powershell
...
### Esempio 4: Rimuovere il filtro dai criteri di controllo di un database SQL Azure
```powershell
...
### Esempio 5: Abilitare i criteri di controllo dell'hub eventi di un database SQL Azure
```powershell
...
### Esempio 6: Disabilitare i criteri di controllo dell'hub eventi di un database SQL Azure
```powershell
...
### Esempio 7: Abilitare i criteri di controllo di analisi dei log di un database SQL Azure
```powershell
...
### Esempio 8: Disabilitare i criteri di controllo di analisi dei log di un database SQL Azure
```powershell
...
### Esempio 9: Disabilitare, tramite pipeline, i criteri di controllo di analisi dei log di un database SQL Azure
```powershell
...
### Esempio 10: Disabilitare l'invio dei record di controllo di un database di azure SQL all'archiviazione BLOB e abilitarli all'analisi del log.
```powershell
...
### Esempio 11: Abilitare l'invio dei record di controllo di un database SQL Azure all'archiviazione BLOB, all'hub eventi e all'analisi del log.
```powershell
...
### -AuditAction
Insieme di azioni di controllo.  
...
RECEIVE  
REFERENCES  
La maschera generale per definire un'azione da controllare è: [azione] ON [oggetto] BY [entità] Si noti che [oggetto] nel formato precedente può fare riferimento a un oggetto come una tabella, una visualizzazione o una stored procedure oppure a un intero database o schema. Per gli ultimi casi, vengono usate, rispettivamente, le maschere DATABASE::[nome db] e SCHEMA::[nome schema].
Per esempio:  
SELECT su dbo.myTable per pubblico  
SELECT su DATABASE::myDatabase per pubblico  
SELECT su SCHEMA::mySchema per pubblico  
Per altre informazioni, vedere https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions .
...
### -AuditActionGroup
La combinazione consigliata di gruppi di azioni da usare è la seguente: in questo modo verranno controllati tutte le query e le stored procedure eseguite nel database, nonché gli accessi riusciti e non riusciti:  
  
"BATCH_COMPLETED_GROUP",  
"SUCCESSFUL_DATABASE_AUTHENTICATION_GROUP",  
"FAILED_DATABASE_AUTHENTICATION_GROUP"  
Anche questa combinazione è configurata per impostazione predefinita. Questi gruppi coprono tutte SQL istruzione e le stored procedure eseguite sul database e non devono essere usati insieme ad altri gruppi perché ciò genererà log di controllo duplicati.
Per altre informazioni, vedere https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-action-groups .
...
### -DatabaseObject
Oggetto di database per la gestione dei criteri di controllo.
...
Indica se l'analisi del log è una destinazione per i record di controllo.
...
### -PassThru
Specifica se l'output del criterio di controllo deve essere generato al termine dell'esecuzione del cmdlet
...
### -PredicateExpression
Predicato di SQL T (clausola WHERE) usato per filtrare i log di controllo.
...
### -StorageKeyType
Specifica quale delle chiavi di accesso di archiviazione usare.
...
### -WorkspaceResourceId
ID area di lavoro (ID risorsa di un'area di lavoro Analisi log) per un'area di lavoro Analisi log a cui inviare i log di controllo. Esempio: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/rba2
