...
## DESCRIZIONE
Il cmdlet **Enable-AzSqlServerAdvancedDataSecurity** abilita Advanced Data Security in un server. Advanced Data Security è un pacchetto di sicurezza unificato che include la classificazione dei dati, la valutazione delle vulnerabilità e Advanced Threat Protection per il server. Verrà creato automaticamente un nuovo account di archiviazione per il salvataggio delle valutazioni di vulnerabilità. Se un account di archiviazione è stato creato in precedenza per questo scopo, verrà usato al suo posto)

## ESEMPI

### Esempio 1: Abilitare Advanced Data Security del server
```powershell
...
### Esempio 2: Abilitare Advanced Data Security del server dalla risorsa server
```powershell
...
Abilita Advanced Data Security in un server. (autogenerati)
...
### -InputObject
Oggetto server da usare con l'operazione dei criteri di sicurezza dei dati avanzata
