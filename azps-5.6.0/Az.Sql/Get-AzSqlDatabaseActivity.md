...
## DESCRIZIONE
Il cmdlet **Get-AzSqlDatabaseActivity** ottiene lo stato delle operazioni di database in Azure SQL Database.

## ESEMPI

### Esempio 1: Ottenere lo stato per tutte le istanze SQL database
```
PS C:\>Get-AzSqlDatabaseActivity -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -ElasticPoolName "ElasticPool01"
```

Questo comando restituisce lo stato dell'operazione di tutte SQL di database in un pool elastico denominato ElasticPool01.

### Esempio 2: Ottenere lo stato per tutte le SQL di un database
```
PS C:\>Get-AzSqlDatabaseActivity -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
```

Questo comando restituisce lo stato di tutte le SQL di un database.

## PARAMETERS

### -DatabaseName
Specifica il nome del database per cui il cmdlet ottiene lo stato.
...
### -ElasticPoolName
Specifica il nome del pool di database elastico per cui il cmdlet ottiene lo stato.
...
### -OperationId
Specifica l'ID dell'operazione che riceve questo cmdlet.
...
### -ServerName
Specifica il nome della cartella di Microsoft SQL Server che ospita il database.
