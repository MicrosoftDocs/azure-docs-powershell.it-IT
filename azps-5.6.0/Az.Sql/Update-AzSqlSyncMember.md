...
## DESCRIZIONE
Il cmdlet **Update-AzSqlSyncGroup** modifica le proprietà di un membro di sincronizzazione del database SQL Azure.
...
Questo comando reimposta la password dell'amministratore per il database dei membri.
...
Credenziali (nome utente e password) del database di SQL Azure.
...
ID della risorsa per il database dei membri di sincronizzazione, usato se UsePrivateLinkConnection è impostato su true.
...
Se usare un collegamento privato quando ci si connette a questo membro di sincronizzazione.
