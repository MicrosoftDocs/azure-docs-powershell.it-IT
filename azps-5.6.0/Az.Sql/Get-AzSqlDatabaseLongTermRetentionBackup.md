...
Questo comando ottiene tutti i backup di conservazione a lungo termine per tutti i database (che potrebbero essere attivi o eliminati) nel nordeuropeo, il gruppo di risorse verrà impostato solo se il server è attivo.

### Esempio 2: Ottenere tutte le copie di backup per una posizione in un gruppo di risorse
```powershell
