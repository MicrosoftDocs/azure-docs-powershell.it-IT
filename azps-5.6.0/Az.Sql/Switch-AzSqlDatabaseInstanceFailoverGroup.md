...
## DESCRIZIONE
Questo comando scambia i ruoli delle istanze gestite in un gruppo di failover di istanza effettuando il failover sull'area secondaria specificata, rendendola la nuova area principale. Tutte le nuove sessioni TDS che si connettono all'endpoint principale vengono automaticamente ri instradati alla nuova area primaria. 
...
Eseguire un'operazione di failover che consente la perdita di dati tramite piping nel gruppo di failover dell'istanza.
...
### -AllowDataLoss
Completare il failover anche in questo caso può causare la perdita di dati.
In questo modo il failover può proseguire anche se un database primario non è disponibile.
...
### -InputObject
L'oggetto gruppo di failover dell'istanza da cambiare
...
### -ResourceId
ID risorsa del gruppo di failover dell'istanza da cambiare.
