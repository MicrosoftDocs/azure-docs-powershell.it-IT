...
### MigrateSqlServerSqlDb (impostazione predefinita)
```
...
## DESCRIZIONE
Il cmdlet New-AzDataMigrationSelectedDB crea un oggetto informazioni di database contenente informazioni sui database di origine e di destinazione, nonché i mapping di tabella, per la migrazione. Questo cmdlet può essere usato come parametro con il cmdlet New-AzDataMigrationTask cmdlet.
...
### -BackupFileShare
Condivisione file in cui eseguire il backup dei file di database del server di origine per il database.
Usare questa impostazione per ignorare le informazioni sulla condivisione file per ogni database.
Usare il nome di dominio completo per il server.
...
Impostare il database su readonly prima della migrazione
...
Impostare il tipo di migrazione SQL Server migrazione SQL a DB.
...
### -MigrateSqlServerSqlDbMi
Impostare il tipo di migrazione SQL Server per SQL migrazione MI di DB.
...
### -TableMap
Mapping tra tabelle di origine e di destinazione
