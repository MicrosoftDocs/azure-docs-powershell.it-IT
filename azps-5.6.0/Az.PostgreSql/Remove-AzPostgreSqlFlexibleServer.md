...
### Esempio 1: Rimuovere il server PostgreSql in base a resourceGroup e al nome del server
```powershell
PS C:\> Remove-AzPostgreSqlFlexibleServer -ResourceGroupName PowershellPostgreSqlTest -Name postgresql-test

```

Questo cmdlet rimuove il server PostgreSql in base a resourceGroup e al nome del server.

### Esempio 2: Rimuovere il server PostgreSql in base all'identità
```powershell
...
Questi cmdlet rimuovono il server PostgreSql in base all'identità.
