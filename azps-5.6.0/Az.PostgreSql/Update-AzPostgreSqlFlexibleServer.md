...
Il corpo della richiesta può contenere da una a molte delle proprietà presenti nella normale definizione del server.
Usare Update-AzPostSqlFlexibleServerConfiguration invece se si vogliono aggiornare i parametri del server, ad esempio wait_timeout o net_retry_count.
...
### Esempio 1: Aggiornare il server PostgreSql in base al gruppo di risorse e al nome del server
```powershell
...
Questo cmdlet aggiorna il server PostgreSql in base al gruppo di risorse e al nome del server.

### Esempio 2: Aggiornare il server PostgreSql in base all'identità.
```powershell
...
Questo cmdlet aggiorna il server PostgreSql in base all'identità.
...
### -HaEnabled
Abilitare o disabilitare la funzionalità disponibilità elevata.
...
### -MaintenanceWindow
Periodo di tempo (UTC) designato per la manutenzione.
Esempi: "Dom:23:30" per programmare la domenica, 23:30 UTC.
Per tornare al pass predefinito in "Disabilitato"
...
### -SKU
Il nome della sKU, ad esempio Burstable_B1ms, Standard_D2ds_v4
...
### -SkuTier
Livello dello specifico SKU.
Valori accettati: Burstable, GeneralPurpose, Memory Optimized.
Impostazione predefinita: esplosa.
...
### -SslEnforcement
Abilitare o meno l'applicazione del ssl quando ci si connette al server.
...
### -StorageAutogrow
Abilitare l'estensione automatica dello spazio di archiviazione.
...
<IPostgreSqlIdentity>INPUTOBJECT: parametro identity.
