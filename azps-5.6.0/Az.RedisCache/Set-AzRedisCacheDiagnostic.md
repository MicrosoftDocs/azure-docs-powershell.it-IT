...
## DESCRIZIONE
Il cmdlet **Set-AzRedisCacheDiagnostic abilita** la diagnostica per una cache di Azure Redis.

## ESEMPI

### Esempio 1: Abilitare la diagnostica
```
...
Questo comando abilita la diagnostica per una cache di Ridis di Azure.
Questo comando abiliterà la diagnostica o aggiornerà l'account di archiviazione per tutte le cache di Azure Redis nella stessa area geografica per la sottoscrizione.
...
### -StorageAccountId
Specifica l'ID risorsa dell'account di archiviazione usato per archiviare i dati di diagnostica.
...
## NOTE
* Parole chiave: azure, azurerm, arm, risorsa, gestione, manager, ridisposizione, cache, Web, webapp, sito Web
