...
### ReimageExpanded (impostazione predefinita)
```
...
### Esempio 1: Istanze del ruolo Reimage del servizio cloud
```powershell
...
Questo comando reimmagine 2 istanze di ruoli ContosoFrontEnd_IN_0 e ContosoBackEnd_IN_1 di un servizio cloud denominato ContosoCS che appartiene al gruppo di risorse denominato ContosOrg.

### Esempio 2: Visualizzare nuovamente tutti i ruoli del servizio cloud
```powershell
PS C:\> Invoke-AzCloudServiceReimage -ResourceGroupName "ContosOrg" -CloudServiceName "ContosoCS" -RoleInstance "*"
```

Questo comando ricrea l'immagine di tutte le istanze di ruolo del servizio cloud denominate ContosoCS che appartengono al gruppo di risorse denominato ContosOrg.
