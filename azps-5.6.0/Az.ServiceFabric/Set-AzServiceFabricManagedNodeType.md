...
## DESCRIZIONE
Imposta le proprietà delle risorse del tipo di nodo o esegue azioni di reimage su specifici tipi di nodo con il parametro -Reimage. Durante l'operazione di ripristino, i nodi dell'infrastruttura del servizio verranno disabilitati prima di riprovare le macchine virtuali e ri abilitati nuovamente una volta tornati. Se questa operazione viene eseguita nei tipi di nodo primario, l'operazione potrebbe richiedere del tempo perché potrebbe non essere possibile ricreare l'immagine di tutti i nodi contemporaneamente. Usare -ForceReimage forza l'operazione anche se l'infrastruttura del servizio non è in grado di disabilitare i nodi, ma con cautela, perché ciò potrebbe causare la perdita di dati se nel nodo sono in esecuzione carichi di lavoro con stato.
...
Aggiornare le properite della posizione del tipo di nodo. In questo modo verranno sovrascritte le precedenti properite del posizionamento, se presenti.
...
Aggiornare il numero di istanze del tipo di nodo con piping.

## PARAMETERS

### -ApplicationEndPort
Porta finale dell'applicazione di un intervallo di porte.
...
### -Capacità
Tag di capacità applicati ai nodi nel tipo di nodo come coppie chiave/valore. Gestione risorse cluster usa questi tag per determinare la quantità di risorse di un nodo. L'aggiornamento eseguirà l'override dei valori correnti.
...
### -EphemeralEndPort
Porta finale effimera di una gamma di porte.
...
### -EphemeralStartPort
Porta iniziale effimera di un intervallo di porte.
...
### -ForceReimage
L'uso di questo flag forza la rimozione anche se non è possibile disabilitare i nodi.
Usare con cautela perché ciò potrebbe causare la perdita di dati se nel nodo sono in esecuzione carichi di lavoro con stato.
...
### -NodeName
Elenco dei nomi dei nodi per l'operazione.
...
### -PlacementProperty
Tag di posizionamento applicati ai nodi nel tipo di nodo come coppie chiave/valore, che possono essere usati per indicare dove eseguire determinati servizi (carico di lavoro). L'aggiornamento eseguirà l'override dei valori correnti.
...
### -Reimage
Specificare per visualizzare nuovamente i nodi dell'immagine nel tipo di nodo.
