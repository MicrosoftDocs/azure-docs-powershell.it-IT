...
Il comando archivia l'oggetto nella $PoolInformation variabile.
Il secondo comando assegna l'ID Pool22 alla proprietà **PoolId** dell'oggetto in $PoolInformation.
Il comando finale crea un processo il cui ID è ContosoJob35.
Le attività aggiunte al processo vengono eseguite nel pool con l'ID Pool22.
...
Specifica le variabili di ambiente comuni, come coppie chiave/valore, che questo cmdlet imposta per tutte le attività nel processo.
...
### -JobManagerTask
Specifica l'attività Gestione processi.
Il servizio batch esegue l'attività Gestione processi all'avvio del processo.
...
### -JobPreparationTask
Specifica l'attività Preparazione processo.
Il servizio batch esegue l'attività preparazione processo in un nodo di elaborazione prima di avviare le attività di quel processo in quel nodo di elaborazione.
...
### -JobReleaseTask
Specifica l'attività Job Release.
Il servizio batch esegue l'attività Job Release al termine del processo.
Il servizio batch esegue l'attività Job Release in ogni nodo di elaborazione in cui è stata eseguita qualsiasi attività del processo.
...
### -OnAllTasksComplete
Specifica un'azione eseguita dal servizio batch se tutte le attività nel processo sono in stato completato.
...
### -OnTaskFailure
Specifica un'azione eseguita dal servizio batch se un'attività del processo non riesce.
...
Il valore -1000 è la priorità più bassa.
Il valore 1000 è la priorità più alta.
