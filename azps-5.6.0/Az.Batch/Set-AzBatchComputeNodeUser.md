...
## DESCRIZIONE
Il cmdlet **Set-AzBatchComputeNodeUser** modifica le proprietà di un account utente in un nodo di elaborazione batch di Azure.

## ESEMPI

### Esempio 1: Aggiornare un account utente
```
...
Questo comando modifica l'account utente denominato PFuller nel nodo di elaborazione che ha l'ID specificato nel pool denominato ContosoPool.
Il comando modifica la password e la scadenza dell'account.
...
### -ComputeNodeId
Specifica l'ID del nodo di calcolo su cui viene eseguito il cmdlet.
...
### -ExpiryTime
Specifica la scadenza per l'account utente.
...
### -Name
Specifica il nome dell'account utente modificato dal cmdlet.
...
### -PoolId
Specifica l'ID del pool che contiene il nodo di calcolo in cui viene eseguito il cmdlet.
