...
## DESCRIZIONE
Il cmdlet **Get-AzBatchSupportedImage** ottiene le immagini delle macchine virtuali supportate disponibili in un account batch di Azure.
...
Il primo comando ottiene un contesto di account batch contenente i tasti di scelta per l'abbonamento usando **Get-AzBatchAccountKey.**
Il comando archivia il contesto nella $Context variabile da usare nel comando successivo.
Il secondo comando ottiene tutte le immagini supportate disponibili per l'account batch.
...
### -Filter
Specifica una clausola del filtro OData per le immagini supportate.
Se non si specifica un filtro, questo cmdlet restituisce tutte le immagini supportate dall'account batch.
