...
## DESCRIZIONE
Il cmdlet **New-AzBatchComputeNodeUser** crea un account utente in un nodo di elaborazione batch di Azure.

## ESEMPI

### Esempio 1: Creare un account utente con credenziali di amministratore
```
PS C:\>New-AzBatchComputeNodeUser -PoolId "MyPool01" -ComputeNodeId "ComputeNode01" -Name "TestUser" -Password "Password" -ExpiryTime ([DateTime]::Now.AddDays(7)) -IsAdmin -BatchContext $Context
```

Questo comando crea un account utente nel nodo di elaborazione con ID ComputeNode01.
Il nodo si trova nel pool con ID MyPool01.
Il nome utente è TestUser, la password è Password, l'account scade tra sette giorni e l'account ha credenziali di amministratore.

### Esempio 2: Creare un account utente in un nodo di calcolo usando la pipeline
```
PS C:\>Get-AzBatchComputeNode "MyPool01" -ComputeNodeId "ComputeNode01" -BatchContext $Context | New-AzBatchComputeNodeUser -Name "TestUser" -Password "Password" -BatchContext $Context
```

Questo comando recupera il nodo di elaborazione denominato ComputeNode01 usando il cmdlet **Get-AzBatchComputeNode.**
Il nodo si trova nel pool con ID MyPool01.
...
### -ComputeNode
Specifica il nodo di calcolo, come oggetto **PSComputeNode,** in cui questo cmdlet crea un account utente.
...
### -ComputeNodeId
Specifica l'ID del nodo di calcolo in cui questo cmdlet crea un account utente.
...
### -ExpiryTime
Specifica la scadenza del nuovo account utente.
...
### -IsAdmin
Indica che il cmdlet crea un account utente con credenziali di amministratore.
...
### -PoolId
Specifica l'ID del pool che contiene il nodo di calcolo in cui creare l'account utente.
