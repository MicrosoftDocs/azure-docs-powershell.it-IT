...
## DESCRIZIONE
Il cmdlet **Remove-AzBatchPool** elimina il pool batch di Azure specificato.
Viene richiesta una conferma, a meno che non si usi il *parametro Force.*

## ESEMPI

### Esempio 1: Eliminare un pool batch in base all'ID pool
```
PS C:\>Remove-AzBatchPool -Id "MyPool" -BatchContext $Context
```

Questo comando elimina il pool con ID MyPool.
All'utente viene chiesto di confermare l'esecuzione dell'operazione di eliminazione.

### Esempio 2: Eliminare tutti i pool di batch forzati
```
PS C:\>Get-AzBatchPool -BatchContext $Context | Remove-AzBatchPool -Force -BatchContext $Context
```

Questo comando elimina tutti i pool di batch.
Poiché il *parametro Force* è presente, la richiesta di conferma viene eliminata.
...
### -Id
Specifica l'ID del pool da eliminare.
