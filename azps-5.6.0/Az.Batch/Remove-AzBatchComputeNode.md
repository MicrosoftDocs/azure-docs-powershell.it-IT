...
## DESCRIZIONE
Il cmdlet **Remove-AzBatchComputeNode rimuove** i nodi di elaborazione batch di Azure da un pool.
...
Questo comando rimuove il nodo di calcolo con l'ID specificato dal pool che ha l'ID Pool07.
Il comando specifica l'opzione Disassegnazione.
...
Questo comando recupera il nodo di calcolo con l'ID specificato dal pool che contiene l'ID Pool07 usando il cmdlet Get-AzBatchComputeNode> .
Il comando passa il nodo al cmdlet corrente usando la pipeline.
Il cmdlet corrente rimuove il nodo di calcolo.
...
Questo comando rimuove due nodi di calcolo dal pool che contiene l'ID Pool07.
Il comando non richiede conferma.
...
### -ComputeNode
Specifica **l'oggetto PSComputeNode** che rappresenta il nodo di calcolo rimosso dal cmdlet.
...
### -DeallocationOption
Specifica un'opzione di deassegnazione per l'operazione di rimozione avviata da questo cmdlet.
...
### -ID
Specifica una matrice di ID dei nodi di calcolo rimossi dal cmdlet dal pool.
...
### -PoolId
Specifica l'ID del pool che contiene i nodi di calcolo rimossi dal cmdlet.
