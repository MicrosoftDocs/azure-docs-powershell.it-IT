...
### Esempio 1: Creare un archivio di configurazione delle app
```powershell
...
Questo comando crea un archivio di configurazione delle app.
...
Questo comando crea una configurazione dell'app e le assegna un'identità gestita assegnata all'utente.
Vedere l'esempio `Update-AzAppConfigurationStore` di per la procedura seguente per abilitare CMK (chiave gestita cusomer).
...
È possibile abilitare le identità gestite assegnate al sistema e allo stesso tempo assegnare le identità agli utenti.
Vedere l'esempio `Update-AzAppConfigurationStore` di per la procedura seguente per abilitare CMK (chiave gestita cusomer).
...
Non è possibile modificare questa impostazione dopo la creazione della risorsa.
