...
## SYNOPSIS
Interrompe un'app per le funzioni.

## SINTASSI

### StopByName (impostazione predefinita)
```
...
### Esempio 1: Ottenere un'app per le funzioni in base al nome e arrestarla.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Stop-AzFunctionApp -Force
```

Questo comando recupera un'app per funzione in base al nome e la interrompe.

### Esempio 2: Arrestare un'app per funzioni in base al nome.
```powershell
PS C:\> Stop-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Questo comando interrompe l'esecuzione di un'app per funzione in base al nome.
...
### -Force
Forza il cmdlet ad arrestare l'app funzione senza richiedere conferma.
...
INPUTOBJECT <ISite> : 
  ...
  - `[Enabled <Boolean?>]`: <code>true</code> se l'app è abilitata; in caso contrario. <code>false</code> Impostando questo valore su false, l'app viene disabilitata (l'app viene disconnessa).
  ...
  - `[IsXenon <Boolean?>]`: Obsoleta: sandbox Hyper-V.
