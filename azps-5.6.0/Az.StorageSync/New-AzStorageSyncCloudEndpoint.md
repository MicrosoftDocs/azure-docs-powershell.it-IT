...
## DESCRIZIONE
Questo comando crea un endpoint cloud di Azure File Sync. Un endpoint cloud è un riferimento a una condivisione file di Azure esistente. Rappresenta la condivisione file e la definisce come partecipazione alla sincronizzazione di tutti i file che fanno parte del gruppo di sincronizzazione in cui è stato creato l'endpoint cloud.
...
Un endpoint cloud è un membro integrale di un gruppo di sincronizzazione, in questo esempio ne viene creato uno all'interno di un gruppo di sincronizzazione esistente denominato "mySyncGroupName".
...
### -AzureFileShareName
Nome condivisione account di archiviazione (nome condivisione file di Azure)
...
### -Name
Nome dell'endpoint cloud. Una volta creato nel portale di Azure, Il nome viene impostato sul nome della condivisione file di Azure a cui fa riferimento.
...
ID tenant account di archiviazione (ID directory aziendale)
