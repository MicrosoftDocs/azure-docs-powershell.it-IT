...
Ottiene i record dell'evento dei criteri generati nell'ultimo giorno per tutte le risorse all'interno della sottoscrizione specificata.
...
Recupera i record di eventi dei criteri generati nell'ultimo giorno per tutte le risorse (all'interno del tenant nel contesto della sessione corrente) a cui è stata attivata la definizione del set di criteri specificata (esistente nella sottoscrizione specificata).
...
Si può anche selezionare di elencare solo un sottoinsieme di colonne per ogni record.
...
Ottiene i record dell'evento dei criteri generati nell'intervallo di date specificato per tutte le risorse all'interno della sottoscrizione nel contesto della sessione corrente.
...
Recupera i record dell'evento dei criteri generati nell'ultimo giorno per tutte le risorse nella sottoscrizione nel contesto della sessione corrente. Il comando limita i risultati restituiti dal filtro in base all'azione di definizione dei criteri (include solo gli eventi di controllo e di negazione).
Raggruppa i risultati in base all'ID risorsa. In questo modo viene generato l'elenco di tutte le risorse all'interno della sottoscrizione che ha generato un evento dei criteri per almeno un criterio di controllo o di negazione.
...
Raggruppa prima i risultati in base all'assegnazione dei criteri, alla definizione dei criteri e all'ID risorsa. Quindi, raggruppa ulteriormente i risultati di questo raggruppamento con le stesse proprietà tranne l'ID risorsa e calcola il numero di record in ognuno di questi gruppi, che viene restituito all'interno della proprietà AdditionalProperties.
Ordina i risultati in base all'aggregazione conteggio in ordine decrescente e accetta solo i primi 5 di quelli elencati in tale ordine.
