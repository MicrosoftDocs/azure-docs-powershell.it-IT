...
## SYNOPSIS
Creare una regola per la raccolta dei dati.
...
## DESCRIZIONE
Il cmdlet **New-AzDataCollectionRule** crea una regola per la raccolta dei dati.
...
Per usare il parametro -RuleFile, creare un file json contenente tre proprietà: dataSources, destinations, dataFlows (vedere esempio #1)

I dettagli dello schema sono [qui.](https://docs.microsoft.com/rest/api/monitor/datacollectionrules/create)

È supportato anche l'output di un DCR serializzato con il cmdlet ConvertTo-Json (vedere l'esempio #2).

## ESEMPI

### Esempio 1: Creare una regola di raccolta dati, JSON dall'API Rest
```powershell
...
Questo comando crea regole di raccolta dati per la sottoscrizione corrente.

### Esempio 2: Creare una regola per la raccolta dei dati, JSON da PSDataCollectionRuleResource
```powershell
PS C:\>New-AzDataCollectionRule -Location 'East US 2 EUAP' -ResourceGroupName 'testdcr'
