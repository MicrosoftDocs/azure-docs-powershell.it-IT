...
## SYNOPSIS
Aggiorna (sostituzione completa) una regola per la raccolta dei dati.
...
## DESCRIZIONE
Il cmdlet **Set-AzDataCollectionRule** sostituisce una regola di raccolta dati esistente.
...
Per usare il parametro -RuleFile, creare un file json contenente tre proprietà: dataSources, destinations, dataFlows (vedere l'esempio #1).

I dettagli dello schema sono [qui.](https://docs.microsoft.com/rest/api/monitor/datacollectionrules/create)

È supportato anche l'output di un DCR serializzato con il cmdlet ConvertTo-Json (esempio #2).

## ESEMPI

### Esempio 1: Aggiornare la regola di raccolta dati, JSON dall'API Rest
```powershell
...
Questo comando sostituisce le regole di raccolta dati esistenti per la sottoscrizione corrente.

### Esempio 2: Aggiornare la regola di raccolta dati, JSON da PSDataCollectionRuleResource
```powershell
PS C:\>Set-AzDataCollectionRule -Location 'East US 2 EUAP'
...
### Esempio 3: Aggiornare la regola di raccolta dati dall'oggetto
```powershell
