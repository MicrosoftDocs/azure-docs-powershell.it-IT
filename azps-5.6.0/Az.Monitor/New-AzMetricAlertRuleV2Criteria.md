...
## DESCRIZIONE
Il cmdlet **New-AzMetricAlertRuleV2Criteria** crea un oggetto criteri metrico locale da usare come cmdlet [add-AzMetricAlertRuleV2](https://docs.microsoft.com/powershell/module/az.monitor/add-azmetricalertrulev2) di input, che crea una nuova regola di avviso per la metrica.

## ESEMPI

### Esempio 1: Creare un semplice criterio di avviso per una metrica
...
Questo comando crea un semplice criterio di avviso metrico che può essere usato in una regola di avviso metrica

### Esempio 2: Creare criteri di avviso per una metrica dinamica
...
### Esempio 3: Creare criteri di avviso per una metrica più complessi
...
Questo set di comandi crea un criterio di avviso metrico più complesso, che include la selezione della dimensione

### Esempio 4: Creare criteri di disponibilità per un test Web
...
Questo comando crea un criterio di disponibilità per il test Web che può essere usato in una regola di avviso metrica
...
Il numero totale di punti esaminati
...
### -FailedLocationCount
Il numero minimo di posizioni non riuscite per generare un avviso.
...
### -SkipMetricValid
Consente di creare una regola di avviso su una metrica personalizzata non ancora emessa, causando l'omissione della convalida della metrica
...
### -TimeAggregation
Operazione di aggregazione usata per eseguire il roll up di più valori di metrica nell'intervallo della finestra
...
### -ViolationCount
Il numero minimo di violazioni necessarie nell'intervallo di tempo di lookback selezionato necessario per generare un avviso
