...
## DESCRIZIONE
Il cmdlet **Set-AzVMADDomainExtension** aggiunge un'estensione di macchina virtuale del dominio di Azure Active Directory (AD) a una macchina virtuale.
Questa estensione consente alla macchina virtuale di aggiungere un dominio.
...
### -JoinOption
Specifica l'opzione di join. Per le opzioni di join, [vedere fJoinOptions](https://docs.microsoft.com/windows/desktop/api/lmjoin/nf-lmjoin-netjoindomain)
...
### -Name
Specifica il nome dell'estensione di dominio da aggiungere.
...
### -OUPath
Specifica un'unità organizzativa (OU) per l'account di dominio.
...
Per rendere effettiva la modifica è spesso necessario un riavvio.
...
### -TypeHandlerVersion
Specifica la versione dell'estensione di dominio.
