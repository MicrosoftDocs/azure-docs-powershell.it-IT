...
### -EnableUltrasSD
Consente di avere uno o più dischi dati gestiti con un UltraSSD_LRS account di archiviazione nel set di scalabilità della macchina virtuale.
...
### -Extension
Specifica l'oggetto informazioni sull'estensione per il sistema VMSS. È possibile usare il cmdlet **Add-AzVmssExtension** per aggiungere questo oggetto.
...
### -MaxPrice
Specifica il prezzo massimo che sei disposto a pagare per una macchina virtuale/VMSS Spot. Questo prezzo è in dollari statunitensi. Questo prezzo verrà confrontato con il prezzo spot corrente per le dimensioni della macchina virtuale. Inoltre, i prezzi vengono confrontati al momento della creazione/aggiornamento di Spot VM/VMSS e l'operazione avrà esito positivo solo se maxPrice è maggiore del prezzo spot corrente. MaxPrice verrà usato anche per l'eliminazione di una VM/VMSS Spot se il prezzo corrente di Spot supera il valore maxPrice dopo la creazione di VM/VMSS. I valori possibili sono: qualsiasi valore decimale maggiore di zero. Esempio: 0,01538.  -1 indica che non è consigliabile sfrattare spot VM/VMSS per motivi di prezzo. Inoltre, il prezzo massimo predefinito è -1 se non viene fornito dall'utente.
...
"Posizione" è per la macchina virtuale spot.
...
### -Zone
Specifica l'elenco di zona per il set di scalabilità della macchina virtuale.
