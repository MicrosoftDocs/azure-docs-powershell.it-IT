...
Il primo comando crea un oggetto disco vuoto locale con dimensioni di 5 GB Standard_LRS tipo di account di archiviazione. Configura anche il tipo di sistema operativo Windows e abilita le impostazioni di crittografia. Il secondo e il terzo comando impostano la chiave di crittografia disco e le relative impostazioni per l'oggetto disco. L'ultimo comando accetta l'oggetto disco e crea un disco con il nome 'Disk01' nel gruppo di risorse 'ResourceGroup01'.
...
### -BurstingEnabled
Abilita lo scatto a raffica oltre la destinazione delle prestazioni di provisioning del disco. Per impostazione predefinita, lo scatto a scatto a scatto è disabilitato. Non si applica ai dischi Ultra.
...
### -DiskMBpsReadOnly
"descrizione": "Velocità effettiva totale (MBps) consentita in tutte le macchine virtuali che montaggio il disco condiviso come ReadOnly. MBps significa milioni di byte al secondo - MB usa la notazione ISO, ovvero potenze di 10.
...
### -DiskMBpsReadWrite
La larghezza di banda consentita per il disco; impostabile solo per dischi UltraSSD. MBps significa milioni di byte al secondo - MB usa la notazione ISO, ovvero potenze di 10.
...
### -SKUName
Specifica il nome SKU dell'account di archiviazione.  I valori disponibili Standard_LRS, Premium_LRS, StandardSSD_LRS e UltraSSD_LRS.  UltraSSD_LRS può essere usato solo con valori vuoti per il parametro CreateOption.
