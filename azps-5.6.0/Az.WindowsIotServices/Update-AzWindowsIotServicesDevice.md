...
## SYNOPSIS
Aggiorna i metadati di un servizio dispositivi Windows IoT.
Il modello più comune per modificare una proprietà è recuperare i metadati e i metadati di sicurezza di Windows IoT Device Service e quindi combinarli con i valori modificati in un nuovo corpo per aggiornare il servizio dispositivi Windows IoT.
...
### Esempio 1: Aggiornare un servizio IoT di Windows in base al nome
```powershell
...
Questo comando aggiorna i servizi Windows IoT in base al nome.

### Esempio 2: Aggiornare un servizio IoT di Windows tramite pipeline
```powershell
...
Questo comando aggiorna i servizi IoT di Windows tramite pipeline.

## PARAMETERS

### -AdminDomainName
Dominio OEM AAD del servizio dispositivi Windows IoT
...
### -BillingDomainName
Dominio AAD del servizio ODM per dispositivi Windows IoT
...
### -Etag
Il campo Etag non *è* obbligatorio.
Se viene fornito nel corpo della risposta, deve anche essere fornito come intestazione in base alla normale convenzione ETag.
...
### -IfMatch
ETag del servizio dispositivi Windows IoT.
Non specificare per la creazione di un nuovo servizio dispositivi Windows IoT.
Necessario per aggiornare un servizio dispositivi Windows IoT esistente.
...
### -Location
Area geografica di Azure in cui si trova la risorsa
...
### -Nota
Note di Windows IoT Device Service.
...
### -Quantity
Allocazione del dispositivo Servizio dispositivi IoT di Windows,
...
INPUTOBJECT <IWindowsIotServicesIdentity> : parametro Identity
  - `[DeviceName <String>]`: nome del servizio dispositivi Windows IoT.
