---
Module Name: Az.Storage
Module Guid: da67eaa7-4cb1-4bfa-a194-8bf3faae8ac5
Download Help Link: https://docs.microsoft.com/powershell/module/az.storage
Help Version: 4.2.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
ms.openlocfilehash: a410a2e398fc9f1389193383f9a8be00aaa259cb
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 03/04/2021
ms.locfileid: "101988775"
---
# Modulo Az.Storage
## Descrizione
Questo argomento visualizza gli argomenti della Guida per i cmdlet di Gestione archiviazione di Azure.

## Cmdlet di Az.Storage
### [Add-AzRmStorageContainerLegalHold](Add-AzRmStorageContainerLegalHold.md)
Aggiunge tag di blocco a un contenitore BLOB di archiviazione

### [Add-AzStorageAccountManagementPolicyAction](Add-AzStorageAccountManagementPolicyAction.md)
Aggiunge un'azione all'oggetto gruppo di azioni ManagementPolicy di input oppure crea un oggetto gruppo di azioni ManagementPolicy con l'azione. L'oggetto può essere usato in New-AzStorageAccountManagementPolicyRule.

### [Add-AzStorageAccountNetworkRule](Add-AzStorageAccountNetworkRule.md)
 Aggiungere IpRules o VirtualNetworkRules alla proprietà NetworkRule di un account di archiviazione

### [Close-AzStorageFileHandle](Close-AzStorageFileHandle.md)
Chiude gli handle di file di una condivisione file, di una directory o di un file.

### [Copy-AzStorageBlob](Copy-AzStorageBlob.md)
Copiare un BLOB in modo sincrono.

### [Disable-AzStorageBlobDeleteRetentionPolicy](Disable-AzStorageBlobDeleteRetentionPolicy.md)
Disabilitare l'eliminazione dei criteri di conservazione per il servizio BLOB Archiviazione di Azure.

### [Disable-AzStorageBlobRestorePolicy](Disable-AzStorageBlobRestorePolicy.md)
Disabilita i criteri di ripristino BLOB in un account di archiviazione.

### [Disable-AzStorageDeleteRetentionPolicy](Disable-AzStorageDeleteRetentionPolicy.md)
Disabilitare l'eliminazione dei criteri di conservazione per il servizio BLOB Archiviazione di Azure.

### [Disable-AzStorageStaticWebsite](Disable-AzStorageStaticWebsite.md)
Disabilitare il sito Web statico per l'account di archiviazione di Azure.

### [Enable-AzStorageBlobDeleteRetentionPolicy](Enable-AzStorageBlobDeleteRetentionPolicy.md)
Abilitare l'eliminazione dei criteri di conservazione per il servizio BLOB Archiviazione di Azure.

### [Enable-AzStorageBlobRestorePolicy](Enable-AzStorageBlobRestorePolicy.md)
Abilita i criteri di ripristino BLOB in un account di archiviazione.

### [Enable-AzStorageDeleteRetentionPolicy](Enable-AzStorageDeleteRetentionPolicy.md)
Abilitare l'eliminazione dei criteri di conservazione per il servizio BLOB Archiviazione di Azure.

### [Enable-AzStorageStaticWebsite](Enable-AzStorageStaticWebsite.md)
Abilitare il sito Web statico per l'account di archiviazione di Azure.

### [Get-AzDataLakeGen2Item](Get-AzDataLakeGen2ChildItem.md)
Elenca le sotto directory e i file da una directory o da una radice del file system.

### [Get-AzDataLakeGen2Item](Get-AzDataLakeGen2Item.md)
Recupera i dettagli di un file o di una directory in un filesystem.

### [Get-AzDataLakeGen2ItemContent](Get-AzDataLakeGen2ItemContent.md)
Scaricare un file.

### [Get-AzRmStorageContainer](Get-AzRmStorageContainer.md)
Ottiene o elenca contenitori BLOB di archiviazione

### [Get-AzRmStorageContainerImmutabilityPolicy](Get-AzRmStorageContainerImmutabilityPolicy.md)
Ottiene ImmutabilityPolicy di contenitori BLOB di archiviazione

### [Get-AzRmStorageShare](Get-AzRmStorageShare.md)
Ottiene o elenca le condivisioni file di archiviazione.

### [Get-AzStorageAccount](Get-AzStorageAccount.md)
Ottiene un account di archiviazione.

### [Get-AzStorageAccountKey](Get-AzStorageAccountKey.md)
Ottiene le chiavi di accesso per un account di archiviazione di Azure.

### [Get-AzStorageAccountManagementPolicy](Get-AzStorageAccountManagementPolicy.md)
Recupera i criteri di gestione di un account di archiviazione di Azure.

### [Get-AzStorageAccountNameAvailability](Get-AzStorageAccountNameAvailability.md)
Controlla la disponibilità di un nome di account di archiviazione.

### [Get-AzStorageAccountNetworkRuleSet](Get-AzStorageAccountNetworkRuleSet.md)
Ottenere la proprietà NetWorkRule di un account di archiviazione

### [Get-AzStorageBlob](Get-AzStorageBlob.md)
Elenca i BLOB in un contenitore.

### [Get-AzStorageBlobContent](Get-AzStorageBlobContent.md)
Scarica un BLOB di archiviazione.

### [Get-AzStorageBlobCopyState](Get-AzStorageBlobCopyState.md)
Ottiene lo stato della copia di un BLOB di Archiviazione di Azure.

### [Get-AzStorageBlobQueryResult](Get-AzStorageBlobQueryResult.md)
Applica una semplice istruzione Structured Query Language (SQL) ai contenuti di un BLOB e salva solo il sottoinsieme di dati in cui viene eseguita la query in un file locale.

### [Get-AzStorageBlobServiceProperty](Get-AzStorageBlobServiceProperty.md)
Ottiene le proprietà del servizio per i servizi BLOB di archiviazione di Azure.

### [Get-AzStorageContainer](Get-AzStorageContainer.md)
Elenca i contenitori di archiviazione.

### [Get-AzStorageContainerStoredAccessPolicy](Get-AzStorageContainerStoredAccessPolicy.md)
Recupera i criteri di accesso archiviati per un contenitore di archiviazione di Azure.

### [Get-AzStorageCORSRule](Get-AzStorageCORSRule.md)
Ottiene le regole CORS per un tipo di servizio di archiviazione.

### [Get-AzStorageEncryptionScope](Get-AzStorageEncryptionScope.md)
Ottenere o elencare gli ambiti di crittografia da un account di archiviazione.

### [Get-AzStorageFile](Get-AzStorageFile.md)
Elenca le directory e i file per un percorso.

### [Get-AzStorageFileContent](Get-AzStorageFileContent.md)
Scarica il contenuto di un file.

### [Get-AzStorageFileCopyState](Get-AzStorageFileCopyState.md)
Ottiene lo stato di un'operazione di copia.

### [Get-AzStorageFileHandle](Get-AzStorageFileHandle.md)
Elenca gli handle di file di una condivisione file, di una directory o di un file.

### [Get-AzStorageFileServiceProperty](Get-AzStorageFileServiceProperty.md)
Ottiene le proprietà del servizio per i servizi File di archiviazione di Azure.

### [Get-AzStorageObjectReplicationPolicy](Get-AzStorageObjectReplicationPolicy.md)
Ottiene o elenca i criteri di replica degli oggetti di un account di archiviazione.

### [Get-AzStorageQueue](Get-AzStorageQueue.md)
Elenca le code di archiviazione.

### [Get-AzStorageQueueStoredAccessPolicy](Get-AzStorageQueueStoredAccessPolicy.md)
Recupera i criteri di accesso archiviati per una coda di archiviazione di Azure.

### [Get-AzStorageServiceLoggingProperty](Get-AzStorageServiceLoggingProperty.md)
Ottiene le proprietà di registrazione per i servizi di archiviazione di Azure.

### [Get-AzStorageServiceMetricsProperty](Get-AzStorageServiceMetricsProperty.md)
Recupera le proprietà delle metriche per il servizio di archiviazione di Azure.

### [Get-AzStorageServiceProperty](Get-AzStorageServiceProperty.md)
Ottiene le proprietà per i servizi di archiviazione di Azure.

### [Get-AzStorageShare](Get-AzStorageShare.md)
Ottiene un elenco di condivisioni file.

### [Get-AzStorageShareStoredAccessPolicy](Get-AzStorageShareStoredAccessPolicy.md)
Ottiene i criteri di accesso archiviati per una condivisione di archiviazione.

### [Get-AzStorageTable](Get-AzStorageTable.md)
Elenca le tabelle di archiviazione.

### [Get-AzStorageTableStoredAccessPolicy](Get-AzStorageTableStoredAccessPolicy.md)
Recupera i criteri di accesso archiviati per una tabella di archiviazione di Azure.

### [Get-AzStorageUsage](Get-AzStorageUsage.md)
Ottiene l'uso delle risorse di archiviazione della sottoscrizione corrente.

### [Invoke-AzStorageAccountFailover](Invoke-AzStorageAccountFailover.md)
Richiama il failover di un account di archiviazione.

### [Lock-AzRmStorageContainerImmutabilityPolicy](Lock-AzRmStorageContainerImmutabilityPolicy.md)
Blocca i criteri di messaggistica non modificabile di contenitori BLOB di archiviazione

### [Move-AzDataLakeGen2Item](Move-AzDataLakeGen2Item.md)
Spostare un file o una directory in un altro file o directory nello stesso account di archiviazione.

### [New-AzDataLakeGen2Item](New-AzDataLakeGen2Item.md)
Creare un file o una directory in un filesystem.

### [New-AzRmStorageContainer](New-AzRmStorageContainer.md)
Crea un contenitore BLOB di archiviazione

### [New-AzRmStorageShare](New-AzRmStorageShare.md)
Crea una condivisione file di archiviazione.

### [New-AzStorageAccount](New-AzStorageAccount.md)
Crea un account di archiviazione.

### [New-AzStorageAccountKey](New-AzStorageAccountKey.md)
Rigenera una chiave di archiviazione per un account di archiviazione di Azure.

### [New-AzStorageAccountManagementPolicyFilter](New-AzStorageAccountManagementPolicyFilter.md)
Crea un oggetto filtro regola ManagementPolicy, che può essere usato in New-AzStorageAccountManagementPolicyRule.

### [New-AzStorageAccountManagementPolicyRule](New-AzStorageAccountManagementPolicyRule.md)
Crea un oggetto regola ManagementPolicy, che può essere usato in Set-AzStorageAccountManagementPolicy.

### [New-azStorageAccountSASToken](New-AzStorageAccountSASToken.md)
Crea un token di firma di accesso condiviso a livello di account.

### [New-AzStorageBlobQueryConfig](New-AzStorageBlobQueryConfig.md)
Crea un oggetto di configurazione della query BLOB, che può essere usato in Get-AzStorageBlobQueryResult.

### [New-AzStorageBlobRangeToRestore](New-AzStorageBlobRangeToRestore.md)
Crea un oggetto Intervallo BLOB per ripristinare un account di archiviazione.

### [New-AzStorageBlobSASToken](New-AzStorageBlobSASToken.md)
Genera un token di firma di accesso condiviso per un BLOB di archiviazione di Azure.

### [New-AzStorageContainer](New-AzStorageContainer.md)
Crea un contenitore di archiviazione di Azure.

### [New-azStorageContainersASToken](New-AzStorageContainerSASToken.md)
Genera un token di firma di accesso condiviso per un contenitore di archiviazione di Azure.

### [New-AzStorageContainerStoredAccessPolicy](New-AzStorageContainerStoredAccessPolicy.md)
Crea criteri di accesso archiviato per un contenitore di archiviazione di Azure.

### [New-AzStorageContext](New-AzStorageContext.md)
Crea un contesto di Archiviazione di Azure.

### [New-AzStorageDirectory](New-AzStorageDirectory.md)
Crea una directory.

### [New-AzStorageEncryptionScope](New-AzStorageEncryptionScope.md)
Crea un ambito di crittografia per un account di archiviazione.

### [New-azStorageFilesASToken](New-AzStorageFileSASToken.md)
Genera un token di firma di accesso condiviso per un file di archiviazione.

### [New-AzStorageObjectReplicationPolicyRule](New-AzStorageObjectReplicationPolicyRule.md)
Crea una regola dei criteri di replica degli oggetti.

### [New-AzStorageQueue](New-AzStorageQueue.md)
Crea una coda di archiviazione.

### [New-azStorageQueuesASToken](New-AzStorageQueueSASToken.md)
Genera un token di firma di accesso condiviso per una coda di archiviazione di Azure.

### [New-AzStorageQueueStoredAccessPolicy](New-AzStorageQueueStoredAccessPolicy.md)
Crea criteri di accesso archiviato per una coda di archiviazione di Azure.

### [New-AzStorageShare](New-AzStorageShare.md)
Crea una condivisione file.

### [New-azStorageShareSASToken](New-AzStorageShareSASToken.md)
Generare il token della firma di accesso condiviso per la condivisione di Archiviazione di Azure.

### [New-AzStorageShareStoredAccessPolicy](New-AzStorageShareStoredAccessPolicy.md)
Crea criteri di accesso archiviati in una condivisione di archiviazione.

### [New-AzStorageTable](New-AzStorageTable.md)
Crea una tabella di archiviazione.

### [New-azStorageTableSASToken](New-AzStorageTableSASToken.md)
Genera un token di firma di accesso condiviso per una tabella di archiviazione di Azure.

### [New-AzStorageTableStoredAccessPolicy](New-AzStorageTableStoredAccessPolicy.md)
Crea criteri di accesso archiviato per una tabella di archiviazione di Azure.

### [Remove-AzDataLakeGen2AclRecursive](Remove-AzDataLakeGen2AclRecursive.md)
Rimuovere L'ACL in modo ricorsivo nel percorso specificato. 

### [Remove-AzDataLakeGen2Item](Remove-AzDataLakeGen2Item.md)
Rimuovere un file o una directory.

### [Remove-AzRmStorageContainer](Remove-AzRmStorageContainer.md)
Rimuove un contenitore BLOB di archiviazione

### [Remove-AzRmStorageContainerImmutabilityPolicy](Remove-AzRmStorageContainerImmutabilityPolicy.md)
Rimuove ImmutabilityPolicy di un contenitore BLOB Archiviazione con un criterio sbloccato

### [Remove-AzRmStorageContainerLegalHold](Remove-AzRmStorageContainerLegalHold.md)
Rimuove i tag di blocco legale da un contenitore BLOB di archiviazione

### [Remove-AzRmStorageShare](Remove-AzRmStorageShare.md)
Rimuove una condivisione file di archiviazione.

### [Remove-AzStorageAccount](Remove-AzStorageAccount.md)
Rimuove un account di archiviazione da Azure.

### [Remove-AzStorageAccountManagementPolicy](Remove-AzStorageAccountManagementPolicy.md)
Rimuove i criteri di gestione di un account di archiviazione di Azure.

### [Remove-AzStorageAccountNetworkRule](Remove-AzStorageAccountNetworkRule.md)
Rimuovere IpRules o VirtualNetworkRules dalla proprietà NetWorkRule di un account di archiviazione

### [Remove-AzStorageBlob](Remove-AzStorageBlob.md)
Rimuove il BLOB di archiviazione specificato.

### [Remove-AzStorageContainer](Remove-AzStorageContainer.md)
Rimuove il contenitore di archiviazione specificato.

### [Remove-AzStorageContainerStoredAccessPolicy](Remove-AzStorageContainerStoredAccessPolicy.md)
Rimuove un criterio di accesso archiviato da un contenitore di archiviazione di Azure.

### [Remove-AzStorageCORSRule](Remove-AzStorageCORSRule.md)
Rimuove cors per un servizio di archiviazione.

### [Remove-AzStorageDirectory](Remove-AzStorageDirectory.md)
Elimina una directory.

### [Remove-AzStorageFile](Remove-AzStorageFile.md)
Elimina un file.

### [Remove-AzStorageObjectReplicationPolicy](Remove-AzStorageObjectReplicationPolicy.md)
Rimuove i criteri di replica degli oggetti specificati da un account di archiviazione.

### [Remove-AzStorageQueue](Remove-AzStorageQueue.md)
Rimuove una coda di archiviazione.

### [Remove-AzStorageQueueStoredAccessPolicy](Remove-AzStorageQueueStoredAccessPolicy.md)
Rimuove un criterio di accesso archiviato da una coda di archiviazione di Azure.

### [Remove-AzStorageShare](Remove-AzStorageShare.md)
Elimina una condivisione file.

### [Remove-AzStorageShareStoredAccessPolicy](Remove-AzStorageShareStoredAccessPolicy.md)
Rimuove i criteri di accesso archiviati da una condivisione di archiviazione.

### [Remove-AzStorageTable](Remove-AzStorageTable.md)
Rimuove una tabella di archiviazione.

### [Remove-AzStorageTableStoredAccessPolicy](Remove-AzStorageTableStoredAccessPolicy.md)
Rimuove un criterio di accesso archiviato da una tabella di archiviazione di Azure.

### [Restore-AzRmStorageShare](Restore-AzRmStorageShare.md)
Ripristina una condivisione file eliminata.

### [Restore-AzStorageBlobRange](Restore-AzStorageBlobRange.md)
Ripristina un account di archiviazione per intervalli BLOB specifici.

### [Revoke-AzStorageAccountUserDelegationKeys](Revoke-AzStorageAccountUserDelegationKeys.md)
Revocare tutte le chiavi di delega utente di un account di archiviazione.

### [Set-AzCurrentStorageAccount](Set-AzCurrentStorageAccount.md)
Modifica l'account di archiviazione corrente della sottoscrizione specificata.

### [Set-AzDataLakeGen2AclRecursive](Set-AzDataLakeGen2AclRecursive.md)
Impostare L'ACL in modo ricorsivo nel percorso specificato. 

### [Set-AzDataLakeGen2ItemAclObject](Set-AzDataLakeGen2ItemAclObject.md)
Crea/aggiorna un oggetto ACL dell'elemento DataLake gen2, che può essere usato nel cmdlet Update-AzDataLakeGen2Item dati.

### [Set-AzRmStorageContainerImmutabilityPolicy](Set-AzRmStorageContainerImmutabilityPolicy.md)
Crea o aggiorna ImmutabilityPolicy di contenitori BLOB di archiviazione

### [Set-AzStorageAccount](Set-AzStorageAccount.md)
Modifica un account di archiviazione.

### [Set-AzStorageAccountManagementPolicy](Set-AzStorageAccountManagementPolicy.md)
Crea o modifica i criteri di gestione di un account di archiviazione di Azure.

### [Set-AzStorageBlobContent](Set-AzStorageBlobContent.md)
Carica un file locale in un BLOB di Archiviazione di Azure.

### [Set-AzStorageContainerAcl](Set-AzStorageContainerAcl.md)
Imposta l'autorizzazione di accesso pubblico su un contenitore di archiviazione.

### [Set-AzStorageContainerStoredAccessPolicy](Set-AzStorageContainerStoredAccessPolicy.md)
Imposta un criterio di accesso archiviato per un contenitore di archiviazione di Azure.

### [Set-AzStorageCORSRule](Set-AzStorageCORSRule.md)
Imposta le regole CORS per un tipo di servizio di archiviazione.

### [Set-AzStorageFileContent](Set-AzStorageFileContent.md)
Carica il contenuto di un file.

### [Set-AzStorageObjectReplicationPolicy](Set-AzStorageObjectReplicationPolicy.md)
Crea o aggiorna i criteri di replica degli oggetti specificati in un account di archiviazione.

### [Set-AzStorageQueueStoredAccessPolicy](Set-AzStorageQueueStoredAccessPolicy.md)
Imposta un criterio di accesso archiviato per una coda di archiviazione di Azure.

### [Set-AzStorageServiceLoggingProperty](Set-AzStorageServiceLoggingProperty.md)
Modifica la registrazione per i servizi di archiviazione di Azure.

### [Set-AzStorageServiceMetricsProperty](Set-AzStorageServiceMetricsProperty.md)
Modifica le proprietà delle metriche per il servizio archiviazione di Azure.

### [Set-AzStorageShareQuota](Set-AzStorageShareQuota.md)
Imposta la capacità di archiviazione per una condivisione.

### [Set-AzStorageShareStoredAccessPolicy](Set-AzStorageShareStoredAccessPolicy.md)
Aggiorna i criteri di accesso archiviati in una condivisione di archiviazione.

### [Set-AzStorageTableStoredAccessPolicy](Set-AzStorageTableStoredAccessPolicy.md)
Imposta i criteri di accesso archiviato per una tabella di archiviazione di Azure.

### [Start-AzStorageBlobCopy](Start-AzStorageBlobCopy.md)
Inizia a copiare un BLOB.

### [Start-AzStorageBlobIncrementalCopy](Start-AzStorageBlobIncrementalCopy.md)
Avviare un'operazione di copia incrementale da uno snapshot BLOB delle pagine al BLOB delle pagine di destinazione specificato.

### [Start-AzStorageFileCopy](Start-AzStorageFileCopy.md)
Inizia a copiare un file di origine.

### [Stop-AzStorageBlobCopy](Stop-AzStorageBlobCopy.md)
Interrompe un'operazione di copia.

### [Stop-AzStorageFileCopy](Stop-AzStorageFileCopy.md)
Interrompe un'operazione di copia nel file di destinazione specificato.

### [Update-AzDataLakeGen2AclRecursive](Update-AzDataLakeGen2AclRecursive.md)
Aggiornare L'ACL in modo ricorsivo nel percorso specificato. 

### [Update-AzDataLakeGen2Item](Update-AzDataLakeGen2Item.md)
Aggiornare un file o una directory in base a proprietà, metadati, autorizzazioni, ACL e proprietario.

### [Update-AzRmStorageContainer](Update-AzRmStorageContainer.md)
Modifica un contenitore BLOB di archiviazione

### [Update-AzRmStorageShare](Update-AzRmStorageShare.md)
Modifica una condivisione file di archiviazione.

### [Update-AzStorageAccountNetworkRuleSet](Update-AzStorageAccountNetworkRuleSet.md)
Aggiornare la proprietà NetworkRule di un account di archiviazione

### [Update-AzStorageBlobServiceProperty](Update-AzStorageBlobServiceProperty.md)
Modifica le proprietà del servizio per il servizio BLOB archiviazione di Azure.

### [Update-AzStorageEncryptionScope](Update-AzStorageEncryptionScope.md)
Modificare un ambito di crittografia per un account di archiviazione.

### [Update-AzStorageFileServiceProperty](Update-AzStorageFileServiceProperty.md)
Modifica le proprietà del servizio per il servizio File di archiviazione di Azure.

### [Update-AzStorageServiceProperty](Update-AzStorageServiceProperty.md)
Modifica le proprietà per il servizio di archiviazione di Azure.

