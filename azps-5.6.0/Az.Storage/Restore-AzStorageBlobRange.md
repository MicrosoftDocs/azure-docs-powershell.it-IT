...
## DESCRIZIONE
Il cmdlet **Restore-AzStorageBlobRange** ripristina i BLOB in un account di archiviazione per intervalli BLOB specifici.
L'intervallo iniziale è incluso e l'intervallo finale è escluso in Ripristino BLOB.

## ESEMPI

### Esempio 1: Avviare il ripristino di BLOB in un account di archiviazione con intervalli BLOB specifici
```powershell
...
Questo comando crea prima 2 intervalli di BLOB, quindi avvia il ripristino dei BLOB in un account di archiviazione con i 2 intervalli di BLOB di 1 giorno fa. L'utente può usare Get-AzStorageAccount per tracciare lo stato di ripristino in un secondo momento.

### Esempio 2: Ripristina tutti i BLOB in un account di archiviazione nel back-end
```powershell
...
Questo comando ripristina tutti i BLOB in un account di archiviazione da 30 minuti fa e attende il completamento del ripristino. Poiché il ripristino dei BLOB potrebbe richiedere molto tempo, eseguirlo nel back-end con il parametro -Asjob e quindi attendere il completamento del processo e visualizzare il risultato.

### Esempio 3: Ripristina direttamente i BLOB in base a intervalli BLOB di input e attendi il completamento
```powershell
...
Questo comando ripristina i BLOB in un account di archiviazione da 1 giorno prima, input 2 intervalli BLOB direttamente nel cmdlet Restore-AzStorageBlobRange archiviazione. Questo comando attenderà il completamento del ripristino.
...
### -BlobRestoreRange
Intervallo BLOB da ripristinare.
Se non si specifica questo parametro, verranno ripristinati tutti i BLOB.
