...
Deve essere eseguita nel BLOB di destinazione della copia.
...
Questo comando recupera lo stato della copia del BLOB denominato ContosoPlanning2015 nel contenitore ContosoUploads.

### Esempio 2: Ottenere lo stato di copia di un BLOB tramite la pipeline
```
C:\PS>Get-AzStorageBlob -Blob "ContosoPlanning2015" -Container "ContosoUploads" | Get-AzStorageBlobCopyState
```

Questo comando recupera il BLOB denominato ContosoPlanning2015 nel contenitore ContosoUploads usando il cmdlet **Get-AzStorageBlob** e quindi passa il risultato al cmdlet corrente usando l'operatore della pipeline.
...
Questo comando ottiene il contenitore denominato con il cmdlet **Get-AzStorageBlob** e quindi passa il risultato al cmdlet corrente.
...
### Esempio 4: Avviare la copia e la pipeline per ottenere lo stato di copia
```
...
Il primo comando avvia la copia blob "ContosoPlanning2015" in "ContosoPlanning2015_copy" e restituisce l'output dell'oggetto BLOB destiantion. Il secondo comando pipeline l'oggetto BLOB destiantion a Get-AzStorageBlobCopyState, per ottenere lo stato di copia BLOB. 

## PARAMETERS

### -BLOB
Specifica il nome di un BLOB.
Questo cmdlet ottiene lo stato dell'operazione di copia BLOB per il BLOB Archiviazione di Azure specificato da questo parametro.
...
Questo cmdlet ottiene lo stato della copia per un BLOB nel contenitore specificato da questo parametro.
...
### -WaitForComplete
Indica che questo cmdlet attende il completamento della copia.
Se non si specifica questo parametro, questo cmdlet restituisce immediatamente un risultato.
