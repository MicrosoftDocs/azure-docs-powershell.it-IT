...
## DESCRIZIONE
Il cmdlet **Copy-AzStorageBlob** copia un BLOB in modo sincrono, attualmente supporta solo blob di blocco.
...
Questo comando copia un BLOB dal contenitore di origine al contenitore di destinazione con un nuovo nome DI BLOB. 

### Esempio 2: Copiare BLOB da un oggetto BLOB
```
...
Questo comando copia un BLOB dall'oggetto BLOB di origine al contenitore di destinazione con un nuovo nome DI BLOB.

### Esempio 3: Copiare BLOB da un URI BLOB
```
...
Il primo comando crea un URI BLOB del BLOB di origine, con il token sas dell'autorizzazione "rt". Il secondo comando copia l'URI del BLOB di origine nel BLOB di destinazione.

### Esempio 4: Aggiornare un ambito di crittografia BLOB di blocco
```
...
Questo comando aggiorna un ambito di crittografia BLOB di blocco copiarlo in se stesso con un nuovo ambito di crittografia.
...
### -Context
Oggetto contesto di archiviazione di Azure di origine
...
### -DestContext
Oggetto contesto Archiviazione di destinazione
...
### -EncryptionScope
Ambito di crittografia da usare quando si effettuano richieste al BLOB più recente.
...
### -RehydratePriority
Bloccare BLOB RehydratePriority.
Indica la priorità con cui rehydrate un BLOB archiviato.
I valori validi sono High/Standard.
