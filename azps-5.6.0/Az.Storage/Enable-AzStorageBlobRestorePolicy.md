...
### Esempio 1: Abilita i criteri di Ripristino BLOB per il servizio BLOB Archiviazione di Azure in un account di archiviazione
```powershell
...
Questo comando abilita prima softdelete BLOB e changefeed, quindi abilita Criteri di ripristino BLOB e infine controlla l'impostazione nelle proprietà del servizio BLOB.
Il valore di RestorePolicy del servizio BLOB.Days deve essere minore di DeleteRetentionPolicy.Days.
Blob softdelete e ChangeFeed devono essere abilitati prima di abilitare i criteri di ripristino BLOB.
Se le opzioni softdelete e changefeed sono appena abilitate, potrebbe essere necessario attendere del tempo prima che il server gestirà l'impostazione prima di abilitare i criteri di ripristino BLOB.
...
### -RestoreDays
Imposta il numero di giorni per cui è possibile ripristinare il BLOB.
