...
### Esempio 1: Aggiornare un database esistente in base al nome
```powershell
...
Il comando precedente aggiorna il periodo di eliminazione soft e il periodo di cache a scelta rapida del database Kusto "mykustodatabase" nel cluster "testnewkustocluster" trovato nel gruppo di risorse "testrg".

### Esempio 2: Aggiornare un database esistente con l'identità
```powershell
...
### Esempio 3: Aggiornare un database ReadOnly esistente in base al nome
```powershell
PS C:\> $2ds = New-TimeSpan -Days 2
...
Il comando precedente aggiorna il periodo di cache a scelta rapida del database Kusto "mykustodatabase" nel cluster "myfollowercluster" trovato nel gruppo di risorse "testrg".

### Esempio 4: Aggiornare un database ReadOnly esistente tramite l'identità
```powershell
...
### -HotCachePeriod
Ora in cui i dati devono essere mantenuti nella cache per le query veloci in TimeSpan.
...
### -SoftDeletePeriod
Il tempo che i dati devono mantenere prima di non essere più accessibili alle query nell'intervallo di tempo.
