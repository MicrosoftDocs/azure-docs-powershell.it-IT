...
## DESCRIZIONE
Il cmdlet **Add-AzDelegation** aggiunge una delega del servizio a una subnet Azure.

## ESEMPI

### Esempio 1: Aggiunta di una delega
```powershell
...
Il primo comando recupera la rete virtuale su cui si trova la subnet. Il secondo comando isola la subnet di interesse, ovvero quella che si vuole delegare. Il terzo comando aggiunge una delega alla subnet. Questo esempio può risultare utile quando si vuole abilitare SQL di creare endpoint di interfaccia in questa subnet. Il comando finale invia la subnet aggiornata al server per aggiornare effettivamente la subnet.
