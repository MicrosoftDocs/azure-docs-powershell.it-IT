...
## DESCRIZIONE
Il cmdlet **Set-AzExpressRouteConnection** aggiorna una connessione Express Route creata tra un gateway Express Route e il peering del circuito Express Route locale.
...
La procedura precedente consente di creare un gruppo di risorse, una WAN virtuale, una rete virtuale, un hub virtuale e un sito ExpressRoute negli Stati Uniti occidentali nel gruppo di risorse "testRG" in Azure. Successivamente verrà creato un gateway ExpressRoute nell'hub virtuale con 2 unità di scala.

Una volta creato, il gateway viene connesso al peering del circuito ExpressRoute locale usando il comando New-AzExpressRouteConnection connessione.

La connessione viene quindi aggiornata in modo da avere un valore RoutingWeight diverso usando il comando Set-AzExpressRouteConnection> .
...
### -AuthorizationKey
Chiave di autorizzazione da usare per creare la connessione del gateway ExpressRoute.
...
### -InputObject
Oggetto ExpressRouteConnection da aggiornare.
...
### -RoutingWeight
Peso che deve essere assegnato a questa connessione per il routing dei pacchetti.
