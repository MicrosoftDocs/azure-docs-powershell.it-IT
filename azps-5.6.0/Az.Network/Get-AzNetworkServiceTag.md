...
Tieni presente che le informazioni relative all'area geografica di Azure specificate verranno usate come riferimento per la versione (non come filtro basato sulla posizione). Ad esempio, anche se si specifica che si otterrà l'elenco dei tag di servizio con prefissi in tutte le aree geografiche ma limitati al cloud a cui appartiene l'abbonamento (ad esempio Pubblico, Governo degli Stati Uniti, Cina o `-Location eastus2` Germania).
...
Il comando recupera l'elenco delle risorse di informazioni sui tag di servizio e lo archivia in una `serviceTags` variabile.
...
Il primo comando ottiene l'elenco di risorse di informazioni sui tag di servizio e lo archivia in una `serviceTags` variabile.
Il secondo comando filtra l'elenco per selezionare le informazioni per Sql.

### Esempio 3: Ottenere la risorsa di informazioni sul tag di servizio di archiviazione per Stati Uniti occidentali 2
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
Il primo comando ottiene l'elenco di risorse di informazioni sui tag di servizio e lo archivia in una `serviceTags` variabile.
I comandi seguenti illustrano vari modi per filtrare l'elenco e selezionare le informazioni sui tag di servizio per l'archiviazione negli Stati Uniti occidentali 2.

### Esempio 4: Ottenere tutte le risorse sulle informazioni sui tag di servizio globali
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
Il primo comando ottiene l'elenco di risorse di informazioni sui tag di servizio e lo archivia in una `serviceTags` variabile.
Il secondo comando filtra l'elenco per selezionare solo quelli senza area geografica impostata.
...
### -Location
La posizione che verrà usata come riferimento per la versione (non come filtro basato sulla posizione, si otterrà l'elenco di tag di servizio con prefissi in tutte le aree geografiche ma limitati al cloud a cui appartiene l'abbonamento).
