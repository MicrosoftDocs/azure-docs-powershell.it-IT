...
In questo esempio viene creato prima un firewall senza raccolte di regole dell'applicazione. Dopo la creazione di una regola dell'applicazione e di una raccolta di regole dell'applicazione, l'oggetto Firewall viene modificato in memoria, senza influire sulla configurazione reale nel cloud. Perché le modifiche si riflettano nel cloud, Set-AzFirewall essere chiamato.
...
La nuova rete virtuale e l'INDIRIZZO IP pubblico devono essere nello stesso gruppo di risorse del firewall. Anche in questo caso, per fare in modo che le modifiche si riflettano nel cloud, Set-AzFirewall essere chiamato.
