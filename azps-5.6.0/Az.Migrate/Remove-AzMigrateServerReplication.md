...
## SYNOPSIS
Interrompe la replica per il server di cui è stata eseguita la migrazione.
...
Eseguire di nuovo la sincronizzazione in base all'ID.

### Esempio 2: Rimuovere per oggetto di input
```powershell
...
### -ForceRemove
Specifica se la replica deve essere forzata rimossa.
...
### -TargetObjectID
Specifica il server di replcating per cui è necessario disabilitare la replica.
