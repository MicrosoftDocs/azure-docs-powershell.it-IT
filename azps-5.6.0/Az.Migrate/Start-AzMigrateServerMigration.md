...
### -TargetObjectID
Specifica il server di replcating per cui deve essere avviata la migrazione.
...
INPUTOBJECT: specifica il server di replica per cui deve essere avviata <IMigrationItem> la migrazione. L'oggetto server può essere recuperato usando il cmdlet Get-AzMigrateServerReplication server.
