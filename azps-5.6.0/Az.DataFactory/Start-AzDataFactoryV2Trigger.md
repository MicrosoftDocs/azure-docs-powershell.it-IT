...
## DESCRIZIONE
Il cmdlet **Start-AzDataFactoryV2Trigger** avvia un trigger in un data factory. Se il trigger è in stato "Arrestato", il cmdlet avvia il trigger e alla fine richiama pipeline in base alla relativa definizione. Se il trigger è già in stato "Avviato", questo cmdlet non ha alcun effetto. Se si specifica il parametro Force, il cmdlet non richiede conferma prima di avviare il trigger.
