...
## DESCRIZIONE
Il cmdlet Set-AzAnalysisServicesServer modifica un'istanza del server Analysis Services
...
Modifica il server denominato testserver nel gruppo di test del gruppo di risorse in modo da impostare i tag key1:value1 e key2:value2 e administrator su testuser1@contoso.com

## PARAMETERS

### -Administrator
Stringa che rappresenta un elenco di utenti o gruppi separati da virgola da impostare come amministratori nel server.
È necessario che per gli utenti o i gruppi sia specificato il formato UPN, ad esempio user@contoso.comgroups@contoso.com
...
### -DisableBackup
Opzione per disabilitare il contenitore BLOB di backup.
Per abilitare di nuovo il contenitore BLOB di backup, specificare URI del contenitore BLOB di backup come -BackupBlobContainerUri.
...
### -DisassociateGateway
Disassociare una risorsa Gateway da un server Disassociato
...
### -GatewayResourceId
ID di risorsa del gateway da associare a un server Analysis
...
I valori supportati sono "S0", "S1", "S2", "S4", "S8", "S9", "S8v2", "S9v2" per il livello Standard; "B1", "B2" per il livello Base e "D1" per il livello di sviluppo.
