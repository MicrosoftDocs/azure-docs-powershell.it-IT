...
Creare un'associazione di provider personalizzata. Il provioder di destinazione associato deve essere configurato correttamente con un percorso per le "associazioni"
...
L'ambito può essere qualsiasi istanza di risorsa REST valida.
Ad esempio, usare '/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/Microsoft.Compute/virtualMachines/{vm-name}' per una risorsa della macchina virtuale.
...
### -TargetResourceId
Istanza della risorsa REST della risorsa di destinazione per questa associazione.
