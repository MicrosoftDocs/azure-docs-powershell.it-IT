...
### CreateTemplateArtifact (impostazione predefinita)
```
...
## DESCRIZIONE
Creare un nuovo elemento. Esistono due modi per creare un elemento: tramite un JSON dell'elemento come file di input o fornendo parametri inline per l'elemento. Anche se il metodo JSON non richiede che il tipo dell'elemento sia fornito con il metodo del parametro inline, è necessario fornire all'utente il tipo dell'elemento tramite il parametro -Type.
...
Creare un nuovo elemento tramite un file JSON dell'elemento.
...
Creare un nuovo elemento tramite parametri in linea.
...
Creare un nuovo elemento tramite un ARM file di modello personalizzato.
