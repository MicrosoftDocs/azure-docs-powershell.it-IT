...
## SYNOPSIS
Creare o aggiornare una regola di azione.

## SINTASSI

### BySimplifiedFormatDiagnosticsActionRule (Impostazione predefinita)
```
...
## DESCRIZIONE
**Set-AzActionRule** crea o aggiorna una regola di azione.
...
Questo cmdlet crea una regola di azione per la compressione, con un ambito di sottoscrizione.
...
Questo cmdlet crea una regola azione per il gruppo di azioni, con un elenco dell'ambito dei gruppi di risorse.
...
Questo cmdlet crea una regola azione per le impostazioni di diagnostica, con un ambito di risorse.

## PARAMETERS

### -ActionGroupId
ID gruppo azioni che deve essere notificato.
...
### -ActionRuleType
Formato Json della regola di azione
...
Formato previsto: { \<operation\> : \<comma separated list of values\> } ad esempio.
Contiene:smartgroup
...
Formato previsto: { \<operation\> : \<comma separated list of values\> } ad esempio.
Uguale a:/subscriptions/ad825170-845c-47db-8f00-11978947b089/resourceGroups/abvarma/providers/microsoft.insights/metricAlerts/test-mrmc-vm-abvarma
...
### -Descrizione
Descrizione della regola di azione
...
Formato previsto: { \<operation\> : \<comma separated list of values\> } per esempio.
Contiene:Avviso di test
...
Formato previsto: { \<operation\> : \<comma separated list of values\> } ad esempio.
Uguale a:Piattaforma,Log Analytics
...
### -ReccurenceType
Specifica la durata in cui deve essere applicata l'eliminazione.
...
### -ReccurentValue
Valori ricorrenza, se applicabile. Nel caso di Settimanale - \[ 1,2 Nel caso \] di Mensile - \[ 1,3,5,30\]
...
### -SuppressionEndTime
Ora di fine eliminazione.
Formato 09/12/2018 06:00:00 +Deve essere menzionato in caso di Pianificazione soppressione ricorrente - Una volta, Ogni giorno, Ogni settimana o Ogni mese.
...
Ora di inizio della sospensione.
Formato 09/12/2018 06:00:00 +Deve essere menzionato in caso di Pianificazione soppressione ricorrente - Una volta, Ogni giorno, Ogni settimana o Ogni mese.
...
Formato previsto: { \<operation\> : \<comma separated list of values\> } ad esempio.
Contiene:Macchine virtuali,Account di archiviazione
