...
### Esempio 1: Elencare tutti i set di dati di riferimento nell'ambiente specificato
```powershell
...
### Esempio 2: Ottenere un set di dati di riferimento specificato in base al nome
```powershell
...
### Esempio 3: Ottenere un set di dati di riferimento specificato in base all'oggetto
```powershell
...
### -Name
Nome del set di dati di riferimento Approfondimenti serie temporale associato all'ambiente specificato.
...
INPUTOBJECT <ITimeSeriesInsightsIdentity> : parametro Identity
  ...
  - `[SubscriptionId <String>]`: ID sottoscrizione Azure.
