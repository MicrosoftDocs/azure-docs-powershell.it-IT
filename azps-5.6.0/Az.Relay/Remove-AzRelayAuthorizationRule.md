...
## DESCRIZIONE
Il cmdlet **Remove-AzRelayAuthorizationRule** rimuove la regola di autorizzazione delle entità di inoltro fornite (Spazio dei nomi/WcfRelay/HybridConnection).
...
Rimuove la regola di autorizzazione `AuthoRule1` dello spazio dei `TestNameSpace-Relay1` nomi.
...
Rimuove la regola di `AuthoRule1` autorizzazione di WcfRelay `TestWcfRelay` dallo spazio dei `TestNameSpace-Relay1` nomi.
...
Rimuove la regola di `AuthoRule1` autorizzazione di HybridConnection `TestHybridConnection` dallo spazio dei `TestNameSpace-Relay1` nomi.
