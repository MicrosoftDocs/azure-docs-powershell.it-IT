...
### Esempio 4: Distribuire un modello archiviato in un account di archiviazione non pubblico usando un URI e un token di firma di accesso condiviso
```powershell
PS C:\> New-AzResourceGroupDeployment -ResourceGroupName "RGName" -TemplateUri "https://example.com/example.json" -QueryString "foo"
```

Questo comando crea una nuova distribuzione usando il modello in TemplateUri che non è pubblico e richiede un parametro token per accedere a quello che verrebbe fornito con il parametro QueryString.
