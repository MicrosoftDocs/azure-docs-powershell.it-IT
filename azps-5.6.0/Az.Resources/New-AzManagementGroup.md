...
Creazione di un nuovo gruppo con `DisplayName` e `ParentId` impostato su `null` . Il tenant sarà uguale a quello del gruppo e il padre del `DisplayName` `GroupName` gruppo sarà il tenant.  
...
In questo caso, l'elemento padre del gruppo sarà il tenant e sarà `DisplayName` impostato sul valore assegnato.

### Esempio 3: Creare un gruppo di gestione con un padre e un nome visualizzato
```
...
### Esempio 4: Creare un gruppo di gestione con un oggetto padre (usando un oggetto padre)
```
