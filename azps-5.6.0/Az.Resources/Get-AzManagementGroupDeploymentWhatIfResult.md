...
## SYNOPSIS
Ottiene un modello che What-If risultato per una distribuzione nell'ambito del gruppo di gestione. 
...
## DESCRIZIONE
Il cmdlet **Get-AzManagementGroupDeploymentWhatIfResult** ottiene il risultato ARM modello What-If per la distribuzione di modelli nell'ambito del gruppo di gestione specificato. Restituisce un elenco di modifiche che indicano quali risorse verranno aggiornate se la distribuzione viene applicata senza apportare modifiche alle risorse reali. Per specificare il formato per il risultato restituito, usare il *parametro ResultFormat.*

## ESEMPI

### Esempio 1: Ottenere un risultato What-If'ambito del gruppo di gestione
```powershell
...
Questo comando ottiene un risultato What-If'ambito del gruppo di gestione usando un file di modello personalizzato e un file di parametri sul disco.
...
Il comando usa il *parametro TemplateParameterFile* per specificare un file con parametri di modello.
Il comando usa il *parametro ResultFormat* per impostare What-If risultato in modo da includere payload delle risorse completi.

### Esempio 2: Ottenere un risultato What-If'ambito del gruppo di gestione con ResourceIdOnly
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
Il comando usa il *parametro TemplateFile* per specificare un file modello.
Il comando usa il *parametro TemplateParameterFile* per specificare un file con parametri di modello.
Il comando usa il *parametro ResultFormat* per impostare What-If risultato in modo che contenga solo ID risorsa.
...
### -ExcludeChangeType
Elenco separato da virgole dei tipi di modifica delle risorse da escludere What-If risorse.
...
### -Name
Il nome della distribuzione che creerà.
Se non è specificato, quando viene fornito un file modello viene utilizzato il nome predefinito del file modello; per impostazione predefinita, quando viene fornito un oggetto modello, ad esempio "20131223140835".
