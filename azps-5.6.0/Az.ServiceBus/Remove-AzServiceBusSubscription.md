...
### SubscriptionPropertiesSet (impostazione predefinita)
```
...
## DESCRIZIONE
Il cmdlet **Remove-AzServiceBusSubscription** rimuove la sottoscrizione a un argomento dallo spazio dei nomi del bus di servizio specificato.
...
Rimuove la sottoscrizione `SB-TopicSubscription-Example1` all'argomento `SB-Topic_exampl1` nello spazio dei nomi del bus di servizio `SB-Example1` specificato.
...
### Esempio 4: ResourceId - Uso della variabile:
```powershell
...
Rimuove la sottoscrizione specificata tramite l'ID ARM nel parametro $resourceid/string per -ResourceId 
...
### -InputObject
Oggetto sottoscrizione bus di servizio
...
### -ResourceId
ID risorsa sottoscrizione bus di servizio
