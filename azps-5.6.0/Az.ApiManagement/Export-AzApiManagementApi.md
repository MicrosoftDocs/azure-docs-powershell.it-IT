...
## DESCRIZIONE
Il cmdlet **Export-AzApiManagementApi** esporta un'API di gestione DELLE API di Azure in un file in uno dei formati supportati.

## ESEMPI

### Esempio 1: Esportare un'API nel formato RTFL (Web Application Description Language)
```powershell
...
### Esempio 2: Esportare un'API nel formato delle specifiche OpenApi 3.0 come documento Json
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Questo comando esporta una definizione di API nel formato Open Api come documento Json
...
### -ApiRevision
Identificatore della revisione API. Questo parametro è facoltativo. Se non viene specificato, l'esportazione verrà eseguita per la revisione dell'api attualmente attiva.
...
### -PassThru
Indica che questa operazione restituisce $True se l'API viene esportata correttamente o se $False in caso contrario.
...
psdx_paramvalues Wadl, Wsdl, Swagger, OpenApi e OpenApiJson
