...
## DESCRIZIONE
Il cmdlet **Set-AzApiManagementApiRevision** modifica una revisione dell'API Di gestione API di Azure.

## ESEMPI

### Esempio 1: Modificare una revisione dell'API
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Set-AzApiManagementApiRevision -Context $ApiMgmtContext -ApiId "echo-api" -ApiRevision "2" -Name "EchoApi" -ServiceUrl "https://contoso.com/apis/echo" -Protocols @('https') -Description "Responds with what was sent" -Path "echo"
```

Il cmdlet aggiorna la `2` revisione dell'API `echo-api` con una nuova descrizione, protocollo e percorso.
...
Questo parametro è facoltativo.
Il valore predefinito è $null.
Deve essere specificato se AuthorizationScope è specificato.
...
### -Name
Nome dell'API Web.
Nome pubblico dell'API così come verrebbe visualizzato nei portali di sviluppo e amministrazione.
Questo parametro è obbligatorio.
...
### -OpenIdProviderId
Identificatore del server di autorizzazione OpenId. Questo parametro è facoltativo. Il valore predefinito è $null. Deve essere specificato se si specifica BearerTokenSendingMethods.
...
### -PassThru
Se si specifica l'istanza di Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementApi che rappresenta l'API impostata.
...
Questo URL verrà usato dagli utenti api per l'invio di richieste al servizio Web.
...
Questo URL verrà usato solo da Gestione API di Azure e non verrà reso pubblico.
Deve contenere da 1 a 2000 caratteri.
Questo parametro è obbligatorio.
...
Nome dell'intestazione della chiave di abbonamento.
Questo parametro è facoltativo.
Il valore predefinito è $null.
...
Nome del parametro della stringa di query con chiave di sottoscrizione.
Questo parametro è facoltativo.
Il valore predefinito è $null.
...
Contrassegno per applicare SubscriptionRequired per le richieste all'Api. Questo parametro è facoltativo.
