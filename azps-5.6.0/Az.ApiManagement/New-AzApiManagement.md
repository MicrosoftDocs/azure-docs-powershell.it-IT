...
## DESCRIZIONE
Il cmdlet **New-AzApiManagement crea** una distribuzione di Gestione API in Gestione API di Azure.

## ESEMPI

### Esempio 1: Creare un servizio di gestione API di livello sviluppatore
```powershell
...
Questo comando crea un servizio di gestione API di livello sviluppatore.
Il comando specifica l'organizzazione e l'indirizzo dell'amministratore.
Il comando non specifica il *parametro SKU.*
Di conseguenza, il cmdlet usa il valore predefinito di Developer.
...
Questo comando crea un servizio di gestione API di livello Standard con tre unità.

### Esempio 3: Creare un servizio di livello Consumo
```powershell
...
Questo comando crea un servizio di gestione API di livello a consumo con certificato client abilitato nell'Europa occidentale.

### Esempio 4: Creare un servizio di gestione DELLE API per una rete virtuale esterna
```powershell
PS C:\> $virtualNetwork = New-AzApiManagementVirtualNetwork -Location "West US" -SubnetResourceId "/subscriptions/a8ff56dc-3bc7-4174-b1e8-3726ab15d0e2/resourceGroups/ContosoGroup/providers/Microsoft.Network/virtualNetworks/westUsVirtualNetwork/subnets/backendSubnet"
PS C:\> New-AzApiManagement -ResourceGroupName "ContosoGroup" -Location "West US" -Name "ContosoApi" -Organization Contoso -AdminEmail admin@contoso.com -VirtualNetwork $virtualNetwork -VpnType "External" -Sku "Premium"
```

Questo comando crea un servizio di gestione API di livello Premium in una subnet di rete virtuale di Azure con un endpoint gateway rivolto all'esterno con un'area master negli Stati Uniti occidentali.

### Esempio 5: Creare un servizio di gestione DELLE API per una rete virtuale interna
```powershell
PS C:\> $virtualNetwork = New-AzApiManagementVirtualNetwork -Location "West US" -SubnetResourceId "/subscriptions/a8ff56dc-3bc7-4174-b1e8-3726ab15d0e2/resourceGroups/ContosoGroup/providers/Microsoft.Network/virtualNetworks/westUsVirtualNetwork/subnets/backendSubnet"
PS C:\> New-AzApiManagement -ResourceGroupName "ContosoGroup" -Location "West US" -Name "ContosoApi" -Organization "Contoso" -AdminEmail "admin@contoso.com" -VirtualNetwork $virtualNetwork -VpnType "Internal" -Sku "Premium"
```

Questo comando crea un servizio di gestione API a livello superiore in una subnet di rete virtuale di Azure con un endpoint gateway per uso interno con un'area master negli Stati Uniti occidentali.

### Esempio 6: Creare un servizio di gestione DELLE API e abilitare il protocollo TLS 1.0
```powershell
...
Questo comando crea un servizio di gestione api SKU standard e abilita TLS 1.0 nel client Frontend al gateway ApiManagement e al client Back-end tra gateway ApiManagement e back-end.

## PARAMETERS

### -AdditionalRegions
Altre aree di distribuzione di Gestione API di Azure.
...
### -AdminEmail
Specifica l'indirizzo di posta elettronica di origine per tutte le notifiche inviate dal sistema di gestione DELLE API.
...
### -Capacità
Specifica la capacità di SKU del servizio di gestione delle API di Azure.
...
Configurazioni di nomi host personalizzati. Il valore predefinito è $null. Passando $null, viene impostato il nome host predefinito.
...
### -EnableClientCertificate
Contrassegno da usare solo per il servizio ApiManagement di SKU di consumo. In questo modo viene applicato un certificato client da presentare a ogni richiesta al gateway. In questo modo è anche possibile autenticare il certificato nei criteri del gateway.
...
### -Location
Specifica la posizione in cui creare il servizio di gestione api.
...
### -Name
Specifica un nome per la distribuzione di Gestione API.
...
- Sviluppatore 
- Standard 
- Premium L'impostazione predefinita è Sviluppatore.
...
Certificati emessi dalla CA interna per l'installazione nel servizio. Il valore predefinito è $null.
...
### -VirtualNetwork
Configurazione di rete virtuale dell'area di distribuzione principale di Gestione API Azure.
...
### -VpnType
Tipo di rete virtuale della distribuzione ApiManagement. I valori validi sono 
- "Nessuno" (valore predefinito. ApiManagement non fa parte di alcuna rete virtuale")
- "Esterno" (la distribuzione di ApiManagement viene impostata all'interno di una rete virtuale con un endpoint internet)
- "Interno" (la distribuzione di ApiManagement viene impostata all'interno di una rete virtuale che ha un endpoint rivolto alla Intranet)
