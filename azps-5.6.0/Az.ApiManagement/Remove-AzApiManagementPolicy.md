...
## DESCRIZIONE
Il cmdlet **Remove-AzApiManagementPolicy** rimuove il criterio di gestione delle API dall'ambito specificato.
...
### Esempio 2: Rimuovere i criteri di ambito del prodotto
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ProductId "0123456789"
```

Questo comando rimuove i criteri di ambito del prodotto da Gestione API.

### Esempio 3: Rimuovere il criterio di ambito API
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210"
```

Questo comando rimuove i criteri di ambito API da Gestione API.

### Esempio 4: Rimuovere il criterio di ambito delle operazioni
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210" -OperationId "777"
```

Questo comando rimuove il criterio di ambito dell'operazione da Gestione API.

## PARAMETERS

### -ApiId
Specifica l'identificatore di un'API esistente.
Se si specifica questo parametro, il cmdlet rimuove il criterio di ambito API.
...
Se si specifica questo parametro con il parametro *ApiId,* questo cmdlet rimuove i criteri di ambito dell'operazione.
...
Se si specifica questo parametro, il cmdlet rimuove i criteri di ambito del prodotto.
