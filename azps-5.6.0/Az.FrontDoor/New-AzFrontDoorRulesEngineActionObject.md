...
Usare il cmdlet "New-AzFrontDoorHeaderActionObject" per creare PSHeaderObjects da passare ai parametri "-RequestHeaderActions" e "-ResponseHeaderActions".
...
Creare un'azione del motore di regole e mostrare come visualizzare le proprietà dell'azione del motore di regole creata.

## PARAMETERS

### -BackendPoolName
Nome del BackendPool a cui la regola si instrada
...
### -CustomForwardingPath
Percorso personalizzato usato per riscrivere i percorsi delle risorse corrispondenti a questa regola.
Lasciare vuoto per usare il percorso in arrivo.
...
Frammento è la parte dell'URL che viene dopo #.
Non includere il valore di errore #.
...
Lasciare vuoto per usare l'host della posta in arrivo come host di destinazione.
...
### -CustomPath
Percorso completo da reindirizzare.
Il percorso non può essere vuoto e deve iniziare con /.
Lasciare vuoto per usare il percorso in ingresso come percorso di destinazione.
...
### -CustomQueryString
Set di stringhe di query da inserire nell'URL di reindirizzamento.
L'impostazione di questo valore sostituirebbe qualsiasi stringa di query esistente; lasciare vuota per mantenere la stringa di query in ingresso.
La stringa di query deve essere nel \<key\> = \<value\> formato.
Il primo?
e & verranno aggiunte automaticamente, quindi non includerle nella parte frontale, ma separare più stringhe di query con &.
...
### -DynamicCompression
Se abilitare la compressione dinamica per il contenuto memorizzato nella cache.
...
### -EnableCaching
Se abilitare la memorizzazione nella cache per questa route.
...
### -ForwardingProtocol
Protocollo che verrà utilizzato da questa regola per l'inoltro del traffico ai back-end.
Il valore predefinito è MatchRequest
...
### -FrontDoorName
Nome della porta a cui appartiene questa regola di routing.
...
Il trattamento dei termini di query URL durante la creazione della chiave cache.
...
### -RedirectType
Tipo di reindirizzamento che la regola userà per il reindirizzamento del traffico.
Il valore predefinito viene spostato
...
### -RequestHeaderAction
Elenco delle azioni di intestazione da applicare dalla richiesta da AFD all'origine.
...
### -ResourceGroupName
Nome del gruppo di risorse con cui verrà creata la regola di routing.
...
Elenco di azioni di intestazione da applicare dalla risposta da AFD al client.
...
### Microsoft.Azure.Commands.FrontDoor.Models.PSRulesAction
