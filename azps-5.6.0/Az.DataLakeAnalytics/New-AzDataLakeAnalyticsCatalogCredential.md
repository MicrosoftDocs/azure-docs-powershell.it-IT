...
### CreateByHostNameAndPort (impostazione predefinita)
```
...
## DESCRIZIONE
Il cmdlet New-AzDataLakeAnalyticsCatalogCredential crea una nuova credenziali da usare in un catalogo di Azure Data Lake Analytics per la connessione a origini dati esterne.

## ESEMPI

### Esempio 1: Creare le credenziali per un catalogo specificando host e porta
```
...
Questo comando crea le credenziali specificate per l'account, il database, l'host e la porta specificati usando il protocollo https.

### Esempio 2: Creare le credenziali per un catalogo che specifica l'URI completo
```
PS C:\> New-AzDataLakeAnalyticsCatalogCredential -AccountName "ContosoAdlAccount" `
                  -DatabaseName "databaseName" `
                  -CredentialName "exampleDbCred" `
...
Questo comando crea le credenziali specificate per l'URI dell'account, del database e dell'origine dati esterna specificati.
...
### -Credential
Specifica il nome utente e la password corrispondente delle credenziali.
...
### -CredentialName
Specifica il nome e la password delle credenziali.
...
### -DatabaseName
Specifica il nome del database nell'account Data Lake Analytics in cui verranno archiviate le credenziali.
...
### -Port
Specifica il numero di porta usato per connettersi a DatabaseHost specificato con queste credenziali.
