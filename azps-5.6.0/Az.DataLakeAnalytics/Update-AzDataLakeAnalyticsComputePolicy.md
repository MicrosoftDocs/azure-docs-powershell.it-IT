...
## DESCRIZIONE
**Update-AzDataLakeAnalyticsComputePolicy** aggiorna la regola dei criteri di calcolo specificata per una specifica entità AAD in un account Azure Data Lake Analytics.
...
Questo comando aggiorna un criterio denominato "myPolicy" nell'account "contosoadla" per garantire che l'utente non possa inviare alcun processo con più di 5 unità di analisi.

### Esempio 2: Creare un criterio di calcolo con entrambe le regole aggiornate
```
...
Questo comando crea un criterio denominato "myPolicy" nell'account "contosoadla" per garantire che l'utente non possa inviare alcun processo con più di 5 unità di analisi o con una priorità inferiore a 100
...
Il numero massimo di unità di analisi supportate per ogni processo per questo criterio. È necessario specificare questo parametro, MinPriorityPerJob o entrambi i parametri.
...
### -MinPriorityPerJob
Priorità minima supportata per ogni processo per questo criterio. In entrambi i casi, è necessario specificare MaxAnalyticsUnitsPerJob o entrambi i parametri.
