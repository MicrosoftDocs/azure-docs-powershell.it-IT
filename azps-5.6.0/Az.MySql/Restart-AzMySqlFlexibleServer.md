...
### Esempio 1: Riavviare il server in base al nome della risorsa
```powershell
PS C:\> Restart-AzMySqlFlexibleServer -ResourceGroupName PowershellMySqlTest -Name mysql-test
```

Riavviare il server in base al nome

### Esempio 2: Riavviare il server in base all'identità
```powershell
...
Riavviare il server in base all'identità
