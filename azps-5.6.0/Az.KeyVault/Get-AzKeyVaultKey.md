...
Questo comando recupera tutte le versioni della chiave ITPfx nella chiave vault denominata Contoso.

### Esempio 4: Ottenere una versione specifica di un codice
```powershell
...
Questo comando ottiene il test della chiave3 eliminato in precedenza, ma non ripulito, nella chiave vault denominata Contoso.
...
### Esempio 8: Scaricare una chiave pubblica come file PEM
...
Questo è un passaggio per l'importazione delle chiavi protette da HSM nel Vault delle chiavi di Azure. Vedi https://docs.microsoft.com/azure/key-vault/keys/hsm-protected-keys
...
### -OutFile
Specifica il file di output per cui questo cmdlet salva la chiave. Per impostazione predefinita, la chiave pubblica viene salvata in formato PEM.
