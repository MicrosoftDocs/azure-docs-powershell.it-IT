...
## DESCRIZIONE
Il cmdlet **Restore-AzKeyVaultCertificate** crea un certificato nella chiave vault specificata da un file di backup.
...
Se il vault della chiave contiene già un certificato con lo stesso nome, questo cmdlet non riesce invece di sovrascrivere il certificato originale.
Se il backup contiene più versioni di un certificato, vengono ripristinate tutte le versioni.
Il vault delle chiavi in cui si ripristina il certificato può essere diverso da quello da cui è stato eseguito il backup del certificato.
...
### Esempio 1: Ripristinare un certificato di cui è stato eseguito il backup
```powershell
...
Questo comando ripristina un certificato, incluse tutte le relative versioni, dal file di backup denominato Backup.BLOB nella chiave vault denominata MyKeyVault.
