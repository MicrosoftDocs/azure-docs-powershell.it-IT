...
## DESCRIZIONE
Il **comando Update-AzKeyVaultNetworkRuleSet** aggiorna le regole di rete in vigore nel vault delle chiavi specificato. 
...
Questo comando aggiorna il set di regole di rete nel vault denominato "myVault" per l'intervallo IP specificato e per la rete virtuale, ignorando la regola di rete per i servizi di Azure.

### Esempio 2

Aggiorna il set di regole di rete in un vault delle chiavi. (autogenerati)
...
### -Bypass
Specifica il bypass della regola di rete.
...
Se questa opzione è specificata, restituisce l'oggetto vault della chiave aggiornato.
...
### -ResourceGroupName
Specifica il nome del gruppo di risorse associato alla chiave vault la cui regola di rete viene modificata.
...
### -VaultName
Specifica il nome di un vault delle chiavi la cui regola di rete viene modificata.
...
Specifica l'identificatore di risorsa di rete virtuale consentito della regola di rete.
