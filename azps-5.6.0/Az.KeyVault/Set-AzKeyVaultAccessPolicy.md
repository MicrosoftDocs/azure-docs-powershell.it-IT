...
Se si registra un'applicazione in una directory diversa da quella predefinita, non è possibile autorizzare l'applicazione a usare il vault delle chiavi.
...
Il secondo comando modifica le autorizzazioni concesse nel primo comando per consentire ora di ottenere informazioni segrete oltre a PattiFuller@contoso.com impostarle ed eliminarle. Le autorizzazioni per le operazioni chiave rimangono invariate dopo questo comando.
...
Usare quindi l'output di questo comando con Set-AzKeyVaultAccessPolicy per concedere le autorizzazioni a group2 per il vault della chiave, **denominato myownvault.** Questo esempio enumera i gruppi denominati "gruppo2" in linea nella stessa riga di comando.
...
Consente a Gestione risorse di Azure di ottenere informazioni segrete da questo vault chiave quando si fa riferimento a questa chiave vault in una distribuzione di modelli.
