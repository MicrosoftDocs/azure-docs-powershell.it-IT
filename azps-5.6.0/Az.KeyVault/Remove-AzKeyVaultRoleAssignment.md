...
## DESCRIZIONE
Usare il `Remove-AzKeyVaultRoleAssignment` cmdlet per revocare l'accesso a qualsiasi entità in base all'ambito e al ruolo assegnato. Oggetto dell'assegnazione, ad esempio l'entità DEVE essere specificata. L'entità può essere un utente (usare i parametri SignInName o ObjectId per identificare un utente), un gruppo di sicurezza (usare il parametro ObjectId per identificare un gruppo) o un'entità servizio (usare parametri ApplicationId o ObjectId per identificare un ServicePrincipal. Il ruolo a cui è assegnata l'entità DEVE essere specificato usando il parametro RoleDefinitionName o RoleDefinitionId.
...
Questo esempio revoca il ruolo di "Amministratore criteri HSM gestito" di user1@microsoft.com " " nell'ambito "/keys".
...
Questo esempio revoca tutti i ruoli di " user1@microsoft.com " in tutti gli ambiti.
...
### -PassThru
Restituisce true quando viene ripristinato HSM.
