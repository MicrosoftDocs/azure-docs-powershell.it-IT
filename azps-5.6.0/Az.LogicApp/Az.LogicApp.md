---
Module Name: Az.LogicApp
Module Guid: 8a7d44bb-e747-4265-8f22-b3937961d709
Download Help Link: https://docs.microsoft.com/powershell/module/az.logicapp
Help Version: 4.0.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
ms.openlocfilehash: 712fcbfb377c1df446d5c633c07692652374bfdd
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 03/04/2021
ms.locfileid: "101995467"
---
# Modulo Az.LogicApp
## Descrizione
Questo argomento visualizza gli argomenti della Guida per i cmdlet di App per la logica.

## Cmdlet Az.LogicApp
### [Get-AzIntegrationAccount](Get-AzIntegrationAccount.md)
Recupera gli account di integrazione.

### [Get-AzIntegrationAccountAgreement](Get-AzIntegrationAccountAgreement.md)
Ottiene un contratto di account di integrazione.

### [Get-AzIntegrationAccountAccountAccount](Get-AzIntegrationAccountAssembly.md)
Ottiene un assembly di account di integrazione.

### [Get-AzIntegrationAccountBatchConfiguration](Get-AzIntegrationAccountBatchConfiguration.md)
Recupera una configurazione batch dell'account di integrazione.

### [Get-AzIntegrationAccountUrl](Get-AzIntegrationAccountCallbackUrl.md)
Ottiene un URL di url personale dell'account di integrazione.

### [Get-AzIntegrationAccountCertificate](Get-AzIntegrationAccountCertificate.md)
Recupera certificati dell'account di integrazione da un gruppo di risorse.

### [Get-AzIntegrationAccountGeneratedIcn](Get-AzIntegrationAccountGeneratedIcn.md)
Questo cmdlet recupera il valore corrente del numero di controllo interscambio generato in base al contratto.

### [Get-AzIntegrationAccountMap](Get-AzIntegrationAccountMap.md)
Ottiene una mappa dell'account di integrazione.

### [Get-AzIntegrationAccountPartner](Get-AzIntegrationAccountPartner.md)
Ottiene partner dell'account di integrazione.

### [Get-AzIntegrationAccountReceivedIcn](Get-AzIntegrationAccountReceivedIcn.md)
Questo cmdlet recupera un numero di controllo interscambio ricevuto specifico per contratto e valore di numero di controllo.

### [Get-AzIntegrationAccountSchema](Get-AzIntegrationAccountSchema.md)
Recupera gli schemi dell'account di integrazione.

### [Get-AzLogicApp](Get-AzLogicApp.md)
Ottiene un'app per la logica da un gruppo di risorse.

### [Get-AzLogicAppRunAction](Get-AzLogicAppRunAction.md)
Recupera un'azione da un'app per la logica.

### [Get-AzLogicAppRunHistory](Get-AzLogicAppRunHistory.md)
Ottiene la cronologia di esecuzione di un'app per la logica.

### [Get-AzLogicAppTrigger](Get-AzLogicAppTrigger.md)
Ottiene i trigger di un'app per la logica.

### [Get-AzLogicAppTriggerUrl](Get-AzLogicAppTriggerCallbackUrl.md)
Ottiene l'URL trigger trigger per l'app per la logica.

### [Get-AzLogicAppTriggerHistory](Get-AzLogicAppTriggerHistory.md)
Recupera la cronologia dei trigger in un'app per la logica.

### [Get-AzLogicAppUpgradedDefinition](Get-AzLogicAppUpgradedDefinition.md)
Ottiene la definizione aggiornata per un'app per la logica.

### [New-AzIntegrationAccount](New-AzIntegrationAccount.md)
Crea un account di integrazione.

### [New-AzIntegrationAccountAgreement](New-AzIntegrationAccountAgreement.md)
Crea un contratto di account di integrazione.

### [New-AzIntegrationAccountAccountAccount](New-AzIntegrationAccountAssembly.md)
Crea un assembly di account di integrazione.

### [New-AzIntegrationAccountBatchConfiguration](New-AzIntegrationAccountBatchConfiguration.md)
Crea una configurazione batch dell'account di integrazione.

### [New-AzIntegrationAccountCertificate](New-AzIntegrationAccountCertificate.md)
Crea un certificato dell'account di integrazione.

### [New-AzIntegrationAccountMap](New-AzIntegrationAccountMap.md)
Crea una mappa dell'account di integrazione.

### [New-AzIntegrationAccountPartner](New-AzIntegrationAccountPartner.md)
Crea un partner account di integrazione.

### [New-AzIntegrationAccountSchema](New-AzIntegrationAccountSchema.md)
Crea uno schema dell'account di integrazione.

### [New-AzLogicApp](New-AzLogicApp.md)
Crea un'app per la logica in un gruppo di risorse.

### [Remove-AzIntegrationAccount](Remove-AzIntegrationAccount.md)
Rimuove un account di integrazione.

### [Remove-AzIntegrationAccountAgreement](Remove-AzIntegrationAccountAgreement.md)
Rimuove il contratto di un account di integrazione.

### [Remove-AzIntegrationAccountAccountAccount](Remove-AzIntegrationAccountAssembly.md)
Rimuove un assembly di account di integrazione.

### [Remove-AzIntegrationAccountBatchConfiguration](Remove-AzIntegrationAccountBatchConfiguration.md)
Rimuove una configurazione batch dell'account di integrazione.

### [Remove-AzIntegrationAccountCertificate](Remove-AzIntegrationAccountCertificate.md)
Rimuove un certificato dell'account di integrazione da un gruppo di risorse.

### [Remove-AzIntegrationAccountMap](Remove-AzIntegrationAccountMap.md)
Rimuove una mappa dell'account di integrazione.

### [Remove-AzIntegrationAccountPartner](Remove-AzIntegrationAccountPartner.md)
Rimuove un partner dell'account di integrazione.

### [Remove-AzIntegrationAccountReceivedIcn](Remove-AzIntegrationAccountReceivedIcn.md)
Questo cmdlet rimuove un numero di controllo interscambio ricevuto specifico per contratto e valore di numero di controllo.

### [Remove-AzIntegrationAccountSchema](Remove-AzIntegrationAccountSchema.md)
Rimuove uno schema dell'account di integrazione.

### [Remove-AzLogicApp](Remove-AzLogicApp.md)
Rimuove un'app per la logica da un gruppo di risorse.

### [Set-AzIntegrationAccount](Set-AzIntegrationAccount.md)
Modifica un account di integrazione.

### [Set-AzIntegrationAccountAgreement](Set-AzIntegrationAccountAgreement.md)
Modifica un contratto di account di integrazione.

### [Set-AzIntegrationAccountAccountAccount](Set-AzIntegrationAccountAssembly.md)
Modifica un assembly di account di integrazione.

### [Set-AzIntegrationAccountBatchConfiguration](Set-AzIntegrationAccountBatchConfiguration.md)
Modifica una configurazione batch dell'account di integrazione.

### [Set-AzIntegrationAccountCertificate](Set-AzIntegrationAccountCertificate.md)
Modifica un certificato dell'account di integrazione.

### [Set-AzIntegrationAccountGeneratedIcn](Set-AzIntegrationAccountGeneratedIcn.md)
Aggiorna il numero di controllo interscambio generato dall'account di integrazione nel gruppo di risorse di Azure.

### [Set-AzIntegrationAccountMap](Set-AzIntegrationAccountMap.md)
Modifica una mappa dell'account di integrazione.

### [Set-AzIntegrationAccountPartner](Set-AzIntegrationAccountPartner.md)
Modifica un partner dell'account di integrazione.

### [Set-AzIntegrationAccountReceivedIcn](Set-AzIntegrationAccountReceivedIcn.md)
Aggiorna l'account di integrazione ricevuto come numero di controllo interscambio (ICN) nel gruppo di risorse di Azure.

### [Set-AzIntegrationAccountSchema](Set-AzIntegrationAccountSchema.md)
Modifica uno schema dell'account di integrazione.

### [Set-AzLogicApp](Set-AzLogicApp.md)
Modifica un'app per la logica in un gruppo di risorse.

### [Start-AzLogicApp](Start-AzLogicApp.md)
Esegue un'app per la logica in un gruppo di risorse.

### [Stop-AzLogicAppRun](Stop-AzLogicAppRun.md)
Annulla un'esecuzione di un'app per la logica.

### [Test-AzLogicApp](Test-AzLogicApp.md)
Convalida una definizione dell'app per la logica.

