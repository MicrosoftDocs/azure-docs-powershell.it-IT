...
## DESCRIZIONE
Il cmdlet Set-AzIntegrationAccountGeneratedIcn aggiorna un account di integrazione esistente che ha ricevuto il numero di controllo di interscambio (ICN) e restituisce un oggetto che rappresenta il numero di controllo dell'interscambio ricevuto dell'account di integrazione.
...
È possibile aggiornare il numero di controllo interscambio ricevuto di un account di integrazione specificando il nome dell'account di integrazione, il nome del gruppo di risorse, il nome del contratto, il valore del numero di controllo e lo stato di elaborazione dei messaggi.
Con questo comando non è possibile creare un nuovo account di integrazione ricevuto tramite questo comando.
...
Questo comando aggiorna il numero di controllo interscambio Edifact ricevuto dall'account di integrazione per uno specifico contratto per l'account di integrazione e il valore con lo stato di elaborazione dei messaggi non riuscito.
