...
## DESCRIZIONE
Questo cmdlet è destinato a essere usato in scenari di ripristino di emergenza per rimuovere dall'account di integrazione un numero di controllo interscambio ricevuto, in modo che il connettore B2B possa elaborare nuovamente il messaggio quando è abilitato il rilevamento dei numeri duplicati.
In rari casi, il numero di controllo dell'interscambio ricevuto potrebbe essere riservato poco prima di un'emergenza e prima che il connettore B2B rifiuti l'interscambio in modo erre.
In questi casi, l'operazione potrebbe voler consentire al sito di ripristino di elaborare di nuovo lo stesso interscambio dopo la correzione del payload.
...
Tenta di ottenere un numero di controllo interscambio X12 ricevuto che il contenuto non è in un formato valido.
Rimuove il numero di controllo interscambio X12 ricevuto.
Conferma che il numero di controllo interscambio X12 ricevuto è stato rimosso provando a ottenerlo di nuovo.
...
Cerca di ottenere un numero di controllo interscambio Edifact ricevuto che il contenuto non è in un formato valido.
Rimuove il numero di controllo interscambio Edifact ricevuto.
Conferma che il numero di controllo interscambio Edifact ricevuto è stato rimosso provando a ottenerlo di nuovo.
