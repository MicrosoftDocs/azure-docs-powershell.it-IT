...
## DESCRIZIONE
LNew-AzHDInsightCluster crea un cluster HDInsight di Azure usando i parametri specificati o usando un oggetto di configurazione creato con il cmdlet New-AzHDInsightClusterConfig.
...
### Esempio 2: Creare un cluster con crittografia disco della chiave gestita dal cliente
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### Esempio 4: Creare un cluster Azure HDInsight con funzionalità di collegamento privato e in uscita di inoltro
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### Esempio 6: Creare un cluster Azure HDInsight che abilita la scalabilità automatica.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### Esempio 7: Creare un cluster Azure HDInsight con proxy Rest di Kafka.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### Esempio 8: Creare un cluster Azure HDInsight con archiviazione di Azure Data Lake Gen2.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### Esempio 9: Creare un cluster Azure HDInsight con il pacchetto di sicurezza enterprise (ESP) e abilitare il broker ID HDInsight.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### -AadTenantId
Specifica l'ID tenant di Azure AD che verrà usato quando si accede ad Azure Data Lake Store.
...
Specifica gli altri account di Archiviazione di Azure per il cluster.
In alternativa, è possibile usare Add-AzHDInsightStorage cmdlet.
...
### -AmbariDatabase
Ottiene o imposta il database per gli ambari.
...
### -ApplicationId
Ottiene o imposta l'ID applicazione entità servizio per l'accesso ad Azure Data Lake.
...
Ottiene o imposta la configurazione di gradazioni automatica
...
### -CertificateFileContents
Specifica il contenuto del file del certificato che verrà usato quando si accede ad Azure Data Lake Store.
...
Il cluster userà questa impostazione per l'accesso ad Azure Data Lake Store.
...
### -ClusterTier
Specifica il livello cluster HDInsight.
Per impostazione predefinita, questo campo è Standard.
Il livello Premium può essere usato solo con cluster Linux e consente l'uso di alcune nuove funzionalità.
...
Ottiene o imposta la SKU host dedicata per l'isolamento del calcolo.
...
Questo oggetto può essere creato con il cmdlet New-AzHDInsightClusterConfig.
...
In alternativa, è possibile usare Add-AzHDInsightConfigValues cmdlet.
...
### -EdgeNodeSize
Specifica le dimensioni della macchina virtuale per il nodo Edge. Usa Get-AzVMSize per dimensioni accettabili di VM e consulta la pagina dei prezzi di HDInsight. Questo parametro è valido solo per i cluster RServer.
...
### -EnableComputeIsolation
Abilita la funzionalità di isolamento dei calcoli HDInsight.
...
### -EnableIDBroker
Abilita la funzionalità del broker di identità HDInsight.
...
### -EncryptionVaultUri
Ottiene o imposta l'URI del vault di crittografia.
...
Usa Get-AzVMSize per dimensioni accettabili di VM e consulta la pagina dei prezzi di HDInsight.
...
### -HiveMetastore
Specifica il database SQL in cui memorizzare i metadati Hive.
In alternativa, è possibile usare Add-AzHDInsightMetastore cmdlet.
...
### -KafkaClientGroupId
Ottiene o imposta l'ID gruppo client per l'accesso proxy Kafka Rest.
...
Ottiene o imposta il nome del gruppo client per l'accesso proxy Kafka Rest.
...
Ottiene o imposta le dimensioni del nodo di gestione di Kafka.
...
### -Location
Specifica la posizione del cluster.
...
Ottiene o imposta la versione minima di TLS supportata.
...
### -ObjectId
Specifica l'ID oggetto di Azure AD (un GUID) dell'entità servizio di Azure AD che rappresenta il cluster.
Il cluster userà questa impostazione per l'accesso ad Azure Data Lake Store.
...
### -OozieMetastore
Specifica l'SQL database in cui archiviare i metadati di Oozie.
In alternativa, è possibile usare Add-AzHDInsightMetastore cmdlet.
...
### -RdpCredential
Specifica le credenziali di Desktop remoto (RDP) per il cluster.
Questo problema si applica solo ai cluster di Windows.
...
### -ScriptActions
Specifica le azioni script da eseguire sul cluster al termine della creazione del cluster.
...
In alternativa, è possibile usare Add-AzHDInsightSecurityProfile cmdlet.
...
Questo problema si applica solo ai cluster Linux.
...
### -StorageAccountKey
Ottiene o imposta la chiave di accesso dell'account di archiviazione per l'account di archiviazione.
...
Ottiene o imposta l'ID della risorsa di archiviazione per l'account di archiviazione.
...
### -StorageContainer
Ottiene o imposta il nome StorageContainer per l'account di archiviazione di Azure predefinito
...
### -StorageFileSystem
Ottiene o imposta il file system per l'account predefinito Azure Data Lake Storage Gen2.
...
### -StorageRootPath
Ottiene o imposta il percorso alla radice del cluster nell'account predefinito Data Lake Store.
...
### -WorkerNodeSize
Specifica le dimensioni della macchina virtuale per il nodo Worker.
Usa Get-AzVMSize per dimensioni accettabili di VM e consulta la pagina dei prezzi di HDInsight.
...
### -MetriakeeperNodeSize
Specifica le dimensioni della macchina virtuale per il nodo Guardiano.
Usa Get-AzVMSize per dimensioni accettabili di VM e consulta la pagina dei prezzi di HDInsight.
Questo parametro è valido solo per cluster HBase o Storm.
...
## NOTE
Parole chiave: azure, azurerm, arm, risorsa, gestione, manager, hadoop, hdinsight, hd, insight
