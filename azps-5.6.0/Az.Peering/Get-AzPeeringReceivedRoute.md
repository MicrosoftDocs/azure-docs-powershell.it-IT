...
## DESCRIZIONE
Elenca le route ricevute da un peering

## ESEMPI

### Elencare le prime 100 route ricevute per un peering
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName
```

Elenca tutte le route ricevute per un peering

### Filtrare in base al percorso AS
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -AsPath "1234 5674 9834"
```

Elenca tutte le route ricevute per un peering con un filtro su AS 

### Filtrare in base a RPKIValidationState
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -RPKIValidationState "Valid"
```

Elenca tutte le route ricevute per un peering con un filtro in RPKIValidState

### Filtrare in base a OriginAsValidationState
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -OriginAsValidationState "Valid"
```

Elenca tutte le route ricevute per un peering con un filtro in OriginAsValidState

## PARAMETERS

### -AsPath
Filtrare in base al percorso AS.
Esempio: '9342 1234 4567'
...
Filtrare in base allo stato di convalida ORIGIN AS.
...
### -RPKIValidState
Filtrare in base allo stato di convalida RPKI.
...
### Microsoft.Azure.PowerShell.Cmdlets.Peering.Models.PSApplicationingReceivedRoute
