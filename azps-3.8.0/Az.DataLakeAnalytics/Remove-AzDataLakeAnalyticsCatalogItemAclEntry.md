...
### RemoveCatalogAclEntryForUser (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **Remove-AzDataLakeAnalyticsCatalogItemAclEntry** consente di rimuovere una voce (ACE) dall'elenco di controllo di accesso (ACL) di un catalogo o un elemento del catalogo in data Lake Analytics.

## ESEMPI

### Esempio 1: rimuovere l'ACL dell'utente per un catalogo
```powershell
...
Questo comando rimuove l'ACL del catalogo per Patti Fuller dell'account di contosoadla.

### Esempio 2: rimuovere l'ACL dell'utente per un database
```powershell
...
Questo comando rimuove l'ACL del database per Patti Fuller dell'account di contosoadla.
...
### -Gruppo
Rimuovere l'immissione di ACL del catalogo per il gruppo.
...
### -ObjectId
Identità dell'utente da rimuovere.
...
### -Utente
Rimuovere l'immissione di ACL del catalogo per l'utente.
