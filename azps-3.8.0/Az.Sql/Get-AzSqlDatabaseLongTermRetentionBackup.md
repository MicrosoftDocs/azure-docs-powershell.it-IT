...
### Posizione (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **Get-AzSqlDatabaseLongTermRetentionBackup** ottiene tutti i backup di conservazione a lungo termine per una posizione, un server o un database o ottiene un backup di conservazione a lungo termine specifico.

## ESEMPI

### Esempio 1: ottenere tutti i backup per una posizione
```powershell
...
Questo comando ottiene tutti i backup di conservazione a lungo termine per tutti i database (che potrebbero essere vivi o eliminati) in northeurope, il gruppo di risorse verrà impostato solo se il server è attivo.
...
Questo comando ottiene tutti i backup di conservazione a lungo termine per tutti i database (che potrebbero essere vivi o eliminati) in un gruppo di risorse in northeurope.
...
Questo comando ottiene il backup con il nome 601061b7-D10B-46e0-bf77-a2bfb16a6add; 131655666550000000
...
### Esempio 5: ottenere backup di conservazione a lungo termine tramite filtro
```powershell
...
Questo comando ottiene tutti i backup con il nome che inizia con "601061b7"
...
### -DatabaseState
Lo stato del database di cui si vogliono trovare i backup, attivo, eliminato o tutti.
Impostazione predefinita per tutti
...
### -InputObject
Oggetto database per cui ottenere i backup.
...
Se ottenere o meno solo il backup più recente per ogni database.
...
### -ResourceId
ID risorsa database per cui ottenere i backup.
...
### -Nomeserver
Nome di Azure SQL Server in cui sono inclusi i backup.
