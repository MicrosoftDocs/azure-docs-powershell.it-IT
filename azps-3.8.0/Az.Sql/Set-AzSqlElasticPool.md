...
## Descrizione
Il cmdlet **set-AzSqlElasticPool** imposta le proprietà per un pool elastico in Azure SQL database. Questo cmdlet può modificare il eDTUs per pool ( *DTU* ), la dimensione massima dello spazio di archiviazione per pool ( *StorageMB* ), il eDTUs massimo per database ( *DatabaseDtuMax* ) e il minimo eDTUs per database ( *DatabaseDtuMin* ).
