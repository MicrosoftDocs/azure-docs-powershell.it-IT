...
## Descrizione
Il cmdlet **New-AzSqlServer** crea un server di database SQL di Azure.

## ESEMPI

### Esempio 1: creare un nuovo server di database SQL di Azure
```
...
Questo comando crea un server di database SQL di una versione 12 Azure.
...
### -Posizione
Specifica la posizione del centro dati in cui questo cmdlet crea il server.
...
### -ResourceGroupName
Specifica il nome del gruppo di risorse a cui questo cmdlet assegna il server.
...
Specificare 2,0 per creare un server versione 11 o 12,0 per creare un server versione 12.
...
Specifica le credenziali di amministratore del server di database SQL per il nuovo server. Per ottenere un oggetto **PSCredential** , usa il cmdlet Get-Credential. Per altre informazioni, digitare `Get-Help
Get-Credential` .
