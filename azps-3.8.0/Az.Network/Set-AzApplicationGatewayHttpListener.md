...
## Descrizione
Il cmdlet **set-AzApplicationGatewayHttpListener** modifica un listener HTTP per un gateway dell'applicazione Azure.
...
Il secondo comando imposta il listener HTTP per il gateway per l'uso della configurazione front-end archiviata in $FIP 01 con il protocollo HTTP sulla porta 80.

### Esempio 2: aggiungere un listener HTTPS con SSL e nomi host
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
### -ApplicationGateway
Specifica il gateway dell'applicazione con cui questo cmdlet associa il listener HTTP.
...
Specifica l'indirizzo IP front-end del gateway dell'applicazione.
...
Specifica l'ID dell'indirizzo IP front-end del gateway dell'applicazione.
...
### -FrontendPort
Specifica la porta di front-end gateway dell'applicazione.
...
### -HostName
Specifica il nome host a cui questo cmdlet invia il listener HTTP.
...
Specifica se il cmdlet richiede l'indicazione del nome del server.
I valori accettabili per questo parametro sono: vero o falso.
...
### -SslCertificateId
Specifica l'ID del certificato SSL (Secure Socket Layer) del listener HTTP.
