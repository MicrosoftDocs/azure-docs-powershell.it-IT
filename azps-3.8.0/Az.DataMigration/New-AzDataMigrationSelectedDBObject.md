...
## Descrizione
Il cmdlet New-AzDataMigrationSelectedDB crea un oggetto info database che contiene informazioni sui database di origine e di destinazione, oltre ai mapping delle tabelle, per la migrazione. Questo cmdlet può essere usato come parametro con il cmdlet New-AzDataMigrationTask.
...
### -BackupFileShare
Condivisione file in cui è necessario eseguire il backup dei file di database del server di origine per il database.
Usare questa impostazione per ignorare le informazioni sulla condivisione di file per ogni database.
...
Impostare il tipo di migrazione su SQL Server per la migrazione a SQL DB.
...
### -MigrateSqlServerSqlDbMi
Impostare il tipo di migrazione su SQL Server su SQL DB MI Migration.
