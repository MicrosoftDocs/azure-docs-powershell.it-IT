...
### Esempio 2: creare (caricare) un file di data Lake da un file di origine locale e il cmdlet viene eseguito in background
```
...
Questo comando Crea (caricare) un file di data Lake da un file di origine locale e il cmdlet viene eseguito in background.
...
### -ConcurrentTaskCount
La quantità totale di attività asincrone simultanee. Il valore predefinito è 10.
...
### -Force
Se passato, viene creato un nuovo elemento senza alcuna richiesta
...
### -Path
Il percorso nel filesystem specificato che deve essere creato.
...
### -Property
Specifica le proprietà per la directory o il file creato. Le proprietà supportate per file sono: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5, ContentType.
...
### -Origine
Specifica il percorso del file di origine locale che verrà caricato in un file datalake Gen2.
...
### -Umask
Quando si crea un nuovo elemento e la directory padre non ha un ACL predefinito, umask limita le autorizzazioni del file o della directory da creare.
L'autorizzazione risultante viene fornita da p & ^ u, dove p è l'autorizzazione e si è umask.
Il simbolo (rwxrw-RW-) è supportato.
