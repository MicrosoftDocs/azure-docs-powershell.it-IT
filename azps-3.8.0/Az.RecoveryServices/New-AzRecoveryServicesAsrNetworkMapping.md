...
Avvia il mapping di rete per l'operazione di creazione usando il nome specificato, le reti primarie e di ripristino e restituisce un processo ASR per tenere traccia dell'operazione (scenario di Azure in Azure).

## PARAMETRI

### -AzureToAzure
Contrassegna per creare uno scenario AzureToAzure.
...
Specifica l'ID rete virtuale Azure della rete principale per il mapping.
...
### -PrimaryFabric
Specifica il tessuto ASR in cui deve essere creato il mapping.
...
### -RecoveryFabric
Oggetto tessuto di recupero del sito di Azure che corrisponde all'area di ripristino di Azure.
...
### -RecoveryNetwork
Specifica l'oggetto rete di ripristino per il mapping di rete ASR.
