...
### FileEventTriggerParameterSet (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **New-AzDataBoxEdgeTrigger** configura un trigger nel dispositivo Edge della casella dati. 
...
### -FileEvent
Passare questo parametro switch per configurare il trigger fileEvent
...
### -PeriodicTimerEvent
Passare questo parametro switch per configurare il trigger PeriodicTimerEvent
...
### -RoleName
Ruolo di calcolo in base a quali eventi verranno generati.
...
### -Programmazione
Frequenza periodica a cui deve essere generato l'evento timer. Specificare una programmazione in giorni (compreso tra 1 e 365), ore (compreso tra 1 e 23) o minuti (compreso tra 1 e 59).
...
### -ShareId
ID condivisione file da passare nel trigger fileEvent
...
### -StartTime
L'ora del giorno che genera un trigger valido. La programmazione viene calcolata con il riferimento all'ora specificata fino a secondi. Se il fuso orario non è specificato, l'ora verrà considerata in Device TimeZone. Il valore verrà sempre restituito come ora UTC.
...
### -Topic
Argomento in cui gli eventi periodici vengono pubblicati nel dispositivo degli anni.
