...
Creare un disco da una versione di immagine della raccolta condivisa.  ID è l'ID della versione dell'immagine della raccolta condivisa. Lun è necessario solo se l'origine è un disco dati.
...
### -DiskAccessId
Ottiene o imposta l'ID ARM della risorsa DiskAccess per l'uso di endpoint privati.
...
### -DiskIOPSReadOnly
Numero totale di IOPS che saranno consentiti in tutte le VM che montano il disco condiviso come ReadOnly. Un'operazione può essere trasferita tra 4K e 256K byte.
...
### -DiskMBpsReadOnly
"Descrizione": "il throughput totale (MBps) che sarà consentito in tutte le VM che monta il disco condiviso come ReadOnly. MBps significa milioni di byte al secondo MB Usa la notazione ISO, di poteri di 10.
...
Oggetto GalleryImageReference.  Obbligatorio per la creazione da un'immagine della raccolta. L'ID sarà l'ID ARM della versione condivisa dell'immagine della cambusa da cui creare un disco.
È necessario un lun se l'origine della copia è uno dei dischi di dati nell'immagine della raccolta; Se null, il disco del sistema operativo dell'immagine verrà copiato.
...
### -LogicalSectorSize
Dimensioni del settore logico in byte per dischi Ultra. 
...
### -MaxSharesCount
Numero massimo di VM che possono essere allegate al disco contemporaneamente.
...
I valori possibili includono: "AllowAll", "AllowPrivate", "valore denyall"
