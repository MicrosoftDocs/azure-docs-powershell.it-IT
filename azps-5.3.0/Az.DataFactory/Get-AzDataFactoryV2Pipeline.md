...
## Descrizione
Il cmdlet Get-AzDataFactoryV2Pipeline ottiene informazioni sulle pipeline in Azure Data Factory.
...
Puoi usare uno dei comandi di esempio seguenti.
...
### Esempio 4: ottenere informazioni sugli input per la prima attività
```powershell
...
Questo comando ottiene le informazioni per la pipeline denominata DPWikisample nella Factory di dati denominata WikiADF e quindi usa la notazione a punti standard per visualizzare la proprietà attività associata alla pipeline.
Il comando Visualizza la proprietà inputs del primo elemento della matrice Activities usando il cmdlet Format-List.
