...
## Sinossi
Imposta impostazioni avanzate per la protezione delle minacce in un pool SQL.
...
## Descrizione
Il cmdlet **Update-AzSynapseSqlPoolAdvancedThreatProtectionSetting** imposta le impostazioni avanzate per la protezione delle minacce in un pool SQL di analisi delle sinapsi di Azure.
Per abilitare la protezione avanzata dalle minacce in un pool SQL, è necessario abilitare le impostazioni di controllo nel pool SQL.
...
Questo comando imposta le impostazioni avanzate di protezione dalle minacce per un pool SQL denominato ContosoSqlPool sotto l'area di lavoro denominata ContosoWorkspace.
...
### Microsoft. Azure. Commands. sinapsi. Models. PSSqlPoolSecurityAlertPolicy
