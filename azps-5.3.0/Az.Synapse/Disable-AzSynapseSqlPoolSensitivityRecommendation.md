...
## Sinossi
Disabilita (respinge) i consigli di sensitività sulle colonne nel pool SQL.
...
## Descrizione
Il cmdlet Disable-AzSynapseSqlPoolSensitivityRecommendation Disabilita le raccomandazioni di sensitivity per le colonne del pool SQL.

## ESEMPI

### Esempio 1: disabilitare i consigli di sensitività per una determinata colonna in un pool di SQL sinapsi di Azure.
```powershell
...
### Esempio 2: disabilitare i consigli di sensitività per le colonne che hanno suggerimenti per la sensibilità in un pool di SQL sinapsi di Azure con piping.
```powershell
...
### Esempio 3: disabilitare i consigli di sensitività per una determinata colonna in un pool di SQL sinapsi di Azure tramite piping.
```powershell
