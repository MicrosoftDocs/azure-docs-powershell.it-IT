...
## Sinossi
Abilita suggerimenti per la sensitività sulle colonne (le raccomandazioni sono abilitate per impostazione predefinita in tutte le colonne) nel pool SQL.
...
## Descrizione
Il cmdlet Enable-AzSynapseSqlPoolSensitivityRecommendation consente di abilitare i consigli di sensitività per le colonne nel pool SQL.

## ESEMPI

### Esempio 1: abilitare i consigli di sensitività per una determinata colonna in un pool di SQL sinapsi di Azure.
```powershell
...
### Esempio 2: abilitare i consigli di sensitività in un pool di SQL sinapsi di colonna Azure con piping.
```powershell
