...
## Sinossi
Creare una nuova configurazione del controllo del codice sorgente Kubernetes.
...
### Esempio 1: creare un configuation per kubernetes cluster
```powershell
...
Questo comando crea un configuation per il cluster kubernetes.
...
### -ClusterScoped
Se passato imposta l'ambito della configurazione su cluster (il valore predefinito è nameSpace).
...
### -EnableHelmOperator
Opzione per abilitare l'operatore Helm per questa configurazione git.
...
Override dei valori per il grafico Helm dell'operatore.
...
Versione del grafico timone operatore.
...
### -OperatorNamespace
Lo spazio dei nomi a cui è installato l'operatore.
Massimo di 253 caratteri alfanumerici, segno meno e punto solo.
...
### -OperatorParameters
Tutti i parametri per l'istanza dell'operatore in formato stringa.
