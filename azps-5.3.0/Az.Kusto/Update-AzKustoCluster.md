...
### Esempio 1: aggiornare un cluster esistente per nome
```powershell
...
Il comando precedente aggiorna l'USK del cluster kusto "testnewkustocluster" disponibile nel gruppo di risorse "testrg".

### Esempio 2: aggiornare un cluster esistente per nome
```powershell
...
Valore booleano che indica se è abilitata la crittografia doppia.
...
Valore booleano che indica se l'acquisizione del flusso è abilitata.
...
Il tipo "None" rimuoverà tutte le identità.
...
I riferimenti chiave del dizionario delle identità utente saranno ID delle risorse ARM nella forma:'/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
La versione della chiave di volta chiave.
...
Per costruire, vedere la sezione Note per le proprietà di LANGUAGEEXTENSIONVALUE e creare una tabella hash.
...
Valore booleano che indica se la caratteristica di scalabilità ottimizzata è abilitata o meno.
...
Conteggio istanze minime consentite.
...
Per costruire, vedere la sezione Note per le proprietà di TRUSTEDEXTERNALTENANT e creare una tabella hash.
...
ID risorsa indirizzo IP pubblico del servizio di gestione dati.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension [] >: l'elenco delle estensioni della lingua.
  - `[Name <LanguageExtensionName?>]`: Nome dell'estensione della lingua.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant [] >: i tenant esterni del cluster.
