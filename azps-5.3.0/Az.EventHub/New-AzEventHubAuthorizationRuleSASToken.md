...
## Sinossi
Genera un token SAS per la regola di autorizzazione di Azure eventhub dello spazio dei nomi/eventhub. 
...
## Descrizione
Il cmdlet New-AzEventHubAuthorizationRuleSASToken genera un token SAS (Shared Access Signature) per un file Azure Eventhub o un Eventhub di Azure
...
Genera il token SAS per la regola authorixation specificata per lo spazio dei nomi con l'ora di inizio e di scadenza.
...
Genera il token SAS per la regola authorixation specificata per lo spazio dei nomi con tempo di scadenza.

## PARAMETRI

### -AuthorizationRuleId
ARM ResourceId della regola Authoraization
