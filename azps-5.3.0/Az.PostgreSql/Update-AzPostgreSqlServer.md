...
### Esempio 1: aggiornare il server PostgreSql per il gruppo di risorse e il nome del server
```powershell
...
### Esempio 2: aggiornare il server PostgreSql per identità.
```powershell
...
Questo cmdlet aggiorna il server PostgreSql per identità.

## PARAMETRI

### -AdministratorLoginPassword
La password dell'account di accesso dell'amministratore.
...
### -SkuCapacity
La capacità di scala su/fuori, che rappresenta le unità di calcolo del server.
...
### -SkuTier
Il livello del particolare SKU, ad esempio Basic.
