...
Creare un Vault chiave e specificare le regole di rete per consentire l'accesso all'indirizzo IP specificato dalla rete virtuale identificata da $myNetworkResId. `New-AzKeyVaultNetworkRuleSetObject`Per altre informazioni, vedere.
...
Se specificato, consente di autorizzare le azioni di dati tramite il controllo di accesso basato sui ruoli (RBAC) e quindi i criteri di accesso specificati nelle proprietà del Vault verranno ignorati. Tieni presente che le azioni di gestione sono sempre autorizzate con RBAC.
...
### -Posizione
Specifica l'area di Azure in cui creare il Vault chiave. Usare il comando [Get-AzLocation](https://docs.microsoft.com/powershell/module/Azure/Get-AzLocation) per visualizzare le opzioni desiderate.
...
### -NetworkRuleSet
Specifica il set di regole di rete del Vault. Regola l'accessibilità del Vault chiave da posizioni di rete specifiche. Creato da `New-AzKeyVaultNetworkRuleSetObject` .
...
Specifica il periodo di conservazione delle risorse eliminate e la durata della cancellazione di un Vault o di un oggetto nello stato eliminato. Il valore predefinito è 90 giorni.
