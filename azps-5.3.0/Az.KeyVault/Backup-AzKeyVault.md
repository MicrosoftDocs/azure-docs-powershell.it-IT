...
## Descrizione
Eseguire il backup completo di un HSM gestito in un account di archiviazione.
Consente `Restore-AzKeyVault` di ripristinare il backup.
...
Il cmdlet creerà una cartella (in genere denominata `mhsm-{name}-{timestamp}` ) nel contenitore di archiviazione, archivierà il backup nella cartella e restituirà l'URI della cartella.
...
### -SasToken
Token SAS (Shared Access Signature) per autenticare l'account di archiviazione.
...
Nome del contenitore di BLOB in cui verrà archiviato il backup.
