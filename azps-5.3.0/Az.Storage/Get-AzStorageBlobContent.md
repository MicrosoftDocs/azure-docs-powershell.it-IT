...
## Descrizione
Il cmdlet **Get-AzStorageBlobContent** Scarica il BLOB di archiviazione specificato.
Se il nome del BLOB non è valido per il computer locale, questo cmdlet lo risolve automaticamente, se possibile.

## ESEMPI

### Esempio 1: scaricare il contenuto BLOB per nome
```
PS C:\>Get-AzStorageBlobContent -Container "ContainerName" -Blob "Blob" -Destination "C:\test\"
```

Questo comando Scarica un BLOB per nome.

### Esempio 2: scaricare il contenuto BLOB usando la pipeline
```
PS C:\>Get-AzStorageBlob -Container containername -Blob blobname | Get-AzStorageBlobContent
```

Questo comando usa la pipeline per trovare e scaricare contenuto BLOB.

### Esempio 3: scaricare il contenuto BLOB usando la pipeline e un carattere jolly
```
PS C:\>Get-AzStorageContainer container* | Get-AzStorageBlobContent -Blob "cbox.exe" -Destination "C:\test"
```

Questo esempio usa il carattere jolly asterisco e la pipeline per trovare e scaricare contenuto BLOB.

### Esempio 4: ottenere un oggetto BLOB e salvarlo in una variabile, quindi scaricare il contenuto BLOB con l'oggetto BLOB
```
PS C:\>$blob = Get-AzStorageBlob -Container containername -Blob blobname 
PS C:\>Get-AzStorageBlobContent -CloudBlob $blob.ICloudBlob -Destination "C:\test"
```

Questo esempio ottiene prima di tutto un oggetto BLOB e lo salva in una variabile, quindi Scarica il contenuto BLOB con l'oggetto BLOB. 
...
### -BLOB
Specifica il nome del BLOB da scaricare.
...
Puoi crearlo o usare il cmdlet Get-AzStorageContainer.
...
### -Contenitore
Specifica il nome del contenitore con il BLOB che si vuole scaricare.
...
### -Contesto
Specifica l'account di archiviazione di Azure da cui si vuole scaricare il contenuto BLOB.
Puoi usare il cmdlet New-AzStorageContext per creare un contesto di archiviazione.
...
### -Force
Sovrascrive un file esistente senza conferma.
...
## Note
* Se il nome del BLOB non è valido per il computer locale, questo cmdlet la risolve in autonomia, se possibile.
