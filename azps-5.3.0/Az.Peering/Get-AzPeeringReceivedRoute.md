...
### Filtrare in base al percorso
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -AsPath "1234 5674 9834"
```

Elenca tutte le route ricevute per un peering con un filtro attivato come 

### Filtrare per RPKIValidationState
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -RPKIValidationState "Valid"
```

Elenca tutte le route ricevute per un peering con un filtro in RPKIValidationState

### Filtrare per OriginAsValidationState
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -OriginAsValidationState "Valid"
```

Elenca tutte le route ricevute per un peering con un filtro in OriginAsValidationState
...
Filtrare in base all'origine come stato di convalida.
...
### -RPKIValidationState
Filtrare in base allo stato di convalida di RPKI.
