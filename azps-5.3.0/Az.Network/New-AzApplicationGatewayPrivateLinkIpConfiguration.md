...
## Sinossi
Crea una configurazione IP da associare alla configurazione di PrivateLink
...
## Descrizione
Il cmdlet **New-AzApplicationGatewayPrivateLinkIpConfiguration** crea una configurazione IP da associare alla configurazione di PrivateLink per un gateway dell'applicazione Azure.

## ESEMPI

### Esempio 1: configurazione IP di PrivateLink
```powershell
PS C:\> $PrivateLinkIpConfiguration = New-AzApplicationGatewayPrivateLinkIpConfiguration -Name "ipConfig01" -Subnet $subnet -Primary
```

Questo comando crea una configurazione IP PrivateLink denominata "ipConfig01" archivia il risultato nella variabile denominata $PrivateLinkIpConfiguration. 
...
### -Primaria
Indicare che la configurazione IP è primaria.
...
### -PrivateIpAddress
Indirizzo IP privato di ipConfiguration se si vuole assegnare l'allocazione statica.
