...
## Sinossi
Restituisce una configurazione dell'indirizzo backend del bilanciamento del carico. 

## SINTASSI

### SetByResourcePublicIpAddress (impostazione predefinita)
```
...
### Esempio 1: nuova configurazione dell'indirizzo LoadBalancer con riferimento alla rete virtuale
```powershell
PS C:\> $virtualNetwork = Get-AzVirtualNetwork -Name $vnetName -ResourceGroupName $resourceGroup
New-AzLoadBalancerBackendAddressConfig -IpAddress "10.0.0.5" -Name "TestVNetRef" -VirtualNetworkId $virtualNetwork.Id
```

### Esempio 2: nuova configurazione degli indirizzi di LoadBalancer con riferimento per le configurazioni IP di LoadBalancer frontend
```powershell
...
### -IpAddress
IPAddress da aggiungere al pool back-end
...
Configurazione IP dell'interfaccia di bilanciamento del carico associata all'indirizzo di backend config
...
### -Nome
Nome della configurazione dell'indirizzo back-end
...
### -VirtualNetworkId
Rete virtuale associata alla configurazione dell'indirizzo backend
