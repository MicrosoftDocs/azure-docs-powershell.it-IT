...
### Esempio 1: creare un ambiente per gli approfondimenti delle serie temporali di Gen1
```powershell
...
Questo comando crea un ambiente per gli approfondimenti delle serie temporali di Gen1.

### Esempio 2: creare un ambiente per gli approfondimenti delle serie temporali di Gen2
```powershell
...
Questo comando crea un ambiente per gli approfondimenti delle serie temporali di Gen2.
...
### -Capacità
Capacità dell'USK.
Per gli ambienti Gen1, questo valore può essere modificato per supportare la scalabilità fuori dagli ambienti dopo la creazione.
...
Elenco delle proprietà dell'evento che verranno usate per partizionare i dati nell'ambiente.
Per costruire, vedere la sezione Note per le proprietà di PARTITIONKEYPROPERTY e creare una tabella hash.
...
### -ResourceGroupName
Nome di un gruppo di risorse Azure.
...
### -StorageAccountKey
Il valore della chiave di gestione che concede l'accesso in scrittura del servizio Insights serie temporali all'account di archiviazione.
...
### -StorageAccountName
Nome dell'account di archiviazione che conterrà i dati a lungo termine dell'ambiente.
...
Comportamento che il servizio approfondimenti della serie temporale deve eseguire quando la capacità dell'ambiente è stata superata
...
L'elenco delle proprietà dell'evento che verranno usate per definire l'ID della serie temporali dell'ambiente. Per costruire, vedere la sezione Note per le proprietà di TIMESERIESIDPROPERTY e creare una tabella hash.
...
ISO8601 TimeSpan che specifica il numero di giorni in cui gli eventi dell'ambiente saranno disponibili per la query dall'archivio caldo.
...
PARTITIONKEYPROPERTY <ITimeSeriesIdProperty [] >: elenco delle proprietà dell'evento che verranno usate per partizionare i dati nell'ambiente.
  - `[Name <String>]`: Nome della proprietà.
  - `[Type <PropertyType?>]`: Il tipo della proprietà.

TIMESERIESIDPROPERTY <ITimeSeriesIdProperty [] >: l'elenco delle proprietà dell'evento che verranno usate per definire l'ID della serie temporale dell'ambiente.
  - `[Name <String>]`: Nome della proprietà.
  - `[Type <PropertyType?>]`: Il tipo della proprietà.
