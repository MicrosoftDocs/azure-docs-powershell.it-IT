...
## Descrizione
Il cmdlet **Add-AzureRmDnsRecordConfig** aggiunge un record DNS (Domain Name System) a un oggetto **Recordset** .
L'oggetto **Recordset** è un oggetto offline e le modifiche apportate non modificano le risposte DNS finché non viene eseguito il cmdlet Set-AzureRmDnsRecordSet per rendere persistente la modifica al servizio DNS di Microsoft Azure.
...
Dato che un set di record CNAME può contenere al massimo un record, deve essere inizialmente un set di record vuoto o i record esistenti devono essere rimossi usando Remove-AzureRmDnsRecordConfig.
...
Si tratta di una rappresentazione offline del **Recordset** e le modifiche apportate non modificano le risposte DNS finché non si esegue il cmdlet Set-AzureRmDnsRecordSet.
