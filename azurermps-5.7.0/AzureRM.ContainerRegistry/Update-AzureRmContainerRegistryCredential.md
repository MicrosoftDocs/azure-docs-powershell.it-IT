...
## Descrizione
Il cmdlet Update-AzureRmContainerRegistryCredential rigenera una credenziale di accesso per un registro di sistema contenitore.

## ESEMPI

### Esempio 1: rigenerare una credenziale di accesso per un registro di sistema contenitore
```powershell
...
Questo comando rigenera una credenziale di accesso per il registro di sistema contenitore specificato.
L'utente di amministratore deve essere abilitato per il registro \` di sistema Registry \` per rigenerare le credenziali di accesso.
