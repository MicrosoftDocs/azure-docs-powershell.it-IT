...
## Descrizione
Il cmdlet **New-AzureRmApiManagementAuthorizationServer** crea un server di autorizzazione Gestione API di Azure.
...
### -ClientID
Specifica l'ID client della console per sviluppatori che rappresenta l'applicazione client.
...
### -ClientSecret
Specifica il segreto client della console per sviluppatori che rappresenta l'applicazione client.
...
Devi specificare che questo parametro è obbligatorio se ResourceOwnerPassword è specificato dal parametro *GrantTypes* .
...
Devi specificare questo parametro se ResourceOwnerPassword è specificato dal parametro *GrantTypes* .
...
### -SupportState
Indica se supportare il parametro di *stato* .
...
### -TokenBodyParameters
Specifica parametri body aggiuntivi usando il formato **Application/x-www-form-urlencoded** .
...
### -TokenEndpointUrl
Specifica l'URL dell'endpoint del token usato dai client per ottenere i token di accesso in Exchange per presentare le concessioni di autorizzazione o aggiornare i token.
