...
## Descrizione
Il cmdlet **set-AzureRmDataLakeStoreAccount** modifica un account di data Lake Store.

## ESEMPI

### Esempio 1: aggiungere un contrassegno a un account
```
PS C:\>Set-AzureRmDataLakeStoreAccount -Name "ContosoADL" -Tags @{"stage"="production"}
```

Questo comando aggiunge il tag specificato all'account di data Lake Store denominato ContosoADL.
...
### -DefaultGroup
Specifica l'ID di un gruppo di directory AzureActive.
...
### -Versione
Se il tipo di crittografia è assegnato dall'utente, l'utente può ruotare la propria versione chiave con questo parametro.
...
### -Nome
Specifica il nome di un account di data Lake Store.
...
### -ResourceGroupName
Specifica il nome del gruppo di risorse che contiene l'account di data Lake Store da modificare.
...
Abilitare o disabilitare facoltativamente i provider di ID attendibili esistenti.
