...
## Sinossi
Aggiornare un percorso di backup.
...
### Esempio 1: impostare la configurazione di backup
```powershell
...
### -Backup
Informazioni sul percorso di backup.
Per costruire, vedere la sezione Note per le proprietà di BACKUP e creare una tabella hash.
...
Periodo di conservazione, in giorni, per gli schienali nel percorso di archiviazione.
...
### -EncryptionCertPath
Percorso del file CERT di crittografia con chiave pubblica (con estensione CER).
...
True se l'utilità di pianificazione di backup è abilitata.
...
### -Password
Password per accedere alla posizione.
...
### -Path
Percorso della posizione di aggiornamento
...
### -UserName
Nome utente per accedere alla posizione.
...
BACKUP <IBackupLocation> : informazioni sul percorso di backup.
  ...
  - `[BackupFrequencyInHours <Int32?>]`: Intervallo, in ore, per la frequenza con cui l'utilità di pianificazione accetta un backup.
  - `[BackupRetentionPeriodInDays <Int32?>]`: Periodo di conservazione, in giorni, per gli schienali nel percorso di archiviazione.
  - `[EncryptionCertBase64 <String>]`: Dati non elaborati Base64 per il certificato di crittografia di backup.
  - `[IsBackupSchedulerEnabled <Boolean?>]`: True se l'utilità di pianificazione backup è abilitata.
  - `[Password <String>]`: Password per accedere alla posizione.
  - `[Path <String>]`: Percorso della posizione di aggiornamento
  - `[UserName <String>]`: Nomeutente per accedere alla posizione.
