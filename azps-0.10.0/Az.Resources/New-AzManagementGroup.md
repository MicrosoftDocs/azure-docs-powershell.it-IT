...
## Descrizione
Il cmdlet **New-AzManagementGroup** crea un gruppo di gestione.
...
Creazione di un nuovo gruppo con `DisplayName` e `ParentId` impostato su `null` . L'oggetto `DisplayName` sarà uguale a `GroupName` e il padre del gruppo sarà il tenant.  

### Esempio 2: creare un gruppo di gestione con un nome visualizzato
```
...
In questo caso, l'elemento padre del gruppo sarà il tenant e l' `DisplayName` impostazione verrà impostata sul valore assegnato.

### Esempio 3: creare un gruppo di gestione con un elemento padre e un nome visualizzato
```
...
### Esempio 4: creare un gruppo di gestione con un elemento padre (tramite un oggetto padre)
```
