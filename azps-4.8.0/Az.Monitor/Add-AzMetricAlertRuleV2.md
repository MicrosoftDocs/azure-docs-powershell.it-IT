...
## Sinossi
Aggiunge o aggiorna una regola di avviso basata su metriche V2 (non classica).
...
## Descrizione
Aggiunge o aggiorna una **regola di avviso basata su metriche V2 (non classica)**. La regola aggiunta è associata a un gruppo di risorse e ha un nome. Questo cmdlet implementa il modello ShouldProcess, vale a dire che potrebbe richiedere conferma all'utente prima di creare, modificare o rimuovere la risorsa.
...
Questo comando crea una regola di avviso metrica per una macchina virtuale. $condition è l'output di New-AzMetricAlertRuleV2Criteria cmdlet e $act è l'output del cmdlet New-AzActionGroup
...
Questo comando crea una regola di avviso metrica per tutte le macchine virtuali nell'abbonamento che si trovano in eastus
...
Questo comando Disabilita una regola di avviso metrica. In questo caso, siamo in uscita piping di Get-AzMetricAlertRuleV2 Add-AzMetricAlertRuleV2 

### Esempio 4: aggiungere una regola di avviso metrica con le dimensioni
...
Per creare una regola di avviso metrica più complessa, ad esempio quelle che prevedono la selezione di valori di dimensione o che hanno più criteri, è possibile usare i cmdlet Helper New-AzMetricAlertRuleV2DimensionSelection e New-AzMetricAlertRuleV2Criteria.

Sopra set di cmdlet verrà creata una regola di avviso metrica con le dimensioni.
...
### -ActionGroupId
ID del gruppo di azioni per la regola
...
### -DisableRule
Il contrassegno Disabilita regola (stato)
...
### -Frequenza
Frequenza di valutazione per la regola
