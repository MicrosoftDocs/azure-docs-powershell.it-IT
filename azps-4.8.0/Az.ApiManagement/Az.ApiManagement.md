...
## Descrizione
In questo argomento vengono visualizzati gli argomenti della Guida per i cmdlet di gestione API di Azure.
...
Eseguire il backup di un servizio di gestione API.
...
Ottiene un segreto client del server di autorizzazione Gestione API.
...
Ottiene i certificati di gestione delle API configurati per l'autenticazione reciproca con backend nel servizio.
...
Ottiene tutto o un gateway di gestione API specifico.

### [Get-AzApiManagementGatewayHostnameConfiguration](Get-AzApiManagementGatewayHostnameConfiguration.md)
Ottiene la configurazione di un nome host tutto o specifico per il gateway esistente.

### [Get-AzApiManagementGatewayKey](Get-AzApiManagementGatewayKey.md)
Ottiene le chiavi del gateway esistente
...
Ottiene un valore segreto del particolare valore denominato.
...
Ottiene il segreto client OpenID Connect provider.
...
Ottiene le chiavi di abbonamento.
...
Ottiene le chiavi di configurazione di Access per un tenant.
...
Ottiene le chiavi di configurazione dell'accesso GIT per un tenant.
...
Crea un nuovo contratto di credenziale back-end.
...
Crea una nuova diagnostica nell'ambito globale o dell'ambito API.
...
Crea un'istanza di **PsApiManagementHttpMessageDiagnostic** che è un'impostazione di diagnostica del messaggio http della diagnostica
...
Creare impostazioni di diagnostica per i messaggi HTTP in arrivo/in uscita nel gateway.

### [New-AzApiManagementProduct](New-AzApiManagementProduct.md)
Crea un prodotto di gestione delle API.
...
Creare un nuovo contratto per la posizione delle risorse (usato nei gateway).
...
Crea un'istanza di `PsApiManagementSystemCertificate` . Il certificato può essere emesso da una CA privata e verrà installato nel servizio di gestione delle API in `CertificateAuthority` o `Root` Store.
...
Pubblica le modifiche da una filiale git al database di configurazione.

### [Remove-AzApiManagement](Remove-AzApiManagement.md)
Rimuove un servizio di gestione API.
...
Rimuove una specifica versione dell'API
...
Rimuovere l'entità di diagnostica dall'ambito del livello globale o dell'API.
...
Elimina un abbonamento esistente.
...
Ripristina un servizio di gestione delle API dal BLOB di archiviazione di Azure specificato.

### [Salva-AzApiManagementTenantGitConfiguration](Save-AzApiManagementTenantGitConfiguration.md)
Consente di salvare le modifiche creando un commit per la configurazione corrente.
...
Aggiorna una versione dell'API impostata nel contesto di gestione delle API.
...
Configura un gruppo di gestione API.
...
Imposta i dettagli del prodotto di gestione delle API.

### [Set-AzApiManagementSubscription](Set-AzApiManagementSubscription.md)
Imposta i dettagli degli abbonamenti esistenti.
...
Aggiorna una specifica versione dell'API.
