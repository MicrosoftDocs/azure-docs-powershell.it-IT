...
## Descrizione
Il cmdlet **New-AzApiManagementGatewayHostnameConfiguration** crea un nome host Configuratin per il gateway esistente.

## ESEMPI

### Esempio 1: creare una configurazione hostname per il gateway esistente
```powershell
...
Questo comando crea una configurazione hostname "H01" per un gateway "G01".

## PARAMETRI

### -CertificateResourceId
Un identificatore di risorsa per l'ID del certificato esistente. Questo parametro è obbligatorio.
...
Identificatore del nuovo nome host del gateway confiuration.
