...
## Sinossi
Crea un archivio di configurazione con i parametri specificati.
...
### Esempio 1: creare un archivio di configurazione dell'app
```powershell
...
Questo comando crea un archivio di configurazione dell'app.

### Esempio 2: creare una configurazione dell'app con IdentityType impostato su "UserAssigned"
```powershell
...
Questo comando crea una configurazione dell'app e assegna a quest'ultimo un'identità gestita assegnata dall'utente.
`Update-AzAppConfigurationStore`Per abilitare CMK (cusomer Managed Key), vedere l'esempio della procedura seguente.

### Esempio 3: creare una configurazione dell'app con IdentityType impostato su "SystemAssigned" 
```powershell
...
Questo comando crea una configurazione dell'app e Abilita l'identità gestita assegnata al sistema associata alla risorsa.
`Update-AzAppConfigurationStore`Per abilitare CMK (cusomer Managed Key), vedere l'esempio della procedura seguente.

### Esempio 4: creare una configurazione dell'app con IdentityType impostato su "SystemAssigned, UserAssigned"
```powershell
PS C:\> $assignedIdentity = New-AzUserAssignedIdentity -ResourceGroupName azpwsh-manual-test -Name assignedIdentity
...
Puoi abilitare l'identità gestita assegnata al sistema e assegnare le identità assegnate agli utenti contemporaneamente.
`Update-AzAppConfigurationStore`Per abilitare CMK (cusomer Managed Key), vedere l'esempio della procedura seguente.
...
Questa operazione non può essere modificata dopo la creazione della risorsa.
