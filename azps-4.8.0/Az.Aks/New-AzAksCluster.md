...
Creare un nuovo cluster AKS (Azure Kubernetes Service).

## ESEMPI

### Nuovo AK con params predefiniti.

```powershell
PS C:\> New-AzAks -ResourceGroupName myResourceGroup -Name myCluster
```

### Creare un contenitore di Windows Server in un AKS.
Per creare un contenitore Windows Server in un AKS, devi specificare almeno quattro parametri seguenti durante la creazione di AKS e il valore per `NetworkPlugin` e `NodeVmSetType` deve essere `azure` e `VirtualMachineScaleSets` rispettivamente.
...
### -AcrNameToAttach
Concedere il ruolo "acrpull" dell'ACR specificato all'entità servizio AKS, ad esempio myacr
...
### -DnsNamePrefix
Prefisso del nome DNS per il cluster. La lunghezza deve essere <= 9 se gli utenti intendono aggiungere un contenitore Windows.
...
### -EnableRbac
Se abilitare l'accesso a Kubernetes Role-Based
...
### -GenerateSshKey
Genera file di chiave SSH in {HOME}/.ssh/id_rsa.
...
### -NetworkPlugin
Plug-in di rete usato per la creazione di rete Kubernetes.
...
### -NodeName
Nome univoco del profilo del pool di agenti nel contesto del gruppo sottoscrizioni e risorse.
...
### -NodeOsType
OsType da usare per specificare il tipo di sistema operativo. Scegliere tra Linux e Windows. Impostazione predefinita per Linux.
...
ScaleSetEvictionPolicy da usare per specificare i criteri di eliminazione per il set di scale della macchina virtuale con priorità bassa. Impostazione predefinita per l'eliminazione.
...
### -NodeSetPriority
ScaleSetPriority da usare per specificare la priorità impostata per la scala della macchina virtuale. Impostazione predefinita su normale.
...
### -NodeVmSize
Le dimensioni della macchina virtuale. Il valore predefinito è Standard_D2_v2.
...
### -NodeVnetSubnetID
VNet SubnetID specifica l'identificatore di subnet di VNet.
...
### -SubnetName
Nome subnet di VirtualNode addon.
...
Il nome utente dell'amministratore da usare per le VM di Windows.
...
La password di amministratore da usare per le VM di Windows, la cui lunghezza deve essere di almeno 12, che contiene almeno un carattere maiuscolo inferiore, vale a dire `[a-z]` uno `[A-Z]` e uno speciale `[!@#$%^&*()]` .
...
### -WorkspaceResourceId
ID risorsa dell'area di lavoro di monitoraggio addon.
