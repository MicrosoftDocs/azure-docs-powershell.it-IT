...
### Esempio 1: rimuovere il server PostgreSql per resourceGroup e il nome del server
```powershell
PS C:\> Remove-AzPostgreSqlServer -ResourceGroupName PostgreSqlTestRG -Name PostgreSqlTestServer

```

Questo cmdlet rimuove il server PostgreSql per resourceGroup e il nome del server.

### Esempio 2: rimuovere il server PostgreSql per identità
```powershell
...
Questi cmdlet rimuovono il server PostgreSql per identità.
