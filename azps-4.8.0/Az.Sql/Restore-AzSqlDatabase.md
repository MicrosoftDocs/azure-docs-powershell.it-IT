...
## Descrizione
Il cmdlet **Restore-AzSqlDatabase** ripristina un database SQL da un backup Geo-ridondante, un backup di un database eliminato, un backup a lungo termine o un momento in un database dinamico.
...
Il primo comando ottiene il backup del database eliminato che si vuole ripristinare tramite [Get-AzSqlDeletedDatabaseBackup](./Get-AzSqlDeletedDatabaseBackup.md).
Il secondo comando avvia il ripristino dal backup del database eliminato usando il cmdlet [Restore-AzSqlDatabase](./Restore-AzSqlDatabase.md) . Se il parametro-PointInTime non viene specificato, il database verrà ripristinato all'ora di eliminazione.
...
Il primo comando ottiene il backup del database eliminato che si vuole ripristinare tramite [Get-AzSqlDeletedDatabaseBackup](./Get-AzSqlDeletedDatabaseBackup.md).
Il secondo comando avvia il ripristino dal backup del database eliminato tramite [Restore-AzSqlDatabase](./Restore-AzSqlDatabase.md). Se il parametro-PointInTime non viene specificato, il database verrà ripristinato all'ora di eliminazione.
...
### -ComputeGeneration
Generazione di Compute da assegnare al database ripristinato
...
Puoi usare il cmdlet Get-AzSqlDeletedDatabaseBackup per ottenere il backup di un database SQL eliminato.
...
Puoi usare il cmdlet Get-AzSqlDatabaseGeoBackup per ottenere un backup Geo-ridondante.
...
### -VCore
I numeri Vcore del database SQL di Azure ripristinato.
...
[Recuperare un database SQL di Azure da un errore utente](http://go.microsoft.com/fwlink/?LinkId=746944)
