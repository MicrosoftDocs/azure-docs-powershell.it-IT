...
## Descrizione
Il cmdlet **Restore-AzSqlInstanceDatabase** ripristina un database di istanza da un backup Geo-ridondante, un momento in un database dinamico o un backup a lungo termine.
...
Il primo comando consente di ottenere i database di istanza eliminati denominati "DB1" nell'istanza "managedInstance1".
Il secondo comando Ripristina il database recuperato, dal backup point-in-Time specificato al database dell'istanza denominato Database01_restored.

### Esempio 5: ripristinare un database di istanza eliminato da un momento
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
Il primo comando consente di ottenere i database di istanza eliminati denominati "DB1" nell'istanza "managedInstance1".
Il secondo comando Ripristina il database recuperato, dal backup temporizzato specificato al database dell'istanza denominato Database01_restored usando l'oggetto input.

### Esempio 6: ripristinare un database dal backup di LTR.
```
...
Ripristina il backup di LTR con l'ID di risorsa indicato (che può essere trovato eseguendo Get-AzSqlInstanceDatabaseLongTermRetentionBackup).
