...
## Descrizione
Imposta le proprietà delle risorse del tipo di nodo o esegue le azioni di riimmagine su specifiche Registrazione dispositivi del parametro tipo di nodo con-reimage. In reimgae l'operazione i nodi del tessuto del servizio verranno disabilitati prima di rieseguire l'imaging delle VM e li ha riattivati quando torneranno. Se l'operazione viene eseguita nei tipi di nodi primari, potrebbe essere necessario un po' di tempo perché potrebbe non ricreare contemporaneamente tutti i nodi. Use-ForceReimage forza l'operazione anche se il tessuto di servizio non è in grado di disabilitare i nodi, ma può essere usato con cautela perché potrebbe causare perdite di dati se nel nodo sono in esecuzione carichi di lavoro con stato.
...
Aggiornare il conteggio delle istanze del tipo di nodo.
...
Aggiornare il proprietà di posizionamento del tipo di nodo. Verrà sovrascritto il proprietà di posizionamento precedente, se disponibile.
...
Nodo riimmagine 0 e 3 nel tipo di nodo.
...
Aggiorna il conteggio delle istanze del tipo di nodo con piping.

## PARAMETRI

### -ApplicationEndPort
Porta di fine applicazione di un intervallo di porte.
...
### -Capacità
I tag di capacità applicati ai nodi del tipo di nodo come coppie chiave/valore, il gestore di risorse del cluster usa questi tag per comprendere la quantità di risorse di un nodo. L'aggiornamento di questa operazione sostituirà i valori correnti.
...
### -EphemeralEndPort
Porta finale effimera di un intervallo di porte.
...
### -EphemeralStartPort
Porta di inizio effimera di un intervallo di porte.
...
### -ForceReimage
L'uso di questo contrassegno forza la rimozione anche se il tessuto di servizio non è in grado di disabilitare i nodi.
Usare con cautela poiché questo può causare perdite di dati se i carichi di lavoro con stato sono in esecuzione nel nodo.
...
### -NodeName
Elenco di nomi di nodo per l'operazione.
...
### -PlacementProperty
I tag di posizionamento applicati ai nodi del tipo di nodo come coppie chiave/valore, che possono essere usati per indicare la posizione in cui devono essere eseguiti determinati servizi (carico di lavoro). L'aggiornamento di questa operazione sostituirà i valori correnti.
...
### -Riimmagine
Specificare i nodi di riimmagine nel tipo di nodo.
