...
### Esempio 1: ottenere un'app di funzione per nome e arrestarla.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Stop-AzFunctionApp -Force
```

Questo comando ottiene un'app di funzione per nome e la arresta.

### Esempio 2: arrestare un'app di funzione per nome.
```powershell
PS C:\> Stop-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Questo comando Arresta un'app di funzione per nome.
...
### -Force
Impone al cmdlet di interrompere l'app della funzione senza richiedere conferma.
