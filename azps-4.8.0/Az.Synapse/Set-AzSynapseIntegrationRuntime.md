...
Il cmdlet aggiunge l'area di lavoro per usare il runtime di integrazione condivisa. Quando `-SharedIntegrationRuntimeResourceId` si usa il parametro `-Type` deve anche essere incluso. Tieni presente che nell'area di lavoro deve essere concessa l'autorizzazione per usare Integration runtime prima di eseguire cmdlet.
...
Il nome Self-Hosted Integration Runtime che viene usato come proxy.
...
Nome del servizio collegato di Azure BLOB Storage che fa riferimento all'archivio dati di staging da usare quando si spostano i dati tra Self-Hosted e il runtime di integrazione di Azure-SSIS.
...
Il percorso nell'archivio dati di staging da usare quando si spostano dati tra Self-Hosted e i runtime di integrazione di Azure-SSIS, se non specificato, verrà usato un contenitore predefinito.
...
### -ExpressCustomSetup
La configurazione personalizzata Express per runtime Integration SSIS che potrebbe essere usata per configurare le configurazioni e i componenti di terze parti senza script di configurazione personalizzata.
...
### -LicenseType
Tipo di licenza che si desidera selezionare per l'IR SSIS.
Esistono due tipi: LicenseIncluded o BasePrice.
Se si è qualificati per i prezzi di Azure Hybrid use benefit (AHUB), selezionare BasePrice.
...
### -VNetId
ID della VNet a cui verrà aggiunto il runtime di integrazione.
