...
## <a name="560---march-2021"></a>5.6.0-2021 marzo
#### <a name="azaccounts"></a>Az.Accounts
* Aggiornare Azure. Identity per risolvere il problema che Connect-AzAccount ha esito negativo quando viene utilizzata la credenziale ADFS [#13560]

#### <a name="azautomation"></a>Az.Automation
* Correzione del problema per cui non è possibile serializzare correttamente la stringa. [#14215]
* Aggiunta del supporto per il tipo Runbook di Python3

#### <a name="azcompute"></a>Az.Compute
* Il parametro '-EnableHotpatching ' è stato aggiunto al cmdlet ' set-AzVMOperatingSystem ' per i computer Windows. 
* È stato aggiunto il parametro '-PatchMode ' ai set di parametri Linux nel cmdlet ' set-AzVMOperatingSystem '. 
* [Modifica di rilievo] Modifiche di rilievo per gli utenti nell'anteprima pubblica per la funzionalità di applicazione di patch per la macchina virtuale guest.
    - La proprietà' RebootStatus ' è stata rimossa dall'oggetto ' Microsoft. Azure. Management. Compute. Models. LastPatchInstallationSummary '. 
    - La proprietà' StartedBy ' è stata rimossa dall'oggetto ' Microsoft. Azure. Management. Compute. Models. LastPatchInstallationSummary '.
    - La proprietà' kbid ' è stata rinominata in ' KbId ' nell'oggetto ' Microsoft. Azure. Management. Compute. Models. VirtualMachineSoftwarePatchProperties '. 
    - La proprietà' patches ' è stata rinominata in ' availablePatches ' nell'oggetto ' Microsoft. Azure. Management. Compute. Models. VirtualMachineAssessPatchesResult '. 
    - L'oggetto ' Microsoft. Azure. Management. Compute. Models. SoftwareUpdateRebootBehavior ' è stato rinominato in ' Microsoft. Azure. Management. Compute. Models. VMGuestPatchRebootBehavior '.
    - L'oggetto ' Microsoft. Azure. Management. Compute. Models. InGuestPatchMode ' è stato rinominato in ' Microsoft. Azure. Management. Compute. Models. WindowsVMGuestPatchMode '.
* [Modifica di rilievo] Rimossi tutti i cmdlet ' servizio contenitore '. L'API del servizio contenitore è stata deprecata nel 2020 gennaio. 
...
* Correzione dell'autenticazione per `Connect-AzContainerRegistry`

#### <a name="azcosmosdb"></a>AZ. CosmosDB
* Sono stati introdotti NetworkAclBypass e NetworkAclBypassResourceIds per i cmdlet account del database.
* Il parametro ServerVersion è stato introdotto in Update-AzCosmosDBAccount.
* Introduzione di BackupInterval e BackupRetention per i cmdlet per gli account di database

#### <a name="azdatafactory"></a>Az.DataFactory
* Versione di ADF .NET SDK aggiornata a 4.14.0

#### <a name="azmigrate"></a>AZ. migrate
* AZ. migrate GA
* Incorporato Initialize-AzMigrateReplicationInfrastructure come cmdlet nel modulo AZ. migrate, dallo script esterno attualmente eseguito.
* Sono stati apportati alcuni parametri di New-AzMigrateServerReplication, senza distinzione tra maiuscole New-AzMigrateDiskMapping e minuscole.
* Aggiunta del supporto per la modifica del ridimensionamento dell'appliance, per gestire le nuove chiavi V3.

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Aggiunto il controllo null per l'account di archiviazione di destinazione nel ripristino FileShare.

#### <a name="azresources"></a>Az.Resources
* Aggiunta del supporto per la distribuzione di risorse di Azure in lingua bicipite
* Correzione dei problemi relativi alle distribuzioni di TemplateSpec in ' New-AzTenantDeployment ' è New-AzManagementGroupDeployment '
* Aggiunta del supporto per il parametro '-QueryString ' nei cmdlet ' test-AZ * Deployments '
* Correzione del problema relativo ai parametri dinamici quando si usa ' New-AZ * Deployments ' con '-QueryString '
* Aggiunta del supporto per il parametro '-TemplateParameterObject ' durante l'uso del parametro '-TemplateSpecId ' nei cmdlet ' New-AZ * Deployments '
* Correzione del messaggio di errore non accurato ricevuto durante il tentativo di distribuire una specifica di modello inesistente

#### <a name="azstorage"></a>Az.Storage
* Aggiornato a Microsoft. Azure. Management. storage 19.0.0 per supportare la nuova versione API 2021-01-01.
* Regola di accesso alle risorse supportata in NetworkRuleSet
    - 'Update-AzStorageAccountNetworkRuleSet'
    - 'Add-AzStorageAccountNetworkRule'
    - ' Remove-AzStorageAccountNetworkRule '
* Versione BLOB supportata e tipo di BLOB di Accodamento nei criteri di gestione
    - 'Add-AzStorageAccountManagementPolicyAction'
    - ' New-AzStorageAccountManagementPolicyFilter '
    - 'Set-AzStorageAccountManagementPolicy'
* Account di creazione/aggiornamento supportato con AllowSharedKeyAccess
    - 'New-AzStorageAccount'
    - 'Set-AzStorageAccount'
* L'ambito di creazione della crittografia è supportato con RequireInfrastructureEncryption
    - 'New-AzStorageEncryptionScope'
* BLOB in blocchi di copia supportati in modo sincrono, con ambito di crittografia
    - ' Copy-AzStorageBlob '
* Correzione del problema che Get-AzStorageBlobContent usare un separatore di directory errato char in Linux e MacOS [#14234]

#### <a name="azwebsites"></a>Az.Websites
* È stata introdotta un'opzione per assegnare il timeout personalizzato per ' Publish-AzWebApp ' 
* Aggiunta del supporto per ambiente del servizio app
...
* @alunmj, Ortografia piccola, modifiche di formattazione (#14155)
* @chakra146, Aggiornare Add-AzLoadBalancerInboundNatPoolConfig.md (#14231)
* Martin Ehrnst ( @ehrnst ), correzione di un errore di digitazione nel cmdlet (#14112)
* Jan David Narkiewicz ( @jdnark )
  * Esempi usati New-AzAks, ovvero cmdlet legacy e alias per New-AzAksCluster. Esempi modificati per utilizzare New-AzAksCluster che è il cmdlet a cui è destinata questa pagina della documentazione. (#14088)
  * Tipo Fox: modificato SshKeyVaule in SshKeyValue (#14087)
* Ivan Kulezic ( @kukislav ), aggiungere esempi di configurazione della manutenzione di SQL mi (#14216)
* @webguynj, Aggiornare Set-AzNetworkSecurityRuleConfig.md (#14176)
* Yannick DILS ( @yannickdils ) ha aggiunto un cmdline aggiuntivo all'esempio che applica le modifiche al servizio di bilanciamento del carico (#14185)

## <a name="550---february-2021"></a>5.5.0-2021 febbraio
#### <a name="azaccounts"></a>Az.Accounts
* Codice CloudError rilevato nell'eccezione
* È stato generato l'evento ' ContextCleared ' quando è stato eseguito ' Clear-AzContext '

#### <a name="azaks"></a>Az.Aks
* Messaggio di errore raffinato dell'errore del cmdlet.
* Gestione delle eccezioni aggiornata per l'utilizzo di Azure PowerShell eccezioni correlate.
* Correzione del problema per cui l'utente non è stato in grado di usare l'entità servizio specificata per creare il cluster Kubernetes. [#13938]

#### <a name="azautomation"></a>Az.Automation
* Correzione del problema di elaborazione di ' PSCustomObject ' è array '.

#### <a name="azcompute"></a>Az.Compute
* Aggiunta del parametro '-EnableAutomaticUpgrade ' a' set-AzVmExtension ' è Add-AzVmssExtension '.
* Il parametro FilterExpression è stato rimosso dalla documentazione del cmdlet ' Get-AzVMImage '. 
* Aggiunta del messaggio di deprecazione ai cmdlet di servizio contenitore:
    ...
    - ' Remove-AzContainerServiceAgentPoolProfile '
    - ' Update-AzContainerService '
* Aggiunta del parametro '-BurstingEnabled ' a' New-AzDiskConfig ' e a' New-AzDiskUpdateConfig '
* Aggiunta dei parametri '-GroupByApplicationId ' è-GroupByUserAgent ' ai cmdlet ' export-AzLogAnalyticThrottledRequest ' è export-AzLogAnalyticRequestRateByInterval '.
* È stato aggiunto il parametro ' VMParameterSet ' impostato sul cmdlet ' Get-AzVMExtension '. Aggiunta del nuovo parametro '-VM ' al set di parametri. 

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Aggiunta di cmdlet a operazioni di repository, manifesto e tag supportate:
...
Supportato-EnableNoPublicIP durante la creazione di un'area di lavoro di databricks

#### <a name="azfrontdoor"></a>Az.FrontDoor
* Aggiunta di FrontDoorId alle proprietà
* Aggiunta di esclusioni JSON e supporto RequestBodyCheck alle regole gestite

#### <a name="azhdinsight"></a>Az.HDInsight
* Aggiunto il nuovo parametro '-EnableComputeIsolation ' è-ComputeIsolationHostSku ' al cmdlet ' New-AzHDInsightCluster ' per supportare la funzionalità di isolamento del calcolo
* Aggiunta proprietà' ComputeIsolationProperties ' è ConnectivityEndpoints ' nella classe AzureHDInsightCluster.

#### <a name="azkeyvault"></a>Az.KeyVault
* Supportato per specificare il tipo di chiave e il nome della curva durante l'importazione di chiavi tramite un file BYOK

#### <a name="aznetwork"></a>Az.Network
* Sono stati aggiunti nuovi cmdlet per sostituire il vecchio nome di prodotto "Virtual Router" con il nuovo nome "route server" in futuro.
    - ' New-AzRouteServer '
    - ' Get-AzRouteServer '
    ...
    - Aggiunta dell'avviso di attributo Deprecation ai cmdlet obsoleti.
* Correzione di bug in ExpressRouteLink MacSecConfig. Aggiunta della nuova proprietà' SciState ' a' PSExpressRouteLinkMacSecConfig '
* Aggiornamento dell'elenco di formato e delle visualizzazioni di tabella del formato per Get-AzVirtualNetworkGatewayConnectionIkeSa

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* Le modifiche ritratte apportate in PowerShell hanno aumentato il limite di righe della richiesta. È stata rimossa un'istruzione non corretta di supporto del paging

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* i limiti di convalida dei criteri sono stati modificati in base al servizio di backup.
* Aggiunta della ridondanza della zona per gli insiemi di credenziali dei servizi di ripristino. 
* Azure Site Recovery supporto per il gruppo di posizionamento vicino per VMware in Azure e HyperV ai provider di Azure.
* Azure Site Recovery il supporto per la zona di disponibilità per VMware in Azure e HyperV ai provider di Azure.
* Supporto Azure Site Recovery per UseManagedDisk per HyperV al provider Azure

#### <a name="azresources"></a>Az.Resources
* Il tipo di entità è stato rimosso New-AzRoleAssignment e Set-AzRoleAssignment perché il mapping corrente sta causando la suddivisione di determinati scenari

#### <a name="azsql"></a>Az.Sql
* Aggiunta di MaintenanceConfigurationId a' New-AzSqlDatabase ',' set-AzSqlDatabase ',' New-AzSqlElasticPool ' è set-AzSqlElasticPool '
* Correzione della regressione in ' set-AzSqlServerAudit ' quando viene fornito l'argomento PredicateExpression

#### <a name="azstorage"></a>Az.Storage
* Impostazioni RoutingPreference supportate nell'account di archiviazione di creazione/aggiornamento
    - 'New-AzStorageAccount'
    - 'Set-AzStorageAccount'
* Aggiornamento di Azure. storage. Blobs a 12.8.0
* Azure. storage. files. Shares è stato aggiornato a 12.6.0
* Aggiornamento di Azure. storage. files. datalake a 12.6.0

#### <a name="azwebsites"></a>Az.Websites
* Aggiunta del supporto per l'importazione di un certificato dell'insieme di credenziali delle chiavi in WebApp.

### <a name="thanks-to-our-community-contributors"></a>Grazie ai collaboratori della community
* @atul-ram, Aggiornare Set-AzEventHub.md (#13921)
* Christoph Bergmeister [MVP] ( @bergmeister ), set-AzDataLakeGen2AclRecursive.MD-Fix typo (directiry-> directory) (#14082)
* Alexander Schmidt ( @devdeer-alex ), correzione di collegamenti interrotti alle linee guida per i contributi (#14009)
* @JiangYuchun, Aggiornare Get-AzApplicationGatewayAuthenticationCertificate.md (#13972)
* Sebastian Olsen ( @Spacebjorn ), comando di esempio corretto (#13901)

## <a name="540---january-2021"></a>5.4.0-2021 gennaio
#### <a name="azaccounts"></a>Az.Accounts
* Visualizzato ID richiesta client corretto nel messaggio di debug [#13745]
* Aggiunta del tipo di eccezione Azure PowerShell comune
* API di archiviazione supportata 2019-06-01

#### <a name="azautomation"></a>Az.Automation
* Correzione del problema per cui la descrizione non è stata popolata per le pianificazioni di gestione aggiornamenti

#### <a name="azcosmosdb"></a>AZ. CosmosDB
* Disponibilità generale del modulo ' AZ. CosmosDB '
* Limitazione New-AzCosmosDBAccount cmdlet per effettuare chiamate di aggiornamento agli account di database esistenti.
* Introduzione a AnalyticalStorageTTL in sqlcontainer.

#### <a name="aziothub"></a>Az.IotHub
* Correzione di una regressione relativa alla generazione di token SAS

#### <a name="azkeyvault"></a>Az.KeyVault
* Correzione di un problema nel modulo di gestione dei segreti

#### <a name="azlogicapp"></a>Az.LogicApp
* Correzione del problema che ' Get-AzLogicAppTriggerHistory ' è Get-AzLogicAppRunAction ' recuperano solo la prima pagina di risultati [#9141]

#### <a name="azmonitor"></a>Az.Monitor
* Aggiunta di cmdlet per le regole di raccolta dati: 
    - ' Get-AzDataCollectionRule '
    - ' New-AzDataCollectionRule '
    - ' Set-AzDataCollectionRule '
    - ' Update-AzDataCollectionRule '
    - ' Remove-AzDataCollectionRule '
* Aggiunta di cmdlet per associazioni di regole di raccolta dati
...
* Cmdlet aggiornati per abilitare l'impostazione di ConnectionMode nelle connessioni del gateway di rete virtuale.
    - 'New-AzVirtualNetworkGatewayConnection'
    - 'Set-AzVirtualNetworkGatewayConnection'
* Il cmdlet ' New-AzFirewallPolicyApplicationRule ' è stato aggiornato:
    - Aggiunta del parametro TargetUrl
    - Aggiunta del parametro TerminateTLS
* Aggiunta di nuovi cmdlet per le funzionalità di Azure Firewall Premium
    - ' New-AzFirewallPolicyIntrusionDetection '
    - ' New-AzFirewallPolicyIntrusionDetectionBypassTraffic '
    - ' New-AzFirewallPolicyIntrusionDetectionSignatureOverride '
* Cmdlet New-AzFirewallPolicy aggiornato:
    - Aggiunta del parametro-SkuTier
    - Aggiunta del parametro-Identity
    - Aggiunta del parametro-UserAssignedIdentityId
    - Aggiunta del parametro-IntrusionDetection
    - Aggiunta del parametro-TransportSecurityName
    - Aggiunta del parametro-TransportSecurityKeyVaultSecretId
* Aggiunta del nuovo cmdlet per recuperare le associazioni di sicurezza IKE per le connessioni del gateway di rete virtuale.
    - ' Get-AzVirtualNetworkGatewayConnectionIkeSa '
* Aggiunta del supporto per l'autenticazione multipla per p2sVpnGateway
    - Aggiornamento New-AzVpnServerConfiguration e Update-AzVpnServerConfiguration per consentire l'impostazione di più parametri di autenticazione.
* Aggiornamento del cmdlet ' New-AzVpnGateway ' è New-AzP2sVpnGateway ' completato:
    - Aggiunta del parametro EnableRoutingPreferenceInternetFlag

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Aggiunta della funzionalità di ripristino tra aree.  
* Blocco del recupero del carico di lavoro quando l'elemento di destinazione è un gruppo di disponibilità.

#### <a name="azresources"></a>Az.Resources
* Aggiunta del supporto per il parametro-QueryString nei cmdlet New-AZ * Deployments

#### <a name="azsql"></a>Az.Sql
* Il cmdlet ' Start-AzSqlInstanceDatabaseLogReplay ' è stato eseguito in modo sincrono e il flag AsJob è stato aggiunto

#### <a name="azstorage"></a>Az.Storage
* Correzione di ContinuationToken mai null durante l'elenco di BLOB con-IncludeVersion
    - 'Get-AzStorageBlob'

#### <a name="azwebsites"></a>Az.Websites
* Aggiunta del supporto per i certificati gestiti del servizio app
    - ' New-AzWebAppCertificate '
    - ' Remove-AzWebAppCertificate '
* È stato risolto un problema che causava la rimozione della password di Docker da appSettings in ' set-AzWebApp ' è set-AzWebAppSlot '

### <a name="thanks-to-our-community-contributors"></a>Grazie ai collaboratori della community
* Ivan Akcheurov ( @ivanakcheurov ), Update set-AzSecurityWorkspaceSetting.MD (#13877)
* @javiermarasco, Esempio di aggiornamento (#13837)
* @jhaprakash26, Aggiornare Set-AzVirtualNetwork.md (#13857)
* Michael Holmes ( @MichaelHolmesWP ), Update New-AzStorageTableStoredAccessPolicy.MD (#13871)
* Michael James ( @mikejwhat ), Consenti a Get-AzLogicAppTriggerHistory e Get-AzLogicAppRunAction di restituire più di 30 risultati (#13846)
* @Willem-J-an, Correzione di bug che causa la rimozione della password Docker da appSettings in set-AzWebApp (slot) (#13866)

## <a name="530---december-2020"></a>5.3.0 - Dicembre 2020
#### <a name="azaccounts"></a>Az.Accounts
* Corretto il problema per cui il proxy HTTP non viene rispettato in Windows PowerShell [#13647]
* Migliorato il log di debug per le operazioni a esecuzione prolungata nei moduli generati

#### <a name="azautomation"></a>Az.Automation
* Corretto il problema per cui i parametri di 'Start-AzAutomationRunbook' non possono convertire la stringa con wrapping in PSObject in una stringa JSON [#13240]
* Corretto il completamento del percorso per il cmdlet New-AzAutomationUpdateManagementAzureQuery

#### <a name="azcompute"></a>Az.Compute
* Aggiunto il nuovo parametro 'VM' nel nuovo set di parametri 'VMParameterSet' ai cmdlet 'Get-AzVMDscExtensionStatus' e 'Get-AzVMDscExtension'. 

#### <a name="azdatabricks"></a>Az.Databricks
* Corretto un problema che potrebbe causare la restituzione di 'New-AzDatabricksVNetPeering' prima che ne venga completato il provisioning (https://github.com/Azure/autorest.powershell/issues/610) )

#### <a name="azdatafactory"></a>Az.DataFactory
* Corretto il problema del comando 'Invoke-AzDataFactoryV2Pipeline' per SupportsShouldProcess

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Aggiunta la proprietà StartVMOnConnect a hostpool.

#### <a name="azhdinsight"></a>Az.HDInsight
* Aggiunte le proprietà: Fqdn e EffectiveDiskEncryptionKeyUrl nella classe AzureHDInsightHostInfo.

#### <a name="azkeyvault"></a>Az.KeyVault
* Aggiunto un nuovo parametro '-AsPlainText' a 'Get-AzKeyVaultSecret' per restituire direttamente il segreto in testo normale [#13630]
* Aggiunto il supporto per il ripristino selettivo di una chiave da un backup completo di un modulo di protezione hardware gestito [#13526]
* Corretti alcuni problemi secondari [#13583] [#13584]
* Aggiunti gli oggetti restituiti mancanti di 'Get-Secret' nel modulo SecretManagement
* Corretto un problema che potrebbe causare la creazione di un insieme di credenziali senza criteri di accesso predefiniti [#13687]

#### <a name="azkusto"></a>Az.Kusto
* Aggiornata l'API alla versione 2020-09-18.

#### <a name="aznetwork"></a>Az.Network
* Corretto un problema nel cmdlet remove peering and connection per lo scenario ExpressRouteCircuit
    - 'Remove-AzExpressRouteCircuitPeeringConfig' e 'Remove-AzExpressRouteCircuitConnectionConfig'

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* Aggiunto il supporto per la restituzione di risultati impaginati per Get-AzPolicyState

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Abilitata la funzionalità di eliminazione temporanea per SQL.
* Corretto il ripristino dei gruppi di disponibilità SQL e rimosso il controllo del nome del contenitore.
* Cambiato il formato del nome del contenitore per l'elemento di backup di File di Azure.
* Aggiunto il supporto per la funzionalità delle chiavi gestite dal cliente per l'insieme di credenziali di servizi di ripristino.

#### <a name="azresources"></a>Az.Resources
* Corretto un problema di eccezione NullRef in 'New-AzureManagedApplication' e 'Set-AzureManagedApplication'.
* Aggiornato Azure Resource Manager SDK per l'uso dell'ultima versione api-version in disponibilità generale di DeploymentScripts: 2020-10-01.

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Corretto 'Add-AzServiceFabricNodeType'. Aggiunto un tipo di nodo al cluster di Service Fabric prima della creazione di un set di scalabilità di macchine virtuali.

#### <a name="azsql"></a>Az.Sql
* Corretta la descrizione dei parametri per il comando 'InstanceFailoverGroup'.
* Aggiornata la logica per cui schemaName, tableName e columnName vengono estratti dall'ID dei comandi di classificazione dati di SQL.
* Corretti i campi Status e StatusMessage in 'Get-AzSqlDatabaseImportExportStatus' conformemente alla documentazione
* Aggiunti i cmdlet di controllo delle operazioni di supporto Microsoft (DevOps): Get-AzSqlServerMSSupportAudit, Set-AzSqlServerMSSupportAudit, Remove-AzSqlServerMSSupportAudit

#### <a name="azstorage"></a>Az.Storage
* Aggiunto il supporto per create/update/get/list EncryptionScope di un account di archiviazione
    - 'New-AzStorageEncryptionScope'
    - 'Update-AzStorageEncryptionScope'
    - 'Get-AzStorageEncryptionScope'
* Aggiunto il supporto per la creazione di contenitori e l'upload di BLOB con l'impostazione dell'ambito di crittografia
    - 'New-AzRmStorageContainer'
    - 'New-AzStorageContainer'
    - 'Set-AzStorageBlobContent'

### <a name="thanks-to-our-community-contributors"></a>Grazie ai collaboratori della community
* Andreas Wolter (@AndreasWolter), rimosso il linguaggio di marketing, migliorato il filtro di esempio (#13671)
* Tidjani Belmansour (@BelRarr), aggiornato Get-AzBillingInvoice.md (#13634)
* David Klempfner (@DavidKlempfner)
  * Corretto un errore di ortografia (#13677)
  * Aggiornato PSMetricNoDetails.cs (#13676)
* @kilobyte97, bugfix per il cmdlet remove per eliminare la configurazione (#13655)
* @kongou-ae, aggiornato Set-AzFirewall.md (#13727)
* @MasterKuat, corretto lo scambio tra titolo e codice nella documentazione (#13666)
* NickT (@nukeulis), aggiornato Set-AzContext.md (#13702)
* @PaulHCode, aggiornato Start-AzJitNetworkAccessPolicy.md - Corretto l'esempio per mostrare il cmdlet corretto da illustrare (#13713)
* Ryan Borstelmann (@ryanborMSFT), rimosso l'ID sottoscrizione (#13715)
* Shashikant Shakya (@shshakya), aggiornato Set-AzSqlDatabase.md (#13674)
* Sebastian Olsen (@Spacebjorn), aggiornato Get-AzRecoveryServicesBackupItem.md (#13719)

## <a name="520---december-2020"></a>5.2.0 - Dicembre 2020
#### <a name="azaccounts"></a>Az.Accounts
* Possibilità di analizzare l'ora ExpiresOn da un token non elaborato se non è possibile ottenerla dalla libreria sottostante
* Migliorato il messaggi di avviso relativo all'autenticazione interattiva non disponibile

#### <a name="azapimanagement"></a>Az.ApiManagement
* [Modifica di rilievo] 'New-AzApiManagementProduct' per impostazione predefinita non ha un limite di sottoscrizioni.

#### <a name="azcompute"></a>Az.Compute
* Modificato Get-AzVm per filtrare in base a '-Name' prima di verificare la limitazione dovuta a un numero eccessivo di risorse. 
* Nuovo cmdlet 'Start-AzVmssRollingExtensionUpgrade'.

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Aggiunto il supporto del parametro 'Name' e del valore dell'input della pipeline per 'Get-AzContainerRegistryUsage' [#13605]
* Migliorate le eccezioni per 'Connect-AzContainerRegistry'

#### <a name="azdatafactory"></a>Az.DataFactory
* Aggiornato ADF .Net SDK alla versione 4.13.0

#### <a name="azhealthcareapis"></a>Az.HealthcareApis
* Aggiunto il supporto per chiavi gestite dal cliente

#### <a name="aziothub"></a>Az.IotHub
* Corretto un problema del token di firma di accesso condiviso.

#### <a name="azkeyvault"></a>Az.KeyVault
* Aggiunto il supporto di 'all' come opzione quando si impostano i criteri di accesso all'insieme di credenziali delle chiavi
* Aggiunto il supporto di nuove versioni del modulo SecretManagement [#13366]
* Aggiunto il supporto di ByteArray, String, PSCredential e Hashtable per 'SecretValue' in SecretManagementModule [#12190]
* [Modifica di rilievo] Riprogettata l'API dei cmdlet relativi al modulo di protezione hardware gestito.

#### <a name="azmonitor"></a>Az.Monitor
* Cambiato il parametro 'Rule' di 'New-AzAutoscaleProfile' per accettare un elenco vuoto. [#12903]
* Aggiunti nuovi cmdlet per supportare la creazione più flessibile di impostazioni di diagnostica:
    * 'Get-AzDiagnosticSettingCategory'
    * 'New-AzDiagnosticSetting'
    * 'New-AzDiagnosticDetailSetting'

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Apportate modifiche al testo della Guida e al nome del set di parametri per il cmdlet 'Restore-AzRecoveryServicesBackupItem'.

#### <a name="azresources"></a>Az.Resources
* Aggiunto il parametro '-Tag' per supportare 'Set-AzTemplateSpec' e 'New-AzTemplateSpec'
* Aggiunto il supporto per la visualizzazione tag al formattatore predefinito per le specifiche di modello 

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Aggiunto un esempio per 'Set-AzServiceFabricSetting' con il parametro SettingsSectionDescription
* Aggiornati i cmdlet relativi all'applicazione per evidenziare che il supporto riguarda solo risorse distribuite con modelli di Azure Resource Manager
* Contrassegnati per la deprecazione i cmdlet di certificazione del server 'Add-AzureRmServiceFabricClusterCertificate' e 'Remove-AzureRmServiceFabricClusterCertificate'

#### <a name="azsql"></a>Az.Sql
* Aggiunto SecondaryType a quanto segue: 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'
    - 'New-AzSqlDatabaseSecondary'
* Aggiunto HighAvailabilityReplicaCount a quanto segue: 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'
* Convertito ReadReplicaCount in alias di HighAvailabilityReplicaCount in quanto segue: 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'

#### <a name="azstorage"></a>Az.Storage
* Aggiunto il supporto per il caricamento di file di Azure di dimensioni fino a 4 TiB
    - 'Set-AzStorageFileContent'
* Aggiornato Azure.Storage.Blobs alla versione 12.7.0
* Aggiornato Azure.Storage.Files.Shares alla versione 12.5.0
* Aggiornato Azure.Storage.Files.DataLake alla versione 12.5.0

#### <a name="azstoragesync"></a>Az.StorageSync
* Aggiunta la funzionalità dei criteri di sincronizzazione a livelli con i criteri di download e la modalità di cache locale

#### <a name="azwebsites"></a>Az.Websites
* Impedita la duplicazione di regole di restrizione dell'accesso

### <a name="thanks-to-our-community-contributors"></a>Grazie ai collaboratori della community
* Andrew Dawson (@dawsonar802), aggiornato Get-AzKeyVaultCertificate.md - Ottenere il certificato e salvarlo come sezione pfx per usare PowerShell Core (#13557)
* @iviark, aggiornamenti di BYOK in PowerShell per le API per servizi sanitari (#13518)
* John Duckmanton (@johnduckmanton), corretta l'ortografia di TagPatchOperation (#13508)
* Michael James (@mikejwhat)
  * Migliorata la Guida di Get-AzLogicAppRunHistory (#13513)
* Richard de Zwart (@mountain65)
  * Aggiornato Update-AzAppConfigurationStore.md (#13485)
  * Aggiornato New-AzCosmosDBAccount.md (#13490)
* @SteppingRazor, New-AzApiManagementProduct: cambiato il valore predefinito del parametro SubscriptionsLimit in None (#13457)
* Steve Burkett (@SteveBurkettNZ), corretto un errore di ortografia per il parametro WorkspaceResourceId nell'esempio (#13589)

## <a name="510---november-2020"></a>5.1.0 - Novembre 2020
#### <a name="azaccounts"></a>Az.Accounts
* Corretto un problema per cui TenantId potrebbe non essere rispettato se si usa 'Connect-AzAccount -DeviceCode'[#13477]
* Aggiunto il nuovo cmdlet 'Get-AzAccessToken'
* Corretto un problema per cui si verifica un errore se il percorso del profilo utente è inaccessibile
* Corretto un problema che genera un errore Write-Object durante Connect-AzAccount [#13419]
* Aggiunto il parametro 'ContainerRegistryEndpointSuffix' a: 'Add-AzEnvironment', 'Set-AzEnvironment' 
* Aggiunto il supporto per interrompere l'accesso premendo <kbd>CTRL</kbd>+<kbd>C</kbd>
* Corretto un problema per cui 'Connect-AzAccount -KeyVaultAccessToken' non funziona [#13127]
* Corretto il riferimento Null e il metodo senza distinzione tra maiuscole e minuscole in 'Invoke-AzRestMethod'

#### <a name="azaks"></a>Az.Aks
* Corretto il problema per cui l'utente non può usare l'entità servizio per creare un nuovo cluster Kubernetes. [#13012]

#### <a name="azappconfiguration"></a>Az.AppConfiguration
* Disponibilità generale del modulo 'Az.AppConfiguration'

#### <a name="azdatafactory"></a>Az.DataFactory
* Migliorato il messaggio di errore del comando 'New-AzDataFactoryV2LinkedServiceEncryptedCredential'

#### <a name="azdatalakestore"></a>Az.DataLakeStore
* Aggiornato l'SDK del piano dati di ADLS alla versione 1.2.4-alpha. Modifiche: https://github.com/Azure/azure-data-lake-store-net/blob/preview-alpha/CHANGELOG.md#version-124-alpha

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Aggiunti nuovi cmdlet del pacchetto MSIX e aggiornati i cmdlet per applicazioni.

#### <a name="azeventhub"></a>Az.EventHub
* Corretti i comandi Cluster per il cluster di Hub eventi senza tag
* Aggiornato il testo della Guida per PartnerNamespace dei comandi AzEventHubGeoDRConfiguration 

#### <a name="azhdinsight"></a>Az.HDInsight
* Aggiunti i parametri 'ResourceProviderConnection' e 'PrivateLink' al cmdlet 'New-AzHDInsightCluster' per supportare la funzionalità di inoltro di collegamenti privati e in uscita
* Aggiunto il parametro 'AmbariDatabase' al cmdlet 'New-AzHDInsightCluster' per supportare la funzionalità del database Ambari personalizzato
* Aggiunto il valore 'AmbariDatabase' al parametro 'MetastoreType' del cmdlet 'Add-AzHDInsightMetastore'

#### <a name="aziothub"></a>Az.IotHub
* Consentiti i tag nel cmdlet di creazione di hub IoT.

#### <a name="azkeyvault"></a>Az.KeyVault
* Supportato l'aggiornamento del tag di Key Vault

#### <a name="azlogicapp"></a>Az.LogicApp
* Corretto il problema per cui Get-AzLogicAppRunHistory recupera solo la prima pagina di risultati

#### <a name="aznetwork"></a>Az.Network
* Aggiornati i cmdlet seguenti 
    - 'New-AzLoadBalancerFrontendIpConfigCommand', 'Set-AzLoadBalancerFrontendIpConfigCommand', 'Add-AzLoadBalancerFrontendIpConfigCommand':
        - Aggiunta la proprietà PublicIpAddressPrefix
        - Aggiunta la proprietà PublicIpAddressPrefixId
* Aggiunte nuove proprietà ai cmdlet seguenti per consentire il bilanciamento del carico globale
    - 'New-AzLoadBalancer':
        - Aggiunta la proprietà Sku Tier
    - 'New-AzPuplicIpAddress':
        - Aggiunta la proprietà Sku Tier
    - 'New-AzPublicIpPrefix':
        - Aggiunta la proprietà Sku Tier
    - 'New-AzLoadBalancerBackendAddressConfig':
        - Aggiunta la proprietà LoadBalancerFrontendIPConfigurationId
* Aggiornata la pianificazione per deprecare gli avvisi per i cmdlet seguenti -'New-AzVirtualHubRoute' -'New-AzVirtualHubRouteTable' -'Add-AzVirtualHubRoute' -'Add-AzVirtualHubRouteTable' -'Get-AzVirtualHubRouteTable' -'Remove-AzVirtualHubRouteTable'
* Aggiunta la pianificazione per deprecare gli avvisi sull'argomento 'RouteTable' per i cmdlet seguenti -'New-AzVirtualHub' -'Set-AzVirtualHub' -'Update-AzVirtualHub'
* Resi facoltativi gli argomenti '-MinScaleUnits' e '-MaxScaleUnits' in 'Set-AzExpressRouteGateway'
* Aggiunti nuovi cmdlet per supportare l'autenticazione reciproca e i profili SSL nel gateway applicazione
...
#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Aggiunta la specifica che il criterio BackupTime è in UTC.
* Cambiato l'avviso della modifica di rilievo nel cmdlet Get-AzRecoveryServicesBackupJobDetails.
* Aggiornato il testo della Guida dello script di esempio per il cmdlet Set-AzRecoveryServicesBackupProtectionPolicy.

#### <a name="azresources"></a>Az.Resources
* Corretto un problema per cui What-If mostra due ambiti di gruppi di risorse con combinazioni di maiuscole/minuscole diverse
* Aggiornato 'Export-AzResourceGroup' per l'uso dell'SDK.
* Aggiunte informazioni sulle impostazioni cultura ai metodi di analisi

#### <a name="azsql"></a>Az.Sql
* Corretti i problemi per cui Set-AzSqlDatabaseAudit non supporta il database Hyperscale e non è possibile determinare l'edizione del database
* Aggiunto MaintenanceConfigurationId a 'New-AzSqlInstance' e 'Set-AzSqlInstance'
* Corretto un bug in GetAzureSqlDatabaseReplicationLink.cs per cui il parametro PartnerServerName viene cercato in base a valore invece che in base a chiave

#### <a name="azwebsites"></a>Az.Websites
* Aggiunto il supporto per nuove funzionalità di restrizione dell'accesso: ServiceTag, multi-ip e http-headers

### <a name="thanks-to-our-community-contributors"></a>Grazie ai collaboratori della community
* John Q. Martin (@johnmart82), aggiunte informazioni sui prerequisiti del firewall (#13385)
* Manikandan Duraisamy (@madurais-msft), corretto l'argomento PublicSubnetName (#13417)
* @mahortas, aggiornati i valori del parametro -HostNames (#13349)
* @MariachiForHire, aggiunti i valori TrafficAnalyticsInterval supportati (#13304)
* Michael James (@mikejwhat), aggiornato Get-AzLogicAppRunHistory per restituire più di 30 voci (#13393)
* Shashikant Shakya (@shshakya), aggiornato Restore-AzSqlInstanceDatabase.md (#13404)
