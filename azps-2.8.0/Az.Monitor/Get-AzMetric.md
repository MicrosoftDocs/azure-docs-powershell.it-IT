...
### GetWithDefaultParameters (impostazione predefinita)
```
...
## Descrizione
Il cmdlet **Get-AzMetric** ottiene i valori metrici per una risorsa specificata.
...
### Esempio 4: ottenere l'output riepilogato per una metrica specificata con il filtro di dimensione specificato
```
...
Questo comando ottiene l'output riepilogato per la metrica delle visualizzazioni di pagina con il filtro di dimensione e il tipo di aggregazione specificati.
...
### -MetricFilter
Specifica il filtro della dimensione metrica per la query delle metriche.
...
### -MetricNamespace
Specifica lo spazio dei nomi Metric a cui eseguire la query metriche.
...
### -OrderBy
Specifica l'aggregazione da usare per l'ordinamento dei risultati e la direzione dell'ordinamento (esempio: Sum ASC).
...
Altre informazioni sulle metriche supportate possono essere trovate in: https://docs.microsoft.com/en-us/azure/azure-monitor/platform/metrics-supported
