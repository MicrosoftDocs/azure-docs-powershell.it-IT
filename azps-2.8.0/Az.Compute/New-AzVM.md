...
Usa il cmdlet New-AzVMConfig per creare un oggetto macchina virtuale.
...
Puoi confermare lo stato di accesso usando il cmdlet **Get-AzSubscription** .

### Esempio 3: creare una VM da un'immagine Marketplace senza un IP pubblico
```
$VMLocalAdminUser = "LocalAdminUser"
$VMLocalAdminSecurePassword = ConvertTo-SecureString <password> -AsPlainText -Force
$LocationName = "westus"
...
Questo esempio serve una nuova rete e distribuisce una VM Windows dal Marketplace senza creare un indirizzo IP pubblico o un gruppo di sicurezza di rete.
Questo script può essere usato per il provisioning automatico perché usa le credenziali di amministratore della macchina virtuale locale in linea invece di chiamare **Get-Credential** che richiede l'interazione dell'utente.
...
### -EnableUltraSSD
Usare i dischi UltraSSD per la VM.
...
### -EvictionPolicy
Criteri di eliminazione per la macchina virtuale con priorità bassa.  Solo il valore supportato è "deallocate".
...
- Windows_Client
- Windows_Server questo valore non può essere aggiornato.
...
### -MaxPrice
Prezzo massimo della fatturazione di una macchina virtuale con priorità bassa.
...
### -Priorità
Priorità per la macchina virtuale.  Solo i valori supportati sono "regolari" e "bassi".
...
Nome o ID risorsa del gruppo di posizionamento di prossimità da usare con questa VM.
...
Se il parametro è presente, alla VM viene assegnata un'identità di sistema gestita generata automaticamente.
...
Il nome di un'identità del servizio gestito che deve essere assegnata alla VM.
