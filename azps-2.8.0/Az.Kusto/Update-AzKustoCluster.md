...
Il comando precedente aggiorna il livello del cluster kusto "mykustocluster" trovato nel gruppo di risorse "testrg".

### Esempio 2-aggiornare un cluster esistente tramite piping
...
Il comando precedente restituisce il cluster kusto "mykustocluster" nel gruppo di risorse "testrg" che usa il `Get-AzKustoCluster` cmdlet e quindi convoglia il risultato in per `Update-AzKustoCluster` aggiornare il livello del cluster in "standard".

## PARAMETRI

### -Capacità
Numero di istanza della VM.
...
### -Nome
Nome del cluster da aggiornare.
...
### -SkuName
Nome dell'USK usato per creare il cluster
...
### -Tier
Nome del livello usato per creare il cluster
