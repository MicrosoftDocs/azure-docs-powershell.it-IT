...
## Descrizione
Questo comando a esecuzione prolungata avvia una sessione di debug del flusso di dati per i comandi di debug imminenti. Questo comando può essere associato a una definizione di runtime di Integration per configurare le dimensioni e il tipo di cluster di sessione di debug.
