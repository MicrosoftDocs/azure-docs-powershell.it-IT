...
## Sinossi
Creare un oggetto driver per ImportExport.
...
### Esempio 1: creare una nuova unità per il processo di ImportExport
```powershell
...
Questi cmdlet creano una nuova unità per il processo di ImportExport.

## PARAMETRI

### -BitLockerKey
Chiave di BitLocker usata per crittografare l'unità.
...
Questo campo non viene restituito nella risposta finché l'unità non si trova nello stato di trasferimento.
...
### -IDUnità
Numero seriale hardware dell'unità, senza spazi.
...
### -ErrorLogUri
URI che punta al BLOB che contiene il log degli errori per l'operazione di trasferimento dei dati.
...
### -ManifestHash
Hash MD5 con codifica Base16 del file manifesto nell'unità.
...
### -ManifestUri
URI che punta al BLOB che contiene il file manifesto dell'unità.
...
### -PercentComplete
Percentuale completata per l'unità.
...
### -VerboseLogUri
URI che punta al BLOB che contiene il log dettagliato per l'operazione di trasferimento dei dati.
