...
### Esempio 1: aggiornare la distribuzione del cloud Spring per nome.
```powershell
...
Aggiornare la distribuzione di Spring cloud per nome.

### Esempio 2: aggiornare la distribuzione del cloud Spring da pipe.
```powershell
...
Aggiornare la distribuzione del cloud Spring da pipe.
...
### -MemoryInGb
Dimensioni della memoria necessarie in GB
...
### -SourceArtifactSelector
Selettore per l'elemento da usare per la distribuzione per i progetti a più moduli.
Questo dovrebbe essere il percorso relativo di Bethe per il modulo/progetto di destinazione.
...
### -SourceRelativePath
Percorso relativo dello spazio di archiviazione che archivia l'origine
