...
## Descrizione
Il cmdlet **New-AzFrontDoorBackendpoolsSettingObject** crea un nuovo oggetto PSBackendPoolsSettings per la creazione di porte anteriori.

## ESEMPI

### Esempio 1: creare un oggetto BackendPoolsSettings usando le impostazioni predefinite
```powershell
...
### Esempio 2: creare un oggetto BackendPoolsSettings con valori specificati dall'utente
```powershell
...
Se applicare il controllo nome certificato alle richieste HTTPS a tutti i pool back-end.
Nessun effetto sulle richieste non HTTPS.
...
Timeout di invio e ricezione su richiesta di inoltro al backend. Quando viene raggiunto il timeout, la richiesta non riesce e viene restituita.
