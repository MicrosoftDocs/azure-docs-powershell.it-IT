...
### Esempio 2: aggiungere un'API a un ApiVersionSet esistente
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Questo esempio aggiunge un'API a un set di versioni API esistente

### Esempio 3: cambiare il backend di ServiceUrl in cui punta l'API
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Questo esempio aggiorna l'ServiceUrl `echo-api` a cui punta.
