...
## Descrizione
Il cmdlet **New-AzNetAppFilesVolume** crea un volume di ANF.
...
Questo comando crea il nuovo volume ANF "MyAnfVolume" nel pool "MyAnfPool".
...
### -BackupId
ID backup. UUID V4 o identificatore delle risorse usato per identificare il backup
...
### -CreationToken
Percorso file univoco per il volume
...
### -KerberosEnabled
Descrivi se un volume è abilitato per Kerberos
...
### -ReplicationObject
Matrice Hashtable che rappresenta l'oggetto Replication
...
### -SecurityStyle
Stile di sicurezza di volume. I valori possibili includono:' NTFS ',' Unix '
...
Se Enabled (true) il volume conterrà una directory di sola lettura. snapshot che consente l'accesso a ogni snapshot del volume (impostazione predefinita: true)
...
### -SnapshotId
Creare un volume da uno snapshot. UUID V4 o identificatore delle risorse usato per identificare lo snapshot
...
### -SubnetId
URI di risorsa di Azure per una subnet delegata
...
### -ThroughputMibps
Velocità effettiva massima in Mibps che può essere ottenuta con questo volume
...
### -UsageThreshold
La quota di archiviazione massima consentita per un file System in byte
