...
### Esempio 7: creare un account di archiviazione con l'autenticazione dei servizi di dominio Active Directory Abilita file.
```
...
Questo comando crea un account di archiviazione withenable file di autenticazione del servizio di dominio Active Directory.

### Esempio 8: creare un account di archiviazione con il servizio coda e tabella usare la chiave di crittografia con ambito account e richiedere la crittografia dell'infrastruttura.
```powershell
...
Ottieni quindi le proprietà dell'account di archiviazione e visualizza il tipo di crittografia del servizio di Accodamento e tabella e il valore di RequireInfrastructureEncryption.

### Esempio 9: creare l'account MinimumTlsVersion e AllowBlobPublicAccess
```
...
Il comando Crea account con MinimumTlsVersion e AllowBlobPublicAccess e quindi Mostra le due proprietà dell'account creato 
...
Consentire l'accesso pubblico a tutti i BLOB o contenitori nell'account di archiviazione. L'interpretazione predefinita è true per questa proprietà.
...
### -MinimumTlsVersion
La versione minima di TLS consentita per le richieste di archiviazione. L'interpretazione predefinita è TLS 1,0 per questa proprietà.
...
Il servizio applica un livello secondario di crittografia con le chiavi gestite della piattaforma per i dati a riposo.
