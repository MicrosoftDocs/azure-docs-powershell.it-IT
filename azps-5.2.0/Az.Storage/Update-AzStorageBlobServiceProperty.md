...
## Descrizione
Il cmdlet **Update-AzStorageBlobServiceProperty** modifica le proprietà del servizio per il servizio BLOB di archiviazione di Azure.

## ESEMPI

### Esempio 1: impostare il servizio BLOB DefaultServiceVersion su 2018-03-28
```
...
Questo comando imposta il servizio DefaultServiceVersion di BLOB su 2018-03-28.

### Esempio 2: abilitare changefeed nel servizio BLOB di un account di archiviazione
```
...
Questo comando consente a changefeed su Blob Service di un account di archiviazione il supporto per la modifica del feed in Azure BLOB Storage funziona ascoltando un account di archiviazione di GPv2 o BLOB per eventuali eventi di creazione, modifica o eliminazione a livello di BLOB. Viene quindi restituito un log ordinato di eventi per i BLOB archiviati nel contenitore $blobchangefeed all'interno dell'account di archiviazione. Le modifiche serializzate vengono rese persistenti come file avro di Apache e possono essere elaborate in modo asincrono e incrementale.
...
Questo comando consente di abilitare il controllo delle versioni nel servizio BLOB di un account di archiviazione
...
### -DefaultServiceVersion
Versione predefinita del servizio da impostare
...
### -EnableChangeFeed
Abilitare la registrazione della modifica del feed per l'account di archiviazione impostando su $true, disabilitare la registrazione del feed di modifica impostando su $false.
