...
### Esempio 3: aggiornare ImmutabilityPolicy di un contenitore di BLOB di archiviazione
```
...
Questo comando Aggiorna ImmutabilityPolicy di un contenitore BLOB di archiviazione con l'oggetto contenitore di archiviazione 3 volte: prima di ImmutabilityPeriod 12 giorni senza eTag, quindi per ImmutabilityPeriod 9 giorni con ETag, infine abilitato AllowProtectedAppendWrite.
...
Questa proprietà può essere modificata solo per i criteri di conservazione basati sul tempo sbloccati. Con questa proprietà abilitata, i nuovi blocchi possono essere scritti in un BLOB di Accodamento mantenendo la protezione e la conformità dell'immutabilità. È possibile aggiungere solo nuovi blocchi ed eventuali blocchi esistenti non possono essere modificati o eliminati.
