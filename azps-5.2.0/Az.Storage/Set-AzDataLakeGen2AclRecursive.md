...
## Descrizione
Il cmdlet **set-AzDataLakeGen2AclRecursive** imposta gli ACL ricorsivamente nel percorso specificato. L'ACL di input sostituirà completamente l'ACL originale.

## ESEMPI

### Esempio 1: impostare gli ACL ricorsivamente in un directiry
```
...
Questo comando crea prima di tutto un oggetto ACL con 3 voci ACL, quindi imposta l'ACL in modo ricorsivo in una directory.

### Esempio 2: impostare gli ACL ricorsivamente in un directiry radice di filesystem
```
...
Questo comando imposta innanzitutto ACL in modo ricorsivo in una directory radice e non è riuscito, quindi Riprendi con ContinuationToken dopo che l'utente ha risolto il file non riuscito.

### Esempio 3: impostare ACL in modo ricorsivo per blocco
```
$token = $null
...
Questo script Imposta ACL rescursively in blocco directory per blocco, con dimensioni del blocco come BatchSize * MaxBatchCount. La dimensione del blocco è 200 in questo script.

### Esempio 4: impostare ACL ricorsivamente in una directory e in ContinueOnFailure, quindi riprendere da errori uno alla volta
```
...
Questo comando imposta innanzitutto ACL in modo ricorsivo in una directory con ContinueOnFailure e alcuni elementi non riescono, quindi riprende uno alla volta gli elementi non riusciti.
