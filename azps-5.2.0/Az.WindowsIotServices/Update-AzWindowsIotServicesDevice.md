...
## Sinossi
Aggiorna i metadati di un servizio per dispositivi Windows.
Il modello usuale per la modifica di una proprietà consiste nel recuperare i metadati del servizio di dispositivo Windows e i metadati di sicurezza e quindi combinarli con i valori modificati in un nuovo corpo per aggiornare il servizio Windows sacco.
...
### Esempio 1: aggiornare un servizio Windows per nome
```powershell
...
Questo comando aggiorna un servizio Windows per nome.

### Esempio 2: aggiornare un servizio Windows un sacco di servizi per pipeline
```powershell
...
Questo comando aggiorna un servizio Windows un sacco di servizi per pipeline.

## PARAMETRI

### -AdminDomainName
Domain Service OEM AAD di Windows per dispositivi
...
### -BillingDomainName
Domain Service ODM AAD di Windows sacco
...
Se viene fornito nel corpo della risposta, deve essere fornito anche come intestazione per la normale convenzione ETag.
...
### -IfMatch
ETag del servizio Windows sacco.
Non specificare per la creazione di un nuovo servizio Windows sacco.
Necessario per aggiornare un servizio Windows.
...
### -Nota
Note del servizio Windows sacco di dispositivi.
...
### -Quantità
Allocazione del dispositivo Windows sacco di servizi
