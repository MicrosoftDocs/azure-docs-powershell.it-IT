...
Se si crea o si importa una chiave con lo stesso nome di una chiave esistente nel Vault chiave, la chiave originale viene aggiornata con i valori specificati per la nuova chiave. Puoi accedere ai valori precedenti usando l'URI specifico della versione per la versione della chiave. Per informazioni sulle versioni chiave e sulla struttura URI, vedere [informazioni sulle chiavi e i segreti](http://go.microsoft.com/fwlink/?linkid=518560) nella documentazione dell'API REST di Key Vault.
...
### Esempio 7: generare una chiave di scambio chiave (KEK) per la caratteristica "porta la tua chiave" (BYOK)

```powershell
PS C:\> $key = Add-AzKeyVaultKey -VaultName $vaultName -Name $keyName -Destination HSM -Size 2048 -KeyOps "import"
```

Genera una chiave, denominata chiave di scambio chiave (KEK). La KEK deve essere una chiave RSA-HSM che contiene solo l'operazione chiave di importazione. La sola SKU Key Vault Premium supporta le chiavi RSA-HSM.
Per altre informazioni, consulta https://docs.microsoft.com/en-us/azure/key-vault/keys/hsm-protected-keys
...
- segno
- verificare
- Importa (solo per KEK, Vedi l'esempio 7)
