...
## Descrizione
Il cmdlet **Get-AzSqlDatabaseRestorePoint** recupera i punti di ripristino distinti a cui è possibile ripristinare un data warehouse SQL di Azure.
Per un database SQL di Azure, la finestra di ripristino è continua.
Ciò significa che qualsiasi momento nel periodo di conservazione del backup del database può essere usato come punto di ripristino.
Questo cmdlet è supportato anche dal servizio database stretch di SQL Server in Azure.

## ESEMPI

### Esempio 1: ottenere tutti i punti di ripristino
```
...
Questo comando restituisce tutti i punti di ripristino disponibili per il database SQL di Azure denominato Database01.
...
### -ResourceGroupName
Specifica il nome del gruppo di risorse a cui è assegnato il database SQL.
...
### -Nomeserver
Specifica il nome del server AzureSQL che ospita il database.
