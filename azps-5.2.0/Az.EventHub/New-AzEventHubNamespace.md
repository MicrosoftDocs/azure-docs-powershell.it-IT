...
Crea uno spazio dei nomi Hub di eventi MyNamespace \` \` nella posizione geografica specificata \` in location \` , nel gruppo di risorse \` MyResourceGroupName \` con Kafka e AutoInflate abilitato.
...
### Esempio 5: creazione dello spazio dei nomi con Gestisci identità in un cluster 
```powershell
...
### -ClusterARMId
ID ARM del cluster in cui deve essere creato lo spazio dei nomi
...
### -Identità
Abilitazione o disabilitazione dell'identità per lo spazio dei nomi
