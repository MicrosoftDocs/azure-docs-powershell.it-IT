...
## Descrizione
Questo cmdlet creerà una risorsa cluster gestita senza tipi di nodi. Per avviare il cluster è necessario aggiungere un tipo di nodo primario per usare [New-AzServiceFabricManagedNodeType](./New-AzServiceFabricManagedNodeType.md).
...
Questo comando crea una risorsa cluster con SKU di base predefinito.
...
Questo comando crea una risorsa cluster in centraluseuap con un certificato client di amministrazione iniziale e un SKU standard.
...
### -ClientCertIsAdmin
Consente di specificare se il certificato client ha un livello di amministratore.
...
Elenco delle identificazioni personali dell'autorità di certificazione per il certificato client.
Usare solo in combinazione con ClientCertCommonName.
...
### -Codeversion
Versione del codice del servizio cluster.
...
Endpoint usato dal proxy inverso.
...
### -SKU
SKU del cluster, le opzioni sono di base: avrà un minimo di 3 nodi di inizializzazione e consentirà solo un tipo di nodo e uno standard: avrà un minimo di 5 nodi di inizializzazione e consentirà più tipi di nodi.
...
### -UpgradeMode
Modalità di aggiornamento della versione del codice del servizio cluster.
...
### -UseTestExtension
Se specifichi che il cluster verrà incassato con l'estensione vmss test di servizio.
