...
### Esempio 1: creare un peering VNET per databricks
```powershell
...
Questo comando crea un peering VNET per databricks.

## PARAMETRI

### -AllowForwardedTraffic
Se il traffico inoltrato dalle VM nella rete virtuale locale sarà consentito/non consentito in una rete virtuale remota.
...
### -AllowGatewayTransit
Se i collegamenti del gateway possono essere usati in una rete virtuale remota per creare un collegamento a questo network virtuale.
...
Se le VM nello spazio di rete virtuale locale sarebbero in grado di accedere alle VM nello spazio di rete virtuale remoto.
...
ID della rete virtuale databricks.
...
### -UseRemoteGateway
Se i gateway remoti possono essere usati in questa rete virtuale.
