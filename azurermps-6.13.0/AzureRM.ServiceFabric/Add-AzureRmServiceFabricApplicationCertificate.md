...
## Descrizione
Usare **Add-AzureRmServiceFabricApplicationCertificate** per installare un certificato in tutti i nodi del cluster. Puoi specificare un certificato che hai già o avere il sistema per generarne uno nuovo e caricarlo in un Vault di chiave di Azure nuovo o esistente.
...
Questo comando aggiungerà un certificato dall'archivio di chiavi di Azure esistente a tutti i tipi di nodo del cluster.
...
Questo comando creerà un certificato autofirmato nel Vault di chiave di Azure con il nome del gruppo di risorse del Vault chiave e il nome della chiave del Vault, viene installato in tutti i tipi di nodo del cluster e Scarica il certificato nella cartella "c:\test". Il nome del certificato scaricato è uguale al nome del certificato di volta chiave.

## PARAMETRI

### -CertificateFile
Percorso del file del certificato esistente.
...
Percorso cartella del nuovo certificato da creare.
...
Nome DNS del certificato da creare.
...
### -VAULTNAME
Nome del Vault della chiave di Azure.
...
Nome del gruppo di risorse di Azure Key Vault.
...
### -SecretIdentifier
URI segreto del Vault Key di Azure esistente.
...
### System. String
Parametri: CertificateFile (ByValue), CertificateOutputFolder (ByValue), CertificateSubjectName (ByValue), VAULTNAME (ByValue), KeyVaultResouceGroupName (ByValue), SecretIdentifier (ByValue)

### System. Security. SecureString
Parametri: CertificatePassword (ByValue)
