...
## Descrizione
Il cmdlet **Add-AzureKeyVaultCertificate** avvia il processo di registrazione di un certificato in un Vault chiave in Azure Key Vault.
...
Il primo comando usa il cmdlet New-AzureKeyVaultCertificatePolicy per creare un criterio di certificato e lo archivia nella variabile $Policy.
Il secondo comando USA **Add-AzureKeyVaultCertificate** per avviare il processo per creare un certificato.
Il terzo comando usa il cmdlet Get-AzureKeyVaultCertificateOperation per eseguire il polling dell'operazione per verificare che sia completa.
Il comando finale usa il cmdlet Get-AzureKeyVaultCertificate per ottenere il certificato.
...
Parametri: CertificatePolicy (ByValue)
