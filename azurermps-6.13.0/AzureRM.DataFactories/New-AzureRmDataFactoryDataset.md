...
## Descrizione
Il cmdlet **New-AzureRmDataFactoryDataset** crea un DataSet in Azure Data Factory.
...
Il primo comando crea un DataSet denominato DA_WikipediaClickEvents, come in un esempio precedente, e quindi assegna tale DataSet alla variabile $Dataset.
Il secondo comando usa la notazione standard del punto per visualizzare i dettagli sulla proprietà Availability del DataSet.

### Esempio 3: visualizzare la posizione di un nuovo set di dati
```
PS C:\>$Dataset = New-AzureRmDataFactoryDataset -ResourceGroupName "ADF" -DataFactoryName "WikiADF" -Name "DAWikipediaClickEvents" -File "C:\\samples\\WikiSample\\DA_WikipediaClickEvents.json"
...
Il primo comando crea un DataSet denominato DA_WikipediaClickEvents, come in un esempio precedente, e quindi assegna tale DataSet alla variabile $Dataset.
Il secondo comando Visualizza i dettagli sulla proprietà Location del DataSet.
...
Il primo comando crea un DataSet denominato DA_WikipediaClickEvents, come in un esempio precedente, e quindi assegna tale DataSet alla variabile $Dataset.
Il secondo comando consente di ottenere informazioni dettagliate sulle regole di convalida per il DataSet e quindi di passarle al cmdlet Format-List usando l'operatore pipeline.
...
### -File
Specifica il percorso completo del file JSON (JavaScript Object Notation) che contiene la descrizione del DataSet.
...
### -Force
Indica che questo cmdlet sostituisce un set di dati esistente senza richiedere conferma.
