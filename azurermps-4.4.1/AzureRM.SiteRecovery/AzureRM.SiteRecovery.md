...
Ottenere le proprietà di un tessuto di recupero del sito di Azure.
...
Ottenere gli elementi protettivi in un contenitore di protezione.

### [Get-AzureRmSiteRecoveryProtectionContainer](Get-AzureRmSiteRecoveryProtectionContainer.md)
Ottiene i contenitori di protezione per il Vault di ripristino del sito corrente.
...
Ottiene le informazioni sulle impostazioni per il Vault di ripristino del sito corrente.
...
Crea un criterio di replica del ripristino del sito.
...
Abilita la replica per un elemento protettivo creando un elemento protetto
...
Rimuove un server di ripristino del sito dall'archivio corrente.
...
Imposta le proprietà di ripristino, ad esempio la rete di destinazione e la dimensione della macchina virtuale per un elemento protetto da replica.
...
{{Immettere manualmente Update-AzureRmSiteRecoveryPolicy descrizione qui}}

### [Update-AzureRmSiteRecoveryProtectionDirection](Update-AzureRmSiteRecoveryProtectionDirection.md)
Aggiorna il server di origine e di destinazione per la protezione di un oggetto di ripristino del sito.
