...
### Specificare il nome host e la porta (impostazione predefinita)
```
...
## Descrizione
Il cmdlet New-AzureRmDataLakeAnalyticsCatalogCredential crea una nuova credenziale da usare in un catalogo di dati di analisi di Azure Data Lake per la connessione a origini dati esterne.

## ESEMPI

### Esempio 1: creare una credenziale per un catalogo che specifica l'host e la porta
```
...
### Esempio 2: creare una credenziale per un catalogo che specifica l'URI completo
```
PS C:\> New-AzureRmDataLakeAnalyticsCatalogCredential -AccountName "ContosoAdlAccount" `
                  -DatabaseName "databaseName" `
                  -CredentialName "exampleDbCred" `
...
Questo comando crea le credenziali specificate per l'account, il database e l'URI dell'origine dati esterni specificati.
...
### -Credenziale
Specifica il nome utente e la password corrispondente della credenziale.
...
### -CredentialName
Specifica il nome e la password della credenziale.
...
### -DatabaseName
Specifica il nome del database nel acocunt di analisi dei dati del lago in cui verranno archiviate le credenziali.
