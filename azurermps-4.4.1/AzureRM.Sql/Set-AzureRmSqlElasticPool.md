...
## Descrizione
Il cmdlet **set-AzureRmSqlElasticPool** modifica le proprietà di un pool di database elastici in un database SQL di Azure. Questo cmdlet può modificare le unità minime di throughput del database (DTU) per ogni database, oltre al massimo DTU per database, al numero di DTU per il pool e al limite di spazio di archiviazione per il pool.
...
### Esempio 2: modificare lo spazio di archiviazione max di un pool elastico
```
...
### -Tag
Specifica un dizionario di coppie chiave-valore che questo cmdlet associa al pool elastico in forma di tabella hash. Per esempio:
