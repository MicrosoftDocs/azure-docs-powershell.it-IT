...
## Sinossi
Ottiene l'output del log per un processo.
...
Il cmdlet **Get-AzureHDInsightJobOutput** Ottiene l'output del log per un processo dall'account di archiviazione associato a un cluster.
È possibile ottenere vari tipi di log dei processi, tra cui output standard, errore standard, registri delle attività e riepilogo dei registri delle attività.

## ESEMPI

### Esempio 1: ottenere l'output del processo
```
...
Il secondo comando Archivia il nome cluster nella variabile $Clustername.

Il terzo comando crea una definizione di processo MapReduce e la archivia nella variabile $WordCountJob.
Il comando passa il processo in $WordCountJob al cmdlet **Start-AzureHDInsightJob** per avviare il processo.
Passa inoltre $WordCountJob al cmdlet **wait-AzureHDInsightJob** per attendere il completamento del processo e quindi usa **Get-AzureHDInsightJobOutput** per ottenere l'output del processo.
...
Questo cmdlet consente di ottenere i log dei processi dal cluster specificato da questo parametro.
...
### -DownloadTaskLogs
Indica che questo cmdlet ottiene i registri delle attività per un processo.
...
### -StandardError
Indica che questo cmdlet ottiene l'output StdErr di un processo.
...
### -StandardOutput
Indica che questo cmdlet ottiene l'output SdtOut di un processo.
...
### -TaskLogsDirectory
Specifica una cartella locale in cui archiviare i registri delle attività.
...
### -TaskSummary
Indica che questo cmdlet ottiene il riepilogo del log attività.
