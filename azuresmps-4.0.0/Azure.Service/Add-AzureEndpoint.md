...
Questo cmdlet aggiunge un endpoint denominato Httpin.
L'endpoint ha una porta pubblica 80 e la porta locale 8080.
Il comando passa l'oggetto macchina virtuale al cmdlet **Update-AzureVM** , che implementa le modifiche.

### Esempio 2: aggiungere un endpoint appartenente a un gruppo di bilanciamento del carico
```
...
Questo comando Recupera la configurazione di una macchina virtuale denominata VirtualMachine07.
Il cmdlet corrente aggiunge un endpoint denominato Httpin.
L'endpoint ha una porta pubblica 80 e la porta locale 8080.
L'endpoint appartiene al gruppo di bilanciamento del carico condiviso denominato webfarm.
Una sonda HTTP sulla porta 80 con un percorso di "/" monitora la disponibilità dell'endpoint.
Il comando implementa le modifiche.

### Esempio 3: associare un IP virtuale a un endpoint
```
...
Questo comando Recupera la configurazione di una macchina virtuale denominata VirtualMachine25.
Il cmdlet corrente aggiunge un endpoint denominato Httpin.
L'endpoint ha una porta pubblica 80 e la porta locale 8080.
Questo comando associa un IP virtuale all'endpoint.
Il comando implementa le modifiche.
...
### -DefaultProbe
Indica che questo cmdlet usa l'impostazione di probe predefinita.
...
### -Noprobe
Indica che questo cmdlet usa l'impostazione No Probe.
...
Specifica l'intervallo di polling Probe, in secondi, per l'endpoint.
...
### -ProbeProtocol
Specifica il protocollo della porta.
I valori validi sono: 
...
Specifica il periodo di timeout del polling probe in secondi.
