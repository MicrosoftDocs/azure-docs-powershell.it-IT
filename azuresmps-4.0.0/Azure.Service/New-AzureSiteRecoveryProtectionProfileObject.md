...
## Sinossi
Crea un oggetto del profilo di protezione del ripristino del sito.
...
## Descrizione
Il cmdlet **New-AzureSiteRecoveryProtectionProfileObject** crea un oggetto profilo di protezione del ripristino di Azure site.
Questo cmdlet crea un oggetto **ASRProtectionProfile** da usare con altri cmdlet.

## ESEMPI

### Esempio 1: creare un profilo di protezione
```
...
Questo comando crea un oggetto profilo di protezione.

### Esempio 2: creare un profilo di protezione per il provider di HyperVReplicaAzure
```
...
Questo comando crea un profilo di protezione per un provider di HyperVReplicaAzure.

## PARAMETRI

### -AllowReplicaDeletion
Indica che il profilo di protezione consente l'eliminazione delle entità di replica.
...
Specifica la frequenza, in ore, per gli snapshot coerenti dell'applicazione.
...
### -Autenticazione
Specifica il tipo di autenticazione da usare.
I valori accettabili per questo parametro sono: certificate e Kerberos.
...
### -CompressionEnabled
Indica che il profilo di protezione consente la compressione.
...
### -Nome
Specifica un nome per il profilo di protezione.
...
Specifica il nome di un account di archiviazione di Azure in cui archiviare l'entità di replica di Azure.
...
Specifica l'ID di un abbonamento a Azure per un account di archiviazione.
Questo parametro fa riferimento all'account in cui archiviare l'entità di replica di Azure.
...
Specifica l'intervallo di frequenza, in secondi, per la replica. I valori accettabili per questo parametro sono i seguenti:
...
### -ReplicationMethod
Specifica il metodo di replica. I valori accettabili per questo parametro sono i seguenti:

- Online.
Replica tramite la rete.
- Offline.
...
### -ReplicationPort
Specifica il numero della porta in cui si trova la replica.
...
### -ReplicationProvider
Specifica il tipo di provider di replica. I valori accettabili per questo parametro sono: HyperVReplica e HyperVReplicaAzure.
...
Specificare un'ora entro 24 ore dall'inizio del processo.
