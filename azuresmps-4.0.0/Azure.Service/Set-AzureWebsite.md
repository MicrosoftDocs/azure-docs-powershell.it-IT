...
## Sinossi
Configura un sito Web in uso in Azure.
...
Il cmdlet **set-AzureWebsite** configura un sito Web in uso in Azure.

## ESEMPI

### Esempio 1: abilitare la registrazione HTTP per un sito Web
```
PS C:\> Set-AzureWebsite -HttpLoggingEnabled 1
```

Questo esempio Abilita la registrazione HTTP.

### Esempio 2: impostare le credenziali di archiviazione per un sito Web
```
...
Questo esempio imposta le credenziali di archiviazione in un sito Web denominato sito Web con variabili di ambiente per AZURE_STORAGE_ACCOUNT e AZURE_STORAGE_ACCESS_KEY.

## PARAMETRI

### -AppSettings
Specifica le variabili di ambiente che verranno usate dal sito Web.
...
### -ConnectionStrings
Specifica le stringhe di connessione usate dal sito Web.
...
### -DefaultDocuments
Specifica i documenti che vengono visualizzati automaticamente durante l'esplorazione del sito Web.
...
Determina se sono stati registrati errori di IIS dettagliati per il sito Web.
...
### -HandlerMappings
Specifica i mapping dei gestori usati dal sito Web.
...
### -HostName
Specifica i nomi host completi che possono essere usati per accedere al sito Web.
...
### -HttpLoggingEnabled
Determina se la registrazione http è abilitata per il sito Web.
...
### -Metadata
Specifica i metadati per il sito Web.
...
### -NetFrameworkVersion
Specifica la versione di .NET Framework necessaria per il sito Web.
...
### -NumberOfWorkers
Specifica il numero di processi di lavoro in cui è in corso il sito Web.
...
### -PassThru
Indica che questo cmdlet restituisce un valore **booleano** .
...
### -PhpVersion
Specifica la versione PHP richiesta dal sito Web.
...
Determina se la traccia delle richieste è abilitata per il sito Web.
...
### -RoutingRules
Specifica le regole di routing da usare per i test in produzione.
...
### -SiteWithConfig
Specifica la configurazione usata dal sito Web.
...
Specifica se abilitare la modalità a 32 bit.
...
### -WebSocketsEnabled
Specifica se abilitare WebSockets.
