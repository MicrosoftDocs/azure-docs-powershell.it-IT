...
## Descrizione
Il cmdlet **Get-AzureSiteRecoveryVaultSettingsFile** ottiene il file di impostazioni per un Vault di ripristino di Azure site.

## ESEMPI

### Esempio 1: recuperare il file di impostazioni per un Vault
```
...
Il primo comando ottiene il Vault di ripristino del sito di Azure attivo denominato ContosoVault usando il cmdlet **Get-AzureSiteRecoveryVault** .
Il comando Archivia il Vault nella variabile $Vault.

Il secondo comando ottiene il file di impostazioni per il Vault archiviato in $Vault.

## PARAMETRI

### -Posizione
Specifica la posizione geografica a cui appartiene il Vault.
...
Per archiviare il file in locale, scaricarlo dal portale del Vault Recovery del sito dopo che il comando è terminato.
...
### -Sito
Specifica un sito per cui ottenere un file di impostazioni.
Per ottenere un oggetto **sito** , usare il cmdlet **Get-AzureSiteRecoverySite** .
...
### -Vault
Specifica il Vault per il sito.
